...
#include "Module/Util/localization.h"
#include <string>
#include <functional>
...
namespace pf::gui
{
	struct EventArgs
	{
		...
		pf::Color color;		// color value of color picker operation
	...
	struct Theme
	{
		// Reduced version of pf::image::Params, excluding position, alignment, etc.
		struct Image
		{
			inline static const pf::image::Params params; // prototype for default values
			XMFLOAT4 color = params.color;
			pf::enums::BLENDMODE blendFlag = params.blendFlag;
			pf::image::SAMPLEMODE sampleFlag = params.sampleFlag;
			pf::image::QUALITY quality = params.quality;
			bool background = params.isBackgroundEnabled();
			bool corner_rounding = params.isCornerRoundingEnabled();
			pf::image::Params::Rounding corners_rounding[arraysize(params.corners_rounding)];
			bool highlight = false;
			XMFLOAT3 highlight_color = XMFLOAT3(1, 1, 1);
			float highlight_spread = 1;
			float border_soften = 0;

			void Apply(pf::image::Params& params) const
			{
				params.color = color;
			...
			void CopyFrom(const pf::image::Params& params)
			{
				color = params.color;
		...
		// Reduced version of pf::font::Params, excluding position, alignment, etc.
		struct Font
		{
			inline static const pf::font::Params params; // prototype for default values
			pf::Color color = params.color;
			pf::Color shadow_color = params.shadowColor;
			...
			void Apply(pf::font::Params& params) const
			{
				params.color = color;
			...
			void CopyFrom(const pf::font::Params& params)
			{
				color = params.color;
		...
		pf::Color shadow_color = pf::Color::Shadow(); // shadow color for whole widget
		...
		pf::Color tooltip_shadow_color = pf::Color::Shadow();
	};
	...
	{
	private:
		pf::vector<Widget*> widgets;
		bool focus = false;
		bool visible = true;
	public:

		void Update(const pf::Canvas& canvas, float dt);
		void Render(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const;
		...
		void SetColor(pf::Color color, int id = -1);
		void SetImage(pf::Resource resource, int id = -1);
		void SetShadowColor(pf::Color color);
		void SetTheme(const Theme& theme, int id = -1);

		void ExportLocalization(pf::Localization& localization) const;
		void ImportLocalization(const pf::Localization& localization);
	};

	class Widget : public pf::scene::TransformComponent
	{
	...
	protected:
		...
		pf::Color shadow_color = pf::Color::Shadow();
		bool shadow_highlight = false;
		XMFLOAT3 shadow_highlight_color = XMFLOAT3(1, 1, 1);
		float shadow_highlight_spread = 1;
		...
		pf::Color tooltip_shadow_color = pf::Color::Shadow();
		mutable pf::Sprite tooltipSprite;
		mutable pf::SpriteFont tooltipFont;
		mutable pf::SpriteFont scripttipFont;
	...
	public:
		...
		virtual void Update(const pf::Canvas& canvas, float dt);
		virtual void Render(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const;
		virtual void RenderTooltip(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const;
		...
		virtual void SetColor(pf::Color color, int id = -1);
		virtual void SetShadowColor(pf::Color color);
		virtual void SetImage(pf::Resource textureResource, int id = -1);
		virtual void SetTheme(const Theme& theme, int id = -1);
		virtual const char* GetWidgetTypeName() const { return "Widget"; }

		pf::Sprite sprites[WIDGETSTATE_COUNT];
		pf::SpriteFont font;

		XMFLOAT3 translation = XMFLOAT3(0, 0, 0);
		XMFLOAT3 scale = XMFLOAT3(1, 1, 1);

		pf::primitive::Hitbox2D hitBox;
		pf::graphics::Rect scissorRect;
		...
		void ApplyScissor(const pf::Canvas& canvas, const pf::graphics::Rect rect, pf::graphics::CommandList cmd, bool constrain_to_parent = true) const;
		pf::primitive::Hitbox2D GetPointerHitbox(bool constrained = true) const;
		XMFLOAT2 GetPointerHighlightPos(const pf::Canvas& canvas) const;

		pf::primitive::Hitbox2D active_area; // Pointer hitbox constrain area
		void HitboxConstrain(pf::primitive::Hitbox2D& hb) const;
		...
		virtual void ExportLocalization(pf::Localization& localization) const;
		virtual void ImportLocalization(const pf::Localization& localization);
	...
	public:
		...
		void Update(const pf::Canvas& canvas, float dt) override;
		void Render(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const override;
		...
		pf::SpriteFont font_description;
	...
	public:
		...
		pf::Sprite sprites_knob[SCROLLBAR_STATE_COUNT];
		XMFLOAT2 knob_inset_border = {};

		void Update(const pf::Canvas& canvas, float dt) override;
		void Render(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const override;
		void SetColor(pf::Color color, int id = -1) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	protected:
		...
		static pf::SpriteFont font_input;
	...
	public:
		...
		pf::Sprite sprites_knob[WIDGETSTATE_COUNT];
		...
		void Update(const pf::Canvas& canvas, float dt) override;
		void Render(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const override;
		void RenderTooltip(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const override;
		void SetColor(pf::Color color, int id = -1) override;
		void SetTheme(const Theme& theme, int id = -1) override;
	...
	protected:
		...
		pf::vector<Item> items;

		pf::Color drop_color = pf::Color::Ghost();
		...
		float GetDropOffset(const pf::Canvas& canvas) const;
		float GetDropX(const pf::Canvas& canvas) const;
		float GetItemOffset(const pf::Canvas& canvas, int index) const;
	public:
		void Create(const std::string& name);
		...
		pf::SpriteFont selected_font;

		void ExportLocalization(pf::Localization& localization) const override;
		void ImportLocalization(const pf::Localization& localization) override;
	...
	protected:
		...
		struct Layout
		{
			...
			void add_right(pf::gui::Widget& widget)
			{
				if (!widget.IsVisible())
					return;
				const XMFLOAT2 size = widget.GetSize();
			...
			void add_fullwidth(pf::gui::Widget& widget)
			{
				if (!widget.IsVisible())
					return;
				const XMFLOAT2 size = widget.GetSize();
			...
			void add_fullwidth_aspect(pf::gui::Widget& widget)
			{
				if (!widget.IsVisible())
					return;
				x = padding;
				...
				const pf::Resource& res = widget.sprites[widget.GetState()].textureResource;
				if (res.IsValid())
				{
					const pf::graphics::Texture& tex = res.GetTexture();
			...
			void add_right(pf::gui::Widget& widget, Args&&... args)
			{
			...
			void helper_fitx(float sizx, pf::gui::Widget& widget)
			{
			...
			template<typename... Args>
			void helper_fitx(float sizx, pf::gui::Widget& widget, Args&&... args)
			{
			...
			template<typename... Args>
			void add(pf::gui::Widget& widget, Args&&... args)
			{
	...
	public:
		...
		void RenderTooltip(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const override;
		void SetColor(pf::Color color, int id = -1) override;
		void SetImage(pf::Resource resource, int id = -1) override;
		void SetShadowColor(pf::Color color) override;
		void SetTheme(const Theme& theme, int id = -1) override;
		...
		pf::graphics::Texture background_overlay;
	};
	...
	public:
		...
		pf::Color GetPickColor() const;
		void SetPickColor(pf::Color value);
	...
	protected:
		...
		pf::primitive::Hitbox2D GetHitbox_ListArea() const;
		pf::primitive::Hitbox2D GetHitbox_Item(int visible_count, int level) const;
		pf::primitive::Hitbox2D GetHitbox_ItemOpener(int visible_count, int level) const;
...
template<>
struct enable_bitmask_operators<pf::gui::Window::WindowControls> {
	static const bool enable = true;
};

template<>
struct enable_bitmask_operators<pf::gui::Window::AttachmentOptions> {
	static const bool enable = true;
};

template<>
struct enable_bitmask_operators<pf::gui::LocalizationEnabled> {
	static const bool enable = true;
};
