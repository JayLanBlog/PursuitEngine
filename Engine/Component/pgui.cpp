...
namespace pf::gui
{
	...
	{
		pf::graphics::PipelineState PSO_colored;

		InternalState()
		{
			pf::Timer timer;
			static pf::eventhandler::Handle handle = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [this](uint64_t userdata) { LoadShaders(); });
			LoadShaders();
			p_log("pf::gui Initialized (%d ms)", (int)std::round(timer.elapsed()));
		}

		void LoadShaders()
		{
			PipelineStateDesc desc;
			desc.vs = pf::renderer::GetShader(pf::enums::VSTYPE_VERTEXCOLOR);
			desc.ps = pf::renderer::GetShader(pf::enums::PSTYPE_VERTEXCOLOR);
			desc.il = pf::renderer::GetInputLayout(pf::enums::ILTYPE_VERTEXCOLOR);
			desc.dss = pf::renderer::GetDepthStencilState(pf::enums::DSSTYPE_DEPTHDISABLED);
			desc.bs = pf::renderer::GetBlendState(pf::enums::BSTYPE_TRANSPARENT);
			desc.rs = pf::renderer::GetRasterizerState(pf::enums::RSTYPE_DOUBLESIDED);
			desc.pt = PrimitiveTopology::TRIANGLESTRIP;
			pf::graphics::GetDevice()->CreatePipelineState(&desc, &PSO_colored);
		}
	};
	...
	void GUI::Update(const pf::Canvas& canvas, float dt)
	{
		if (!visible || pf::backlogger::isActive())
		{
			return;
		}

		auto range = pf::profiler::BeginRangeCPU("GUI Update");
		scroll_allowed = true;

		XMFLOAT4 pointer = pf::input::GetPointer();
	...
	void GUI::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		...
		auto range_cpu = pf::profiler::BeginRangeCPU("GUI Render");
		auto range_gpu = pf::profiler::BeginRangeGPU("GUI Render", cmd);
		...
		pf::profiler::EndRange(range_cpu);
		pf::profiler::EndRange(range_gpu);
	}
	...
	void GUI::SetColor(pf::Color color, int id)
	{
	...
	void GUI::SetImage(pf::Resource resource, int id)
	{
		for (auto& widget : widgets)
		{
	...
	void GUI::SetShadowColor(pf::Color color)
	{
		for (auto& widget : widgets)
		{
	...
	void GUI::ExportLocalization(pf::Localization& localization) const
	{
		pf::Localization& section = localization.GetSection("gui");
		for (auto& widget : widgets)
		{
	...
	void GUI::ImportLocalization(const pf::Localization& localization)
	{
		const pf::Localization* section = localization.CheckSection("gui");
		if (section == nullptr)
			return;
		for (auto& widget : widgets)
		{
	...
	Widget::Widget()
	{
		sprites[IDLE].params.color = pf::Color::Booger();
		sprites[FOCUS].params.color = pf::Color::Gray();
		sprites[ACTIVE].params.color = pf::Color::White();
		sprites[DEACTIVATING].params.color = pf::Color::Gray();
		font.params.shadowColor = pf::Color::Shadow();
		font.params.shadow_bolden = 0.2f;
		font.params.shadow_softness = 0.2f;

		tooltipSprite.params.color = pf::Color(255, 234, 165);
		tooltipFont.params.color = pf::Color(25, 25, 25, 255);

		for (int i = IDLE; i < WIDGETSTATE_COUNT; ++i)
		{
			sprites[i].params.blendFlag = pf::enums::BLENDMODE_OPAQUE;
	...
	void Widget::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		scale = pf::math::Max(scale, XMFLOAT3(0.001f, 0.001f, 0.001f));
	...
	void Widget::Render(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
	...
	void Widget::RenderTooltip(const pf::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		if (tooltipTimer > 25)
		{
			...
			tooltipFont.params.h_align = pf::font::WIFALIGN_LEFT;
			tooltipFont.params.v_align = pf::font::WIFALIGN_TOP;
	...
	void Widget::SetSize(const XMFLOAT2& value)
	{
		...
		scale_local = pf::math::Max(scale_local, XMFLOAT3(0.001f, 0.001f, 0.001f));
		UpdateTransform();
	...
	pf::Color Widget::GetColor() const
	{
		return pf::Color::fromFloat4(sprites[GetState()].params.color);
	}
	...
	void Widget::ExportLocalization(pf::Localization& localization) const
	{
		...
		pf::Localization& section = localization.GetSection(GetName());
	...
	void Widget::ImportLocalization(const pf::Localization& localization)
	{
		const pf::Localization* section = localization.CheckSection(GetName());
		if (section == nullptr)
			return;
	...
	void Widget::SetColor(pf::Color color, int id)
	{
		if (id < 0)
		{
	...
	void Widget::SetShadowColor(pf::Color color)
	{
		shadow_color = color;
	}
	void Widget::SetImage(pf::Resource textureResource, int id)
	{
		if (id < 0)
		{
			for (int i = 0; i < arraysize(sprites); ++i)
	...
	void Widget::ApplyScissor(const pf::Canvas& canvas, const Rect rect, CommandList cmd, bool constrain_to_parent) const
	{
		Rect scissor = rect;
	...
	void Widget::HitboxConstrain(pf::primitive::Hitbox2D& hb) const
	{
		float left = hb.pos.x;
	...
	XMFLOAT2 Widget::GetPointerHighlightPos(const pf::Canvas& canvas) const
	{
		XMFLOAT4 pointer = pf::input::GetPointer();
	...
	void Button::Create(const std::string& name)
	{
		...
		font.params.h_align = pf::font::WIFALIGN_CENTER;
		font.params.v_align = pf::font::WIFALIGN_CENTER;

		font_description.params = font.params;
		font_description.params.h_align = pf::font::WIFALIGN_RIGHT;
	}
	void Button::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (pf::input::Press(pf::input::MOUSE_BUTTON_LEFT))
			{
				if (state == FOCUS)
				{
					// activate
			...
			if (pf::input::Down(pf::input::MOUSE_BUTTON_LEFT))
			{
				if (state == DEACTIVATING)
				{
		...
		{
		case pf::font::WIFALIGN_LEFT:
			font.params.posX = translation.x + 2;
			break;
		case pf::font::WIFALIGN_RIGHT:
			font.params.posX = translation.x + scale.x - 2;
			break;
		case pf::font::WIFALIGN_CENTER:
		default:
		...
		{
		case pf::font::WIFALIGN_TOP:
			font.params.posY = translation.y + 2;
			break;
		case pf::font::WIFALIGN_BOTTOM:
	...
	void Button::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
		...
		if (shadow > 0)
		{
			pf::image::Params fx = sprites[state].params;
			fx.gradient = pf::image::Params::Gradient::None;
			fx.pos.x -= shadow;
			fx.pos.y -= shadow;
			fx.siz.x += shadow * 2;
			fx.siz.y += shadow * 2;
	...
	void Image::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		if (!IsVisible())
		{
			return;
		...
		if (shadow > 0)
		{
			pf::image::Params fx = sprites[IDLE].params;
			fx.gradient = pf::image::Params::Gradient::None;
			fx.pos.x -= shadow;
			fx.pos.y -= shadow;
			fx.siz.x += shadow * 2;
	...
	void ScrollBar::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (IsScrollbarRequired() && hitBox.intersects(pointerHitbox))
			{
				if (clicked)
				{
					...
					grab_pos.x = pf::math::Clamp(grab_pos.x, scrollbar_begin + scrollbar_delta, scrollbar_begin + scrollbar_delta + scrollbar_length);
					grab_pos.y = pf::math::Clamp(grab_pos.y, scrollbar_begin + scrollbar_delta, scrollbar_begin + scrollbar_delta + scrollbar_length);
		...
		scrollbar_delta = pf::math::Clamp(scrollbar_delta, 0, scrollbar_size - scrollbar_length);
		if (scrollbar_begin < scrollbar_end - scrollbar_length)
		{
			scrollbar_value = pf::math::InverseLerp(scrollbar_begin, scrollbar_end - scrollbar_length, scrollbar_begin + scrollbar_delta);
		}
		else
		{
			scrollbar_value = 0;
	...
	void ScrollBar::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
		...
		if (shadow > 0)
		{
			pf::image::Params fx = sprites_knob[scrollbar_state].params;
			fx.gradient = pf::image::Params::Gradient::None;
			fx.pos.x -= shadow;
			fx.pos.y -= shadow;
			fx.siz.x += shadow * 2;
		...
		//GraphicsDevice* device = pf::graphics::GetDevice();
		//device->BindScissorRects(1, &scissorRect, cmd);
		//pf::image::Draw(nullptr, pf::image::Params(hitBox.pos.x, hitBox.pos.y, hitBox.siz.x, hitBox.siz.y, pf::Color(255,0,0,100)), cmd);

	}
	void ScrollBar::SetColor(pf::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void Label::Create(const std::string& name)
	{
		...
		scrollbar.SetColor(pf::Color(80, 80, 80, 100), pf::gui::IDLE);
		scrollbar.sprites_knob[ScrollBar::SCROLLBAR_INACTIVE].params.color = pf::Color(140, 140, 140, 140);
		scrollbar.sprites_knob[ScrollBar::SCROLLBAR_HOVER].params.color = pf::Color(180, 180, 180, 180);
		scrollbar.sprites_knob[ScrollBar::SCROLLBAR_GRABBED].params.color = pf::Color::White();
		scrollbar.SetOverScroll(0.25f);
		scrollbar.knob_inset_border = XMFLOAT2(4, 2);
	}
	void Label::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (scroll_allowed && scrollbar.IsEnabled() && scrollbar.IsScrollbarRequired() && pointerHitbox.intersects(hitBox))
			{
				...
				scrollbar.Scroll(pf::input::GetPointer().z * 20);
			}
			else
			{
				state = IDLE;
			...
			if (pointerHitbox.intersects(hitBox) && pf::input::Press(pf::input::MOUSE_BUTTON_LEFT))
			{
				Activate();
			}
		}
	...
	void Label::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
	...
	void Label::SetColor(pf::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	pf::SpriteFont TextInputField::font_input;
	int caret_pos = 0;
	int caret_begin = 0;
	int caret_delay = 0;
	bool input_updated = false;
	pf::Timer caret_timer;
	void TextInputField::Create(const std::string& name)
	{
		...
		SetLocalizationEnabled(pf::gui::LocalizationEnabled::Tooltip); // disable localization of text because that can come from user input and musn't be overwritten!
	}
	...
	void TextInputField::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (state == ACTIVE)
			{
				if (pf::input::Press(pf::input::KEYBOARD_BUTTON_ENTER))
				{
					// accept input...
				...
				//else if (pf::input::Press(pf::input::KEYBOARD_BUTTON_BACKSPACE))
				//{
				//	// delete input...
				//	DeleteFromInput(-1);
				//}
				else if (pf::input::Press(pf::input::KEYBOARD_BUTTON_DELETE))
				{
					// delete input...
					DeleteFromInput(1);
				}
				else if (pf::input::Press(pf::input::KEYBOARD_BUTTON_LEFT) && caret_pos > 0)
				{
					// caret repositioning left:
					caret_pos--;
					if (!pf::input::Down(pf::input::BUTTON::KEYBOARD_BUTTON_LSHIFT) && !pf::input::Down(pf::input::BUTTON::KEYBOARD_BUTTON_RSHIFT))
					{
						caret_begin = caret_pos;
					}
					caret_timer.record();
				}
				else if (pf::input::Press(pf::input::KEYBOARD_BUTTON_RIGHT) && caret_pos < font_input.GetText().size())
				{
				...
				else if ((clicked && !intersectsPointer) || pf::input::Press(pf::input::KEYBOARD_BUTTON_ESCAPE))
				{
					// cancel input
					font_input.text.clear();
					Deactivate();
				}
				else if (pf::input::Down(pf::input::MOUSE_BUTTON_LEFT))
				{
					...
					{
						XMFLOAT2 size = pf::font::TextSize(str.c_str() + i, 1, font_input.params);
						pos += size.x;
				...
				if (pf::input::Down(pf::input::KEYBOARD_BUTTON_LCONTROL) && pf::input::Down((pf::input::BUTTON)'A'))
				{
					caret_begin = 0;
					caret_pos = (int)font_input.GetText().size();
				}
	...
	void TextInputField::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
		...
		if (state == ACTIVE)
		{
			...
			{
				pf::font::Params params = font_input.params;
				XMFLOAT2 size = pf::font::TextSize(font_input.GetText().c_str(), caret_pos, font_input.params);
				params.posX += size.x;
				params.color = pf::Color::lerp(params.color, pf::Color::Transparent(), 0.1f);
				params.size += 4;
				params.posY -= 2;
				params.h_align = pf::font::WIFALIGN_CENTER;
				pf::font::Draw(L"|", params, cmd);
			}
			...
			if (caret_pos != caret_begin)
			{
				...
				const std::wstring& str = font_input.GetText();
				float pos_start = font_input.params.position.x + pf::font::TextSize(str.c_str(), start, font_input.params).x;
				float pos_end = font_input.params.position.x + pf::font::TextSize(str.c_str(), end, font_input.params).x;
				...
				params.blendFlag = pf::enums::BLENDMODE_ALPHA;
				params.color = pf::Color::lerp(font_input.params.color, pf::Color::Transparent(), 0.5f);
				pf::image::Draw(nullptr, params, cmd);
			}
			...
			//GraphicsDevice* device = pf::graphics::GetDevice();
			//device->BindScissorRects(1, &scissorRect, cmd);
			//pf::font::Draw("caret_begin = " + std::to_string(caret_begin) + "\ncaret_pos = " + std::to_string(caret_pos), pf::font::Params(0, 20), cmd);
		}
		else
		{
			font.Draw(cmd);
	...
	void TextInputField::AddInput(const wchar_t inputChar)
	{
		...
		if (value_new.size() >= caret_pos)
		{
			...
			int num = 0;
			if (pf::input::Down(pf::input::KEYBOARD_BUTTON_LCONTROL) || pf::input::Down(pf::input::KEYBOARD_BUTTON_RCONTROL))
			{
				if (pf::input::Down((pf::input::BUTTON)'V'))
				{
					// Paste:
					std::wstring clipboard = pf::helper::GetClipboardText();
					value_new.insert(value_new.begin() + caret_pos, clipboard.begin(), clipboard.end());
					num = (int)clipboard.length();
				}
				else if (pf::input::Down((pf::input::BUTTON)'C'))
				{
					// Copy:
					...
					pf::helper::SetClipboardText(clipboard);
					return;
				}
				else if (pf::input::Down((pf::input::BUTTON)'X'))
				{
					// Cut:
					caret_pos = caret_pos_prev;
					std::wstring text = font_input.GetText();
	...
	void TextInputField::SetColor(pf::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void Slider::Create(float start, float end, float defaultValue, float step, const std::string& name)
	{
		...
		sprites[IDLE].params.color = pf::Color(60, 60, 60, 200);
		sprites[FOCUS].params.color = pf::Color(50, 50, 50, 200);
		sprites[ACTIVE].params.color = pf::Color(50, 50, 50, 200);
		sprites[DEACTIVATING].params.color = pf::Color(60, 60, 60, 200);

		font.params.h_align = pf::font::WIFALIGN_RIGHT;
		font.params.v_align = pf::font::WIFALIGN_CENTER;
	}
	...
	void Slider::SetRange(float start, float end)
	{
		this->start = start;
		this->end = end;
		this->value = pf::math::Clamp(this->value, start, end);
	}
	void Slider::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (state != ACTIVE)
		{
			...
			valueInputField.SetSize(XMFLOAT2(std::max(scale.y, pf::font::TextWidth(valueInputField.GetCurrentInputValue(), valueInputField.font.params) + 4), scale.y));
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (dragged)
			{
				EventArgs args;
				args.clickPos = pointerHitbox.pos;
				value = pf::math::InverseLerp(translation.x, translation.x + hitBox.siz.x, args.clickPos.x);
				value = pf::math::Clamp(value, 0, 1);
				value *= step;
				value = std::floor(value);
				value /= step;
				value = pf::math::Lerp(start, end, value);
				args.fValue = value;
		...
		sprites_knob[state].params.pos.x = pf::math::Lerp(translation.x + knobWidthHalf + 2, translation.x + hitBox.siz.x - knobWidthHalf - 2, pf::math::Clamp(pf::math::InverseLerp(start, end, value), 0, 1));
	...
	void Slider::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
	...
	void Slider::RenderTooltip(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const
	{
	...
	void Slider::SetColor(pf::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void CheckBox::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
	...
	void CheckBox::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
		...
		if (GetCheck())
		{
			...
			else if (!check_text_global.empty())
			{
				...
				params.scaling = 0.75f;
				params.color = font.params.color;
				pf::font::Draw(check_text_global, params, cmd);
			}
			else
			{
				// simple square:
		...
		else if (!uncheck_text.empty())
		{
			...
			params.scaling = 0.75f;
			params.color = font.params.color;
			pf::font::Draw(uncheck_text, params, cmd);
		}
	...
	{
		pf::helper::StringConvert(text, check_text_global);
	}
	void CheckBox::SetCheckText(const std::string& text)
	{
		pf::helper::StringConvert(text, check_text);
	}
	void CheckBox::SetUnCheckText(const std::string& text)
	{
		pf::helper::StringConvert(text, uncheck_text);
	}
	...
	void ComboBox::Create(const std::string& name)
	{
		...
		selected_font = font;
		selected_font.params.h_align = pf::font::WIFALIGN_CENTER;
		selected_font.params.v_align = pf::font::WIFALIGN_CENTER;

		filter.Create("");
		filter.SetCancelInputEnabled(false);
	}
	float ComboBox::GetDropOffset(const pf::Canvas& canvas) const
	{
		float screenheight = canvas.GetLogicalHeight();
		...
		if (!filterText.empty())
		{
			...
			{
				if (pf::helper::toUpper(items[i].name).find(filterText) != std::string::npos)
				{
					visible_items++;
				}
			}
	...
	float ComboBox::GetDropX(const pf::Canvas& canvas) const
	{
	...
	float ComboBox::GetItemOffset(const pf::Canvas& canvas, int index) const
	{
		if (!filterText.empty())
		{
	...
	void ComboBox::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (state == ACTIVE)
			{
					...
					if (combostate != COMBOSTATE_SELECTING && combostate != COMBOSTATE_INACTIVE)
					{
						if (combostate == COMBOSTATE_SCROLLBAR_GRABBED || pointerHitbox.intersects(Hitbox2D(XMFLOAT2(drop_x + drop_width + 1, translation.y + scale.y + drop_offset), XMFLOAT2(scale.y, (float)std::min(maxVisibleItemCount, filteredItemCount) * combo_height()))))
						{
							if (click_down)
							{
								...
								scrollbar_delta = pf::math::Clamp(pointerHitbox.pos.y, scrollbar_begin, scrollbar_end) - scrollbar_begin;
								const float scrollbar_value = pf::math::InverseLerp(scrollbar_begin, scrollbar_end, scrollbar_begin + scrollbar_delta);
				...
				else if (combostate == COMBOSTATE_SELECTING || pf::input::Press(pf::input::KEYBOARD_BUTTON_ESCAPE))
				{
					Deactivate();
					combostate = COMBOSTATE_INACTIVE;
				}
				else if (combostate == COMBOSTATE_HOVER && scroll_allowed)
				{
					...
					if (HasScrollbar())
					{
						int scroll = (int)pf::input::GetPointer().z;
						firstItemVisible -= scroll;
						firstItemVisible = std::max(0, std::min(filteredItemCount - maxVisibleItemCount, firstItemVisible));
						if (scroll)
						{
							const float scrollbar_value = pf::math::InverseLerp(0, float(std::max(0, filteredItemCount - maxVisibleItemCount)), float(firstItemVisible));
							scrollbar_delta = pf::math::Lerp(scrollbar_begin, scrollbar_end, scrollbar_value) - scrollbar_begin;
						}
					}
					...
					for (int i = firstItemVisible; (i < (int)items.size()) && (visible_items < maxVisibleItemCount); ++i)
					{
						if (!filterText.empty() && pf::helper::toUpper(items[i].name).find(filterText) == std::string::npos)
							continue;
						visible_items++;
						Hitbox2D itembox;
			...
			if (state == ACTIVE) // intentionally checks base state again!
			{
				...
				filter.SetDirty();
				filterText = pf::helper::toUpper(filter.GetText());
	...
	void ComboBox::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
		...
		pf::Color color = GetColor();
		if (combostate != COMBOSTATE_INACTIVE)
		{
			color = pf::Color::fromFloat4(sprites[FOCUS].params.color);
		}
		...
		if (drop_arrow)
		{
			...
			if (!vb_triangle.IsValid())
			{
				...
				pf::math::ConstructTriangleEquilateral(1, vertices[0].pos, vertices[1].pos, vertices[2].pos);
		...
		if (state == ACTIVE)
		{
			...
			if (HasScrollbar())
			{
				...
				{
					pf::Color col = pf::Color::fromFloat4(sprites[IDLE].params.color);
					if (combostate == COMBOSTATE_SCROLLBAR_HOVER)
					{
						col = pf::Color::fromFloat4(sprites[FOCUS].params.color);
					}
					else if (combostate == COMBOSTATE_SCROLLBAR_GRABBED)
					{
						col = pf::Color::fromFloat4(sprites[ACTIVE].params.color);
					}
					pf::image::Draw(
						nullptr,
						pf::image::Params(
	...
	{
		pf::helper::StringConvert(text, invalid_selection_text);
	}
	...
	void ComboBox::SetColor(pf::Color color, int id)
	{
		Widget::SetColor(color, id);
	...
	void ComboBox::SetTheme(const Theme& theme, int id)
	{
		...
		if (id == WIDGET_ID_COMBO_DROPDOWN)
		{
			drop_color = pf::Color::fromFloat4(theme.image.color);
		}
		theme.font.Apply(selected_font.params);
	}
	void ComboBox::ExportLocalization(pf::Localization& localization) const
	{
		Widget::ExportLocalization(localization);
		if (has_flag(localization_enabled, LocalizationEnabled::Items))
		{
			pf::Localization& section = localization.GetSection(GetName()).GetSection("items");
	...
	void ComboBox::ImportLocalization(const pf::Localization& localization)
	{
	...
	void Window::Create(const std::string& name, WindowControls window_controls)
	{
		...
		if (!has_flag(window_controls, WindowControls::DISABLE_TITLE_BAR))
		{
			...
			if (has_flag(window_controls, WindowControls::MOVE))
			{
				...
				moveDragger.font.params.h_align = pf::font::WIFALIGN_LEFT;
			...
			if (!has_flag(window_controls, WindowControls::MOVE) && !name.empty())
			{
				...
				label.SetText(name);
				label.font.params.h_align = pf::font::WIFALIGN_LEFT;
		...
		scrollbar_horizontal.SetColor(pf::Color(80, 80, 80, 100), pf::gui::IDLE);
		scrollbar_horizontal.sprites_knob[ScrollBar::SCROLLBAR_INACTIVE].params.color = pf::Color(140, 140, 140, 140);
		scrollbar_horizontal.sprites_knob[ScrollBar::SCROLLBAR_HOVER].params.color = pf::Color(180, 180, 180, 180);
		scrollbar_horizontal.sprites_knob[ScrollBar::SCROLLBAR_GRABBED].params.color = pf::Color::White();
		...
		scrollbar_vertical.SetColor(pf::Color(80, 80, 80, 100), pf::gui::IDLE);
		scrollbar_vertical.sprites_knob[ScrollBar::SCROLLBAR_INACTIVE].params.color = pf::Color(140, 140, 140, 140);
		scrollbar_vertical.sprites_knob[ScrollBar::SCROLLBAR_HOVER].params.color = pf::Color(180, 180, 180, 180);
		scrollbar_vertical.sprites_knob[ScrollBar::SCROLLBAR_GRABBED].params.color = pf::Color::White();
	...
	void Window::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled())
		{
			...
			if (resize_state == RESIZE_STATE_NONE && pf::input::Press(pf::input::MOUSE_BUTTON_LEFT))
			{
			...
			if (pf::input::Down(pf::input::MOUSE_BUTTON_LEFT))
			{
				if (resize_state != RESIZE_STATE_NONE)
				{
					...
					switch (resize_state)
					{
					case pf::gui::Window::RESIZE_STATE_LEFT:
						this->Translate(XMFLOAT3(deltaX, 0, 0));
						this->Scale(XMFLOAT3((scale.x - deltaX) / scale.x, 1, 1));
						break;
					case pf::gui::Window::RESIZE_STATE_TOP:
						this->Translate(XMFLOAT3(0, deltaY, 0));
						this->Scale(XMFLOAT3(1, (scale.y - deltaY) / scale.y, 1));
						break;
					case pf::gui::Window::RESIZE_STATE_RIGHT:
						this->Scale(XMFLOAT3((scale.x + deltaX) / scale.x, 1, 1));
						break;
					case pf::gui::Window::RESIZE_STATE_BOTTOM:
						this->Scale(XMFLOAT3(1, (scale.y + deltaY) / scale.y, 1));
						break;
					case pf::gui::Window::RESIZE_STATE_TOPLEFT:
						this->Translate(XMFLOAT3(deltaX, deltaY, 0));
						this->Scale(XMFLOAT3((scale.x - deltaX) / scale.x, (scale.y - deltaY) / scale.y, 1));
						break;
					case pf::gui::Window::RESIZE_STATE_TOPRIGHT:
						this->Translate(XMFLOAT3(0, deltaY, 0));
						this->Scale(XMFLOAT3((scale.x + deltaX) / scale.x, (scale.y - deltaY) / scale.y, 1));
						break;
					case pf::gui::Window::RESIZE_STATE_BOTTOMRIGHT:
						this->Scale(XMFLOAT3((scale.x + deltaX) / scale.x, (scale.y + deltaY) / scale.y, 1));
						break;
					case pf::gui::Window::RESIZE_STATE_BOTTOMLEFT:
						this->Translate(XMFLOAT3(deltaX, 0, 0));
						this->Scale(XMFLOAT3((scale.x - deltaX) / scale.x, (scale.y + deltaY) / scale.y, 1));
						break;
					default:
					...
					this->scale_local = pf::math::Max(this->scale_local, XMFLOAT3(control_size * 3, control_size * 2, 1)); // don't allow resize to negative or too small
					this->AttachTo(saved_parent);
					resize_begin = pointerHitbox.pos;
				}
			}
		...
		if (parent == nullptr)
		{
			translation_local.x = pf::math::Clamp(translation_local.x, 0, canvas.GetLogicalWidth() - scale_local.x);
			translation_local.y = pf::math::Clamp(translation_local.y, 0, canvas.GetLogicalHeight() - control_size);
			SetDirty();
		}
		...
		{
			float scroll = pf::input::GetPointer().z * 20;
	...
	void Window::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
		...
		// resize indicator:
		{
			...
			if (resize_state == RESIZE_STATE_TOPLEFT || pointerHitbox.intersects(toplefthitbox))
			{
				...
				fx.angular_softness_inner_angle = XM_PI * pf::math::Lerp(0.0f, 0.025f, std::abs(std::sin(resize_blink_timer * 4)));
				XMStoreFloat2(&fx.angular_softness_direction, XMVector2Normalize(XMVectorSet(-1, -1, 0, 0)));
				pf::image::Draw(nullptr, fx, cmd);
				pf::input::SetCursor(pf::input::CURSOR_RESIZE_NWSE);
			}
			else if (resize_state == RESIZE_STATE_TOPRIGHT || pointerHitbox.intersects(toprighthitbox))
			{
				...
				XMStoreFloat2(&fx.angular_softness_direction, XMVector2Normalize(XMVectorSet(1, -1, 0, 0)));
				pf::image::Draw(nullptr, fx, cmd);
				pf::input::SetCursor(pf::input::CURSOR_RESIZE_NESW);
			}
			...
			else if (resize_state == RESIZE_STATE_LEFT || pointerHitbox.intersects(lefthitbox))
			{
				...
				fx.angular_softness_inner_angle = XM_PI * pf::math::Lerp(0.0f, 0.24f, std::abs(std::sin(resize_blink_timer * 4)));
				fx.angular_softness_direction = XMFLOAT2(-1, 0);
				pf::image::Draw(nullptr, fx, cmd);
				pf::input::SetCursor(pf::input::CURSOR_RESIZE_EW);
			}
			...
			else if (resize_state == RESIZE_STATE_TOP || pointerHitbox.intersects(tophitbox))
			{
				...
				pf::image::Draw(nullptr, fx, cmd);
				pf::input::SetCursor(pf::input::CURSOR_RESIZE_NS);
			}
		...
		if (!IsCollapsed())
		{
			pf::image::Params params = sprites[IDLE].params;
			const pf::Resource& res = sprites[IDLE].textureResource;
			if (res.IsValid())
			{
				params.sampleFlag = pf::image::SAMPLEMODE_WRAP;
			...
			pf::image::Draw(sprites[IDLE].GetTexture(), params, cmd);

			if (background_overlay.IsValid())
			{
				params.blendFlag = pf::enums::BLENDMODE_ADDITIVE;
				params.color.w = 0;
		...
		//GraphicsDevice* device = pf::graphics::GetDevice();
		//device->BindScissorRects(1, &scissorRect, cmd);
		//pf::image::Draw(nullptr, pf::image::Params(scrollable_area.active_area.pos.x, scrollable_area.active_area.pos.y, scrollable_area.active_area.siz.x, scrollable_area.active_area.siz.y, pf::Color(255,0,255,100)), cmd);
		//Hitbox2D p = scrollable_area.GetPointerHitbox();
		//pf::image::Draw(nullptr, pf::image::Params(p.pos.x, p.pos.y, p.siz.x * 10, p.siz.y * 10, pf::Color(255,0,0,100)), cmd);
		//if (!IsCollapsed())
		//{
		//	pf::image::Draw(nullptr, pf::image::Params(scrollable_area.translation.x, scrollable_area.translation.y, scale.x, 10, pf::Color(255,0,255,100)), cmd);
		//}

		GetDevice()->EventEnd(cmd);
	}
	void Window::RenderTooltip(const pf::Canvas& canvas, pf::graphics::CommandList cmd) const
	{
	...
	void Window::SetColor(pf::Color color, int id)
	{
		Widget::SetColor(color, id);
		for (auto& widget : widgets)
		{
	...
	void Window::SetImage(pf::Resource resource, int id)
	{
		Widget::SetImage(resource, id);
		for (auto& widget : widgets)
		{
	...
	void Window::SetShadowColor(pf::Color color)
	{
		Widget::SetShadowColor(color);
		for (auto& widget : widgets)
		{
	...
	void Window::ExportLocalization(pf::Localization& localization) const
	{
		Widget::ExportLocalization(localization);
	...
	void Window::ImportLocalization(const pf::Localization& localization)
	{
		Widget::ImportLocalization(localization);
		if (has_flag(localization_enabled, LocalizationEnabled::Children))
		{
	...
	void ColorPicker::Create(const std::string& name, WindowControls window_controls)
	{
		...
		SetColor(pf::Color(100, 100, 100, 100));
		...
		text_H.OnInputAccepted([this](EventArgs args) {
			hue = pf::math::Clamp(args.fValue, 0, 360.0f);
			FireEvents();
			});
		AddWidget(&text_H);
		...
		text_S.OnInputAccepted([this](EventArgs args) {
			saturation = pf::math::Clamp(args.fValue / 100.0f, 0, 1);
			FireEvents();
			});
		AddWidget(&text_S);
		...
		text_V.OnInputAccepted([this](EventArgs args) {
			luminance = pf::math::Clamp(args.fValue / 100.0f, 0, 1);
			FireEvents();
			});
		AddWidget(&text_V);
		...
		text_hex.OnInputAccepted([this](EventArgs args) {
			pf::Color color(args.sValue.c_str());
			SetPickColor(color);
			FireEvents();
			});
		AddWidget(&text_hex);
	...
	void ColorPicker::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled() && dt > 0)
		{
			...
			XMFLOAT2 pointer = GetPointerHitbox().pos;
			float distance = pf::math::Distance(center, pointer);
			bool hover_hue = (distance > colorpicker_radius * sca) && (distance < (colorpicker_radius + colorpicker_width)* sca);

			float distTri = 0;
			XMFLOAT4 A, B, C;
			pf::math::ConstructTriangleEquilateral(colorpicker_radius_triangle, A, B, C);
			...
			{
				//hue pick
				const float angle = pf::math::GetAngle(XMFLOAT2(pointer.x - center.x, pointer.y - center.y), XMFLOAT2(colorpicker_radius, 0));
			...
			else if (colorpickerstate == CPS_SATURATION && dragged)
			{
				// saturation pick
				float u, v, w;
				pf::math::GetBarycentric(O, _A, _B, _C, u, v, w, true);
	...
	void ColorPicker::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		...
		static pf::graphics::GPUBuffer vb_hue;
		static pf::graphics::GPUBuffer vb_picker_saturation;
		static pf::graphics::GPUBuffer vb_picker_hue;
		static pf::graphics::GPUBuffer vb_preview;

		static pf::vector<Vertex> vertices_saturation;
		...
		if (!buffersComplete)
		{
			...
			// saturation
			{
				...
				pf::math::ConstructTriangleEquilateral(colorpicker_radius_triangle, vertices_saturation[0].pos, vertices_saturation[1].pos, vertices_saturation[2].pos);
				...
				vertices_saturation.push_back(vertices_saturation[0]); // outer
				vertices_saturation.push_back(vertices_saturation[0]); // inner
				pf::math::ConstructTriangleEquilateral(colorpicker_radius_triangle + 4, vertices_saturation[3].pos, vertices_saturation[5].pos, vertices_saturation[7].pos); // extrude outer
				vertices_saturation[9].pos = vertices_saturation[3].pos; // last outer
			}
			// hue
			{
				const float edge = 2.0f;
				pf::vector<Vertex> vertices;
		...
		const pf::Color final_color = GetPickColor();
		...
		if (IsEnabled())
		{
			...
			float u, v, w;
			pf::math::GetBarycentric(inner_point, color_corner, white_corner, black_corner, u, v, w, true);
	...
	{
		pf::gui::Window::ResizeLayout();
		const float padding = 4;
		const float width = GetWidgetAreaSize().x;
		float y = GetWidgetAreaSize().y - control_size;
		float jump = 20;

		auto add = [&](pf::gui::Widget& widget) {
			if (!widget.IsVisible())
				return;
		...
		auto add_right = [&](pf::gui::Widget& widget) {
			if (!widget.IsVisible())
				return;
	...
	void ColorPicker::SetColor(pf::Color color, int id)
	{
	...
	void ColorPicker::SetImage(pf::Resource resource, int id)
	{
	...
	pf::Color ColorPicker::GetPickColor() const
	{
		hsv source;
		source.h = hue;
		source.s = saturation;
		source.v = luminance;
		rgb result = hsv2rgb(source);
		return pf::Color::fromFloat4(XMFLOAT4(result.r, result.g, result.b, alphaSlider.GetValue() / 255.0f));
	}
	void ColorPicker::SetPickColor(pf::Color value)
	{
		if (colorpickerstate != CPS_IDLE)
		{
	...
	void TreeList::Create(const std::string& name)
	{
		...
		SetColor(pf::Color(100, 100, 100, 100), pf::gui::IDLE);
	...
	void TreeList::Update(const pf::Canvas& canvas, float dt)
	{
		if (!IsVisible())
		{
			return;
		...
		if (IsEnabled())
		{
			...
			if (pf::input::Down(pf::input::MOUSE_BUTTON_LEFT))
			{
				if (resize_state != RESIZE_STATE_NONE)
				{
					...
					this->scale_local = pf::math::Max(this->scale_local, XMFLOAT3(item_height() * 3, item_height() * 3, 1)); // don't allow resize to negative or too small
		...
		if (IsEnabled() && dt > 0)
		{
			...
			if (onDelete && state == FOCUS && pf::input::Press(pf::input::KEYBOARD_BUTTON_DELETE) && !typing_active)
			{
				int index = 0;
				for (auto& item : items)
				{
			...
			float scroll = pf::input::GetPointer().z * 10;
				...
				for (Item& item : items)
				{
					...
					else
					{
						...
						if (name_box.intersects(pointerHitbox))
						{
							...
							if (clicked)
							{
								if (pf::input::IsDoubleClicked() && onDoubleClick != nullptr)
								{
									EventArgs args;
									args.iValue = i;
								...
								else
								{
									...
									else if (pf::input::Down(pf::input::KEYBOARD_BUTTON_LSHIFT) || pf::input::Down(pf::input::KEYBOARD_BUTTON_RSHIFT))
									{
										if (first_selected >= 0)
										{
	...
	void TreeList::Render(const pf::Canvas& canvas, CommandList cmd) const
	{
		Widget::Render(canvas, cmd);
		if (!IsVisible())
		{
		...
		for (const Item& item : items)
		{
			...
			{
				pf::image::Draw(nullptr
					, pf::image::Params(name_box.pos.x, name_box.pos.y, name_box.siz.x, name_box.siz.y,
			...
			if (DoesItemHaveChildren(i))
			{
				...
				MiscCB cb;
				cb.g_xColor = opener_highlight == i ? pf::Color::White().toFloat4() : sprites[FOCUS].params.color;
	...
	void TreeList::SetColor(pf::Color color, int id)
	{
		Widget::SetColor(color, id);
		scrollbar.SetColor(color, id);
	}
