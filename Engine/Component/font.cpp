...
namespace pf::font {
	namespace font_internal {
		...
		{
			std::string name;
			pf::vector<uint8_t> fontBuffer; // only used if loaded from file, need to keep alive
			...
			{
				if (pf::helper::FileRead(newName, fontBuffer))
				{
		...
		static pf::vector<std::unique_ptr<FontStyle>> fontStyles;
		...
		static pf::unordered_map<int32_t, Glyph> glyph_lookup;
		static pf::unordered_map<int32_t, pf::rectpacker::Rect> rect_lookup;
		struct Bitmap
		{
			int width;
			int height;
		...
		static pf::unordered_map<int32_t, Bitmap> bitmap_lookup;
		...
		static pf::unordered_set<uint32_t> pendingGlyphs;
		...
		static thread_local pf::vector<FontVertex> vertexList;
		...
		ParseStatus ParseText(const char* text, size_t text_length, const Params& params)
		{
			...
			pf::helper::StringConvert(char_temp_buffer, wchar_temp_buffer);
	...
	void LoadShaders()
	{
		pf::renderer::LoadShader(ShaderStage::VS, vertexShader, "fontVS.cso");
		pf::renderer::LoadShader(ShaderStage::PS, pixelShader, "fontPS.cso");
		...
		for (int d = 0; d < DEPTH_TEST_MODE_COUNT; ++d)
		{
			...
			pf::graphics::GetDevice()->CreatePipelineState(&desc, &PSO[d]);
		}
	}
	...
	void UpdateAtlas(float upscaling) {
		...
		if (!pendingGlyphs.empty())
		{
			...
			static thread_local pf::rectpacker::State packer;
			packer.clear();
			...
			if (packer.pack(4096)) {
				...
				pf::vector<uint8_t> atlas(size_t(atlasWidth) * size_t(atlasHeight));
