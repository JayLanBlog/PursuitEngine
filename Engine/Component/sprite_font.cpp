...
namespace pf {
	...
	void SpriteFont::Update(float dt)
	{
		...
		if (anim.typewriter.time > 0)
		{
			...
			size_t text_length_prev = std::min(text_length, size_t(pf::math::Lerp(float(std::min(text_length, anim.typewriter.character_start)), float(text_length + 1), anim.typewriter.elapsed / anim.typewriter.time)));
			anim.typewriter.elapsed += dt;
			size_t text_length_next = std::min(text_length, size_t(pf::math::Lerp(float(std::min(text_length, anim.typewriter.character_start)), float(text_length + 1), anim.typewriter.elapsed / anim.typewriter.time)));

			if (anim.typewriter.soundinstance.IsValid())
			{
				if (!anim.typewriter.IsFinished() && pf::audio::IsEnded(&anim.typewriter.soundinstance) && text_length_prev != text_length_next && text[text_length_next - 1] != ' ' && anim.typewriter.soundinstance.IsValid())
				{
					pf::audio::Stop(&anim.typewriter.soundinstance);
					if (!IsHidden())
					{
						pf::audio::Play(&anim.typewriter.soundinstance);
					}
				}
				pf::audio::ExitLoop(&anim.typewriter.soundinstance);
			}
	...
	{
		return pf::font::TextSize(text, params);
	}
	float SpriteFont::TextWidth() const
	{
		return pf::font::TextWidth(text, params);
	}
	float SpriteFont::TextHeight() const
	{
		return pf::font::TextHeight(text, params);
	}

	void SpriteFont::SetText(const std::string& value)
	{
		pf::helper::StringConvert(value, text);
	}
	...
	{
		std::string retVal;
		pf::helper::StringConvert(text, retVal);
		return retVal;
	}
	...
	size_t SpriteFont::GetCurrentTextLength() const
	{
		...
		if (anim.typewriter.time > 0)
		{
			text_length = std::min(text_length, size_t(pf::math::Lerp(float(std::min(text_length, anim.typewriter.character_start)), float(text_length + 1), anim.typewriter.elapsed / anim.typewriter.time)));
		}
		return text_length;
	}

	pf::primitive::AABB SpriteFont::GetAABB() const
	{
		...
		if (params.h_align == pf::font::WIFALIGN_LEFT)
		{
			center.x += size.x * 0.5f;
		}
		else if (params.h_align == pf::font::WIFALIGN_RIGHT)
		{
			center.x -= size.x * 0.5f;
		}

		if (params.v_align == pf::font::WIFALIGN_TOP)
		{
			center.y += size.y * 0.5f;
		}
		else if (params.v_align == pf::font::WIFALIGN_BOTTOM)
		{
			center.y -= size.y * 0.5f;
		}
		pf::primitive::AABB ret;
		ret.createFromHalfWidth(center, XMFLOAT3(size.x, size.y, 0));
		return ret;
	}

	void SpriteFont::Serialize(pf::Archive& archive, pf::ecs::EntitySerializer& seri)
	{
			...
			if (!fontStyleName.empty())
			{
				pf::jobsystem::Execute(seri.ctx, [&](pf::jobsystem::JobArgs args) {
					if (!fontStyleName.empty())
					{
						...
						fontStyleResource = pf::resourcemanager::Load(fontStyleName);
						params.style = fontStyleResource.GetFontStyle();
					}
					});
			}
		...
		else
		{
			...
			archive << textA;
			archive << pf::helper::GetPathRelative(dir, fontStyleName);
