...
namespace pf::shadercompiler
{
	...
	void Compile_DXCompiler(const CompilerInput& input, CompilerOutput& output)
	{
		...
		vector<uint8_t> shadersourcedata;
		...
		if (input.format == ShaderFormat::HLSL6_XS)
		{
			pf::shadercompiler::xbox::AddArguments(input, args);
		}
			...
			HRESULT Open(D3D_INCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID* ppData, UINT* pBytes) override
			{
				for (auto& x : input->include_directories)
				{
					...
					if (!helper::FileExists(filename))
						continue;
					vector<uint8_t>& filedata = filedatas.emplace_back();
	...
	bool SaveShaderAndMetadata(const std::string& shaderfilename, const CompilerOutput& output)
	{
		...
		Archive dependencyLibrary(helper::ReplaceExtension(shaderfilename, shadermetaextension), false);
		if (dependencyLibrary.IsOpen())
		{
			std::string rootdir = dependencyLibrary.GetSourceDirectory();
			vector<std::string> dependencies = output.dependencies;
	...
	//	Logger("IsShaderOutdated : %s ", shaderfilename.c_str());
		...
		{
			//Logger("File : %s ,is Not Exists ", shaderfilename.c_str());
			return true; // no shader file = outdated shader, apps can attempt to rebuild it
		}

		std::string dependencylibrarypath = helper::ReplaceExtension(shaderfilename, shadermetaextension);
		...
		const uint64_t tim = helper::FileTimestamp(filepath);
		...
		if (dependencyLibrary.IsOpen())
		{
			...
			for (auto& x : dependencies)
			{
				...
				{
					const uint64_t dep_tim = helper::FileTimestamp(dependencypath);
	...
	std::mutex locker;
	unordered_set<std::string> registered_shaders;
