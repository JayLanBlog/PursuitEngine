...
namespace pf::graphics {
	namespace vulkan_internal
	{
		...
		//	These are also defined in pf::shadercompiler.cpp as hard coded compiler arguments for SPIRV, so they need to be the same
		enum
		{
		...
		constexpr VkStencilOp _ConvertStencilOp(StencilOp value)
		{
			switch (value)
			{
			case pf::graphics::StencilOp::KEEP:
				return VK_STENCIL_OP_KEEP;
			case pf::graphics::StencilOp::ZERO:
				return VK_STENCIL_OP_ZERO;
			case pf::graphics::StencilOp::REPLACE:
				return VK_STENCIL_OP_REPLACE;
			case pf::graphics::StencilOp::INCR_SAT:
				return VK_STENCIL_OP_INCREMENT_AND_CLAMP;
			case pf::graphics::StencilOp::DECR_SAT:
				return VK_STENCIL_OP_DECREMENT_AND_CLAMP;
			case pf::graphics::StencilOp::INVERT:
				return VK_STENCIL_OP_INVERT;
			case pf::graphics::StencilOp::INCR:
				return VK_STENCIL_OP_INCREMENT_AND_WRAP;
			case pf::graphics::StencilOp::DECR:
		...
		constexpr VkImageAspectFlags _ConvertImageAspect(ImageAspect value)
		{
			...
			{
			default:
			case pf::graphics::ImageAspect::COLOR:
				return VK_IMAGE_ASPECT_COLOR_BIT;
			case pf::graphics::ImageAspect::DEPTH:
				return VK_IMAGE_ASPECT_DEPTH_BIT;
			case pf::graphics::ImageAspect::STENCIL:
				return VK_IMAGE_ASPECT_STENCIL_BIT;
			case pf::graphics::ImageAspect::LUMINANCE:
				return VK_IMAGE_ASPECT_PLANE_0_BIT;
			case pf::graphics::ImageAspect::CHROMINANCE:
		...
		constexpr VkComponentSwizzle _ConvertComponentSwizzle(ComponentSwizzle value)
		{
			...
			case pf::graphics::ComponentSwizzle::R:
				return VK_COMPONENT_SWIZZLE_R;
			case pf::graphics::ComponentSwizzle::G:
				return VK_COMPONENT_SWIZZLE_G;
			case pf::graphics::ComponentSwizzle::B:
				return VK_COMPONENT_SWIZZLE_B;
			case pf::graphics::ComponentSwizzle::A:
				return VK_COMPONENT_SWIZZLE_A;
			case pf::graphics::ComponentSwizzle::ZERO:
				return VK_COMPONENT_SWIZZLE_ZERO;
			case pf::graphics::ComponentSwizzle::ONE:
		...
		bool checkExtensionSupport(const char* checkExtension, const pf::vector<VkExtensionProperties>& available_extensions)
		{
		...
		bool ValidateLayers(const pf::vector<const char*>& required,
			const pf::vector<VkLayerProperties>& available)
		{
			...
			void* user_data)
		{
			...
			if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
			{
				...
				ss += "\n";
				pf::helper::DebugOut(ss, pf::helper::DebugLevel::Warning);
			}
			else if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
			{
				...
				ss += callback_data->pMessage;
				ss += "\n";
				pf::helper::DebugOut(ss, pf::helper::DebugLevel::Error);
			}
		...
		{
			return pf::helper::GetCurrentPath() + "/pso_cache_vulkan";
		}
		...
		struct Buffer_Vulkan
		{
			...
			pf::vector<BufferSubresource> subresources_srv;
			pf::vector<BufferSubresource> subresources_uav;
		...
		struct Texture_Vulkan
		{
			...
			pf::vector<TextureSubresource> subresources_srv;
			pf::vector<TextureSubresource> subresources_uav;
			pf::vector<TextureSubresource> subresources_rtv;
			pf::vector<TextureSubresource> subresources_dsv;

			pf::vector<SubresourceData> mapped_subresources;
		...
		struct Shader_Vulkan
		{
			...
			pf::vector<VkDescriptorSetLayoutBinding> layoutBindings;
			pf::vector<VkImageViewType> imageViewTypes;

			pf::vector<BindingUsage> bindlessBindings;
			pf::vector<VkDescriptorSet> bindlessSets;
			uint32_t bindlessFirstSet = 0;

			VkPushConstantRange pushconstants = {};

			VkDeviceSize uniform_buffer_sizes[DESCRIPTORBINDER_CBV_COUNT] = {};
			pf::vector<uint32_t> uniform_buffer_dynamic_slots;
		...
		struct BVH_Vulkan
		{
			...
			pf::vector<VkAccelerationStructureGeometryKHR> geometries;
			pf::vector<uint32_t> primitiveCounts;
		...
		struct SwapChain_Vulkan
		{
			...
			pf::vector<VkImage> swapChainImages;
			pf::vector<VkImageView> swapChainImageViews;
			...
			pf::vector<VkSemaphore> swapchainAcquireSemaphores;
			pf::vector<VkSemaphore> swapchainReleaseSemaphores;
		...
		struct VideoDecoder_Vulkan
		{
			...
			pf::vector<VmaAllocation> allocations;
		...
		)
		{
			...
			pf::vector<VkSurfaceFormatKHR> swapchain_formats(formatCount);
			...
			pf::vector<VkPresentModeKHR> swapchain_presentModes(presentModeCount);
	...
	void GraphicsDevice_Vulkan::pso_validate(CommandList cmd)
	{
		...
		if (it == pipelines_global.end())
		{
			...
			if (pipeline == VK_NULL_HANDLE)
			{
				...
				pf::vector<VkVertexInputBindingDescription> bindings;
				pf::vector<VkVertexInputAttributeDescription> attributes;
	...
	GraphicsDevice_Vulkan::GraphicsDevice_Vulkan(platform::window_type window, ValidationMode validationMode_, GPUPreference preference)
	{
		...
		pf::unordered_map<uint32_t, std::shared_ptr<std::mutex>> queue_lockers;
		...
		if (res != VK_SUCCESS)
		{
			//pf::helper::messageBox("volkInitialize failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
			platform::Exit();
		}
		...
		appInfo.pApplicationName = "Pursuit Engine Application";
		appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
		appInfo.pEngineName = "Pursuit Engine";
		appInfo.engineVersion = VK_MAKE_VERSION(pf::version::GetMajor(), pf::version::GetMinor(), pf::version::GetRevision());
		...
		pf::vector<VkLayerProperties> availableInstanceLayers(instanceLayerCount);
		...
		pf::vector<VkExtensionProperties> availableInstanceExtensions(extensionCount);
		vulkan_check(vkEnumerateInstanceExtensionProperties(nullptr, &extensionCount, availableInstanceExtensions.data()));

		pf::vector<const char*> instanceLayers;
		pf::vector<const char*> instanceExtensions;
...
#elif defined(SDL2)
		{
			...
			pf::vector<const char*> extensionNames_sdl(extensionCount);
		...
		if (validationMode != ValidationMode::Disabled)
		{
			...
			static const pf::vector<const char*> validationLayerPriorityList[] =
			{
		...
		// Create instance:
		{
			...
			if (res != VK_SUCCESS)
			{
				//pf::helper::messageBox("vkCreateInstance failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
				platform::Exit();
			}
		...
		// Enumerating and creating devices:
		{
			...
			if (deviceCount == 0)
			{
				//log_messagebox("Failed to find GPU with Vulkan 1.3 support!");
				platform::Exit();
			}

			pf::vector<VkPhysicalDevice> devices(deviceCount);
			vulkan_check(vkEnumeratePhysicalDevices(instance, &deviceCount, devices.data()));

			const pf::vector<const char*> required_deviceExtensions = {
				VK_KHR_SWAPCHAIN_EXTENSION_NAME,
			};
			pf::vector<const char*> enabled_deviceExtensions;
			...
			auto checkPhysicalDeviceAndFillProperties2 = [&](VkPhysicalDevice dev) {
				...
				pf::vector<VkExtensionProperties> available_deviceExtensions(extensionCount);
			...
			for (const auto& dev : devices)
			{
				...
				{
					priority = pf::helper::toUpper(std::string(properties2.properties.deviceName)).find("AMD") != std::string::npos;
				}
				else if (preference == GPUPreference::Nvidia)
				{
					priority = pf::helper::toUpper(std::string(properties2.properties.deviceName)).find("NVIDIA") != std::string::npos;
				}
				else if (preference == GPUPreference::Intel)
				{
					priority = pf::helper::toUpper(std::string(properties2.properties.deviceName)).find("INTEL") != std::string::npos;
				}
			...
			{
				//log_messagebox("Failed to find a suitable GPU!");
				platform::Exit();
			}
			...
			pf::vector<VkDeviceQueueCreateInfo> queueCreateInfos;
			pf::unordered_set<uint32_t> uniqueQueueFamilies = { graphicsFamily,copyFamily,computeFamily,initFamily };
			if (videoFamily != VK_QUEUE_FAMILY_IGNORED)
			{
			...
			if (res != VK_SUCCESS)
			{
			//	pf::helper::messageBox("vkCreateDevice failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
				platform::Exit();
			}
		...
		if (res != VK_SUCCESS)
		{
			//pf::helper::messageBox("vmaCreateAllocator failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
			platform::Exit();
		}
		...
		if (res != VK_SUCCESS)
		{
			//pf::helper::messageBox("Failed to create Vulkan external memory allocator, ERROR: " + std::string(string_VkResult(res)), "Error!");
			platform::Exit();
		}
		...
		for (uint32_t fr = 0; fr < BUFFERCOUNT; ++fr)
		{
			for (int queue = 0; queue < QUEUE_COUNT; ++queue)
			{
				...
				if (res != VK_SUCCESS)
				{
				//	pf::helper::messageBox("vkCreateFence[FRAME] failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
					platform::Exit();
				}
				switch (queue)
				{
		...
		// Pipeline Cache
		{
			...
			pf::vector<uint8_t> pipelineData;
			if (!pf::helper::FileRead(get_shader_cache_path(), pipelineData))
			{
				pipelineData.clear();
			}
	...
	GraphicsDevice_Vulkan::~GraphicsDevice_Vulkan()
	{
		...
		if (pipelineCache != VK_NULL_HANDLE)
		{
			...
			pf::vector<uint8_t> data(size);
			vulkan_check(vkGetPipelineCacheData(device, pipelineCache, &size, data.data()));

			// Write pipeline cache data to a file in binary format
			pf::helper::FileWrite(get_shader_cache_path(), data.data(), size);
	...
	bool GraphicsDevice_Vulkan::CreateSwapChain(const SwapChainDesc* desc, platform::window_type window, SwapChain* swapchain) const
	{
		...
		if (internal_state->surface == VK_NULL_HANDLE)
		{
			...
			if (!SDL_Vulkan_CreateSurface(window, instance, nullptr, &internal_state->surface))
			{
				...
				throw std::runtime_error(std::string("Error creating a Vulkan surface: ") + SDL_GetError());
			}
#else
...
#error WICKEDENGINE VULKAN DEVICE ERROR: PLATFORM NOT SUPPORTED
	...
	bool GraphicsDevice_Vulkan::CreateTexture(const TextureDesc* desc, const SubresourceData* initial_data, Texture* texture, const GPUResource* alias, uint64_t alias_offset) const
	{
		...
		if (profile_list_info.profileCount > 0)
		{
			...
			pf::vector<VkVideoFormatPropertiesKHR> video_format_properties(format_property_count);
		...
		if (has_flag(texture->desc.misc_flags, ResourceMiscFlag::SPARSE))
		{
			...
			pf::vector<VkSparseImageMemoryRequirements> sparse_requirements(sparse_requirement_count);
		...
		if (initial_data != nullptr)
		{
			...
			pf::vector<VkBufferImageCopy> copyRegions;
	...
	bool GraphicsDevice_Vulkan::CreateShader(ShaderStage stage, const void* shadercode, size_t shadercode_size, Shader* shader) const
	{
...

		{
			...
			pf::vector<SpvReflectDescriptorBinding*> bindings(binding_count);
			result = spvReflectEnumerateDescriptorBindings(
			...
			pf::vector<SpvReflectBlockVariable*> pushconstants(push_count);
			...
			if (stage == ShaderStage::CS || stage == ShaderStage::LIB)
			{
				...
				{
					pf::vector<VkDescriptorSetLayout> layouts;
	...
	bool GraphicsDevice_Vulkan::CreateRaytracingPipelineState(const RaytracingPipelineStateDesc* desc, RaytracingPipelineState* rtpso) const
	{
		...
		pf::vector<VkPipelineShaderStageCreateInfo> stages;
		...
		pf::vector<VkRayTracingShaderGroupCreateInfoKHR> groups;
	...
	bool GraphicsDevice_Vulkan::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		{
			pf::vector<StdVideoH264PictureParameterSet> pps_array_h264(desc->pps_count);
			pf::vector<StdVideoH264ScalingLists> scalinglist_array_h264(desc->pps_count);
			...
			pf::vector<StdVideoH264SequenceParameterSet> sps_array_h264(desc->sps_count);
			pf::vector<StdVideoH264SequenceParameterSetVui> vui_array_h264(desc->sps_count);
			pf::vector<StdVideoH264HrdParameters> hrd_array_h264(desc->sps_count);
			...
			pf::vector<VkVideoSessionMemoryRequirementsKHR> requirements(requirement_count);
			...
			pf::vector<VkBindVideoSessionMemoryInfoKHR> bind_session_memory_infos(requirement_count);
	...
	CommandList GraphicsDevice_Vulkan::BeginCommandList(QUEUE_TYPE queue)
	{
		...
		if (commandlist.GetCommandBuffer() == VK_NULL_HANDLE)
		{
			...
			for (uint32_t buffer = 0; buffer < BUFFERCOUNT; ++buffer)
			{
				...
				switch (queue)
				{
				case pf::graphics::QUEUE_GRAPHICS:
					poolInfo.queueFamilyIndex = graphicsFamily;
					break;
				case pf::graphics::QUEUE_COMPUTE:
					poolInfo.queueFamilyIndex = computeFamily;
					break;
				case pf::graphics::QUEUE_COPY:
					poolInfo.queueFamilyIndex = copyFamily;
					break;
				case pf::graphics::QUEUE_VIDEO_DECODE:
	...
	{
		thread_local pf::vector<VkBindSparseInfo> sparse_infos;
		struct DataPerBind
		{
			...
			pf::vector<VkSparseMemoryBind> memory_binds;
			pf::vector<VkSparseImageMemoryBind> image_memory_binds;
		};
		thread_local pf::vector<DataPerBind> sparse_binds;
