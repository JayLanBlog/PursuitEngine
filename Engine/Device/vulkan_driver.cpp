...
namespace pf::graphics {
	...
	GraphicsDevice_Vulkan::GraphicsDevice_Vulkan(platform::window_type window, ValidationMode validationMode_, GPUPreference preference)
	{
		...
		//To DO : res = vulkan_check(volkInitialize());
		//if (res != VK_SUCCESS)
		//{
		////	helper::messageBox("volkInitialize failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
		//	platform::Exit();
		//}
	...
	//	TO DO : appInfo.engineVersion = VK_MAKE_VERSION(version::GetMajor(), wi::version::GetMinor(), wi::version::GetRevision());
...
#elif defined(SDL2)
		{
			...
			vector<const char*> extensionNames_sdl(extensionCount);
		...
		if (validationMode != ValidationMode::Disabled)
		{
			...
			static const vector<const char*> validationLayerPriorityList[] =
			{
		...
		// Create instance:
		{
			...
			if (res != VK_SUCCESS)
			{
				// TO DO :helper::messageBox("vkCreateInstance failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
				platform::Exit();
			}
		...
		// Enumerating and creating devices:
		{
			...
			if (deviceCount == 0)
			{
				//TO DO :log_messagebox("Failed to find GPU with Vulkan 1.3 support!");
			platform::Exit();
			}
			...
			const vector<const char*> required_deviceExtensions = {
				VK_KHR_SWAPCHAIN_EXTENSION_NAME,
			};
			vector<const char*> enabled_deviceExtensions;
			...
			auto checkPhysicalDeviceAndFillProperties2 = [&](VkPhysicalDevice dev) {
				...
				vector<VkExtensionProperties> available_deviceExtensions(extensionCount);
			...
			for (const auto& dev : devices)
			{
				...
				{
					priority = helper::toUpper(std::string(properties2.properties.deviceName)).find("AMD") != std::string::npos;
				}
				else if (preference == GPUPreference::Nvidia)
				{
					priority = helper::toUpper(std::string(properties2.properties.deviceName)).find("NVIDIA") != std::string::npos;
				}
				else if (preference == GPUPreference::Intel)
				{
					priority = helper::toUpper(std::string(properties2.properties.deviceName)).find("INTEL") != std::string::npos;
				}
			...
			unordered_set<uint32_t> uniqueQueueFamilies = { graphicsFamily,copyFamily,computeFamily,initFamily };
			if (videoFamily != VK_QUEUE_FAMILY_IGNORED)
			{
		...
		if (res != VK_SUCCESS)
		{
		//	TO DO : helper::messageBox("vmaCreateAllocator failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
			platform::Exit();
		}
		...
		if (res != VK_SUCCESS)
		{
			//TO DO :helper::messageBox("Failed to create Vulkan external memory allocator, ERROR: " + std::string(string_VkResult(res)), "Error!");
			platform::Exit();
		}
		...
		for (uint32_t fr = 0; fr < BUFFERCOUNT; ++fr)
		{
			for (int queue = 0; queue < QUEUE_COUNT; ++queue)
			{
				...
				if (res != VK_SUCCESS)
				{
					//helper::messageBox("vkCreateFence[FRAME] failed! ERROR: " + std::string(string_VkResult(res)), "Error!");
					platform::Exit();
				}
				switch (queue)
				{
	...
	GraphicsDevice_Vulkan::~GraphicsDevice_Vulkan()
	{
		...
		if (pipelineCache != VK_NULL_HANDLE)
		{
			...
			helper::FileWrite(get_shader_cache_path(), data.data(), size);
...
#error WICKEDENGINE VULKAN DEVICE ERROR: PLATFORM NOT SUPPORTED
	...
	bool GraphicsDevice_Vulkan::CreateTexture(const TextureDesc* desc, const SubresourceData* initial_data, Texture* texture, const GPUResource* alias, uint64_t alias_offset) const
	{
		...
		if (profile_list_info.profileCount > 0)
		{
			...
			vector<VkVideoFormatPropertiesKHR> video_format_properties(format_property_count);
		...
		if (has_flag(texture->desc.misc_flags, ResourceMiscFlag::SPARSE))
		{
			...
			vector<VkSparseImageMemoryRequirements> sparse_requirements(sparse_requirement_count);
	...
	bool GraphicsDevice_Vulkan::CreateShader(ShaderStage stage, const void* shadercode, size_t shadercode_size, Shader* shader) const
	{
...

		{
			...
			vector<SpvReflectDescriptorBinding*> bindings(binding_count);
			result = spvReflectEnumerateDescriptorBindings(
			...
			vector<SpvReflectBlockVariable*> pushconstants(push_count);
	...
	bool GraphicsDevice_Vulkan::CreateVideoDecoder(const VideoDesc* desc, VideoDecoder* video_decoder) const
	{
		...
		{
			vector<StdVideoH264PictureParameterSet> pps_array_h264(desc->pps_count);
			vector<StdVideoH264ScalingLists> scalinglist_array_h264(desc->pps_count);
			...
			vector<StdVideoH264SequenceParameterSet> sps_array_h264(desc->sps_count);
			vector<StdVideoH264SequenceParameterSetVui> vui_array_h264(desc->sps_count);
			vector<StdVideoH264HrdParameters> hrd_array_h264(desc->sps_count);
			...
			vector<VkVideoSessionMemoryRequirementsKHR> requirements(requirement_count);
			...
			vector<VkBindVideoSessionMemoryInfoKHR> bind_session_memory_infos(requirement_count);
	...
	CommandList GraphicsDevice_Vulkan::BeginCommandList(QUEUE_TYPE queue)
	{
		...
		if (commandlist.GetCommandBuffer() == VK_NULL_HANDLE)
		{
			...
			for (uint32_t buffer = 0; buffer < BUFFERCOUNT; ++buffer)
			{
				...
				switch (queue)
				{
				case graphics::QUEUE_GRAPHICS:
					poolInfo.queueFamilyIndex = graphicsFamily;
					break;
				case graphics::QUEUE_COMPUTE:
					poolInfo.queueFamilyIndex = computeFamily;
					break;
				case graphics::QUEUE_COPY:
					poolInfo.queueFamilyIndex = copyFamily;
					break;
				case graphics::QUEUE_VIDEO_DECODE:
