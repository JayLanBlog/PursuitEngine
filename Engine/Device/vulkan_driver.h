...
#define PFCKEDENGINE_BUILD_VULKAN
#endif // PLATFORM_WINDOWS_DESKTOP || PLATFORM_LINUX

#ifdef PFCKEDENGINE_BUILD_VULKAN
...
namespace pf::graphics {
	struct PSOLayoutHash {
		...
		inline void embed_hash()
		{
			...
			for (auto& x : items)
			{
				pf::helper::hash_combine(hash, x.used);
				pf::helper::hash_combine(hash, x.viewType);
				pf::helper::hash_combine(hash, x.binding.binding);
				pf::helper::hash_combine(hash, x.binding.descriptorCount);
				pf::helper::hash_combine(hash, x.binding.descriptorType);
				pf::helper::hash_combine(hash, x.binding.stageFlags);
			}
			pf::helper::hash_combine(hash, push.offset);
			pf::helper::hash_combine(hash, push.size);
			pf::helper::hash_combine(hash, push.stageFlags);
		}
...
{
	template <>
	struct hash<pf::graphics::PSOLayoutHash>
	{
		constexpr size_t operator()(const pf::graphics::PSOLayoutHash& hash) const
		{
...
namespace pf::graphics
{
	...
	protected:
		...
		vector<VkQueueFamilyProperties2> queueFamilies;
		vector<VkQueueFamilyVideoPropertiesKHR> queueFamiliesVideo;
		...
		vector<VkDynamicState> pso_dynamicStates;
		...
		struct CommandQueue
		{
			...
			vector<SwapChain> swapchain_updates;
			vector<VkSemaphoreSubmitInfo> submit_waitSemaphoreInfos;
			vector<VkSemaphoreSubmitInfo> submit_signalSemaphoreInfos;
			vector<VkCommandBufferSubmitInfo> submit_cmds;

			vector<VkSemaphore> swapchainWaitSemaphores;
			vector<VkSwapchainKHR> swapchains;
			vector<uint32_t> swapchainImageIndices;
		...
		mutable vector<VkImageMemoryBarrier2> init_transitions;
		...
		struct DescriptorBinder {
			...
			vector<VkWriteDescriptorSetAccelerationStructureKHR> accelerationStructureViews;
		...
		vector<VkSemaphore> semaphore_pool;
		...
		struct CommandList_Vulkan {
			...
			pf::vector<std::pair<PipelineHash, VkPipeline>> pipelines_worker;
			...
			pf::vector<SwapChain> prev_swapchains;
			bool dirty_pso = {};
			pf::vector<VkMemoryBarrier2> frame_memoryBarriers;
			pf::vector<VkImageMemoryBarrier2> frame_imageBarriers;
			pf::vector<VkBufferMemoryBarrier2> frame_bufferBarriers;
			pf::vector<VkAccelerationStructureGeometryKHR> accelerationstructure_build_geometries;
			pf::vector<VkAccelerationStructureBuildRangeInfoKHR> accelerationstructure_build_ranges;
			RenderPassInfo renderpass_info;
			pf::vector<VkImageMemoryBarrier2> renderpass_barriers_begin;
			pf::vector<VkImageMemoryBarrier2> renderpass_barriers_end;
		...
		pf::vector<std::unique_ptr<CommandList_Vulkan>> commandlists;
		uint32_t cmd_count = 0;
		pf::SpinLock cmd_locker;
		...
		pf::unordered_map<PipelineHash, VkPipeline> pipelines_global;
		...
		pf::vector<VkSampler> immutable_samplers;

		void set_fence_name(VkFence fence, const char* name);
		void set_semaphore_name(VkSemaphore semaphore, const char* name);
public:

		GraphicsDevice_Vulkan(platform::window_type window, ValidationMode validationMode = ValidationMode::Disabled, GPUPreference preference = GPUPreference::Discrete);
		~GraphicsDevice_Vulkan() override;

		bool CreateSwapChain(const SwapChainDesc* desc, platform::window_type window, SwapChain* swapchain) const override;
		...
		mutable unordered_map<PSOLayoutHash, std::shared_ptr<PSOLayout>> pso_layout_cache;
