...
namespace pf
{
	...
	protected:
		...
		pf::eventhandler::Handle swapChainVsyncChangeEvent;
	...
	public:
		...
		pf::Canvas canvas;
		platform::window_type window;

		pf::graphics::Texture splash_screen;
		...
		void ActivatePath(RenderPath* component, float fadeSeconds = 0, pf::Color fadeColor = pf::Color(0, 0, 0, 255), FadeManager::FadeType fadetype = FadeManager::FadeType::FadeToColor);
		...
		struct InfoDisplayer
		{
			...
			pf::graphics::Rect rect;
		};

		// display all-time engine information text
		InfoDisplayer infoDisplay;

		void Fade(float fadeSeconds = 1, pf::Color fadeColor = pf::Color(0, 0, 0, 255), FadeManager::FadeType fadetype = FadeManager::FadeType::FadeToColor)
		{
			fadeManager.Start(fadeSeconds, fadeColor, [] {}, fadetype);
		}
		void CrossFade(float fadeSeconds = 1)
		{
			Fade(fadeSeconds, pf::Color::Black(), pf::FadeManager::FadeType::CrossFade);
		}
