...
namespace pf::terrain
{
	...
	struct Generator
	{
		pf::scene::Scene scene; // The background generation thread can safely add things to this, it will be merged into the main scene when it is safe to do so
		pf::jobsystem::context workload;
		std::atomic_bool cancelled{ false };
		pf::vector<SplineComponent> splines;
	};

	pf::jobsystem::context virtual_texture_ctx;
	...
	void VirtualTextureAtlas::Residency::init(uint32_t resolution)
	{
		...
		if (resolution > SVT_TILE_SIZE)
		{
			...
			pf::vector<uint32_t> default_data(td.width * td.height);
			pf::vector<SubresourceData> initdata(lod_count);
...

			{
				...
				pf::vector<uint32_t> data(tile_count);
			...
			}
			{
				...
				pf::vector<uint32_t> data(tile_count + 1);
	...
	{
		Generation_Cancel();
		pf::jobsystem::Wait(virtual_texture_ctx);
	}
	...
	{
		if (pf::jobsystem::IsBusy(generator->workload))
		...
		for (pf::ecs::Entity entity : materialEntities)
		{
		...
		for (auto it = chunks.begin(); it != chunks.end();)
		{
			...
			if (pf::renderer::GetOcclusionCullingEnabled())
			{
			...
			{
				pf::HairParticleSystem* grass = scene->hairs.GetComponent(chunk_data.grass_entity);
				if (grass != nullptr)
				{
		...
		pf::jobsystem::Execute(generator->workload, [=](pf::jobsystem::JobArgs a) {
			...
			auto request_chunk = [&](int offset_x, int offset_z)
			{
				...
				if (it == chunks.end() || it->second.entity == INVALID_ENTITY)
				{
					...
					object.filterMask |= pf::enums::FILTER_NAVIGATION_MESH;
					object.filterMask |= pf::enums::FILTER_TERRAIN;
					...
					pf::HairParticleSystem grass = grass_properties;
					...
					pf::jobsystem::Dispatch(ctx, vertexCount_padded, chunk_width_padded * 4, [&](pf::jobsystem::JobArgs args) {
						...
						for (size_t j = 0; j < generator->splines.size(); ++j)
						{
							...
							const float splinedist = pf::math::Distance(XMVectorSetY(P, splineheight), S);
					...
					pf::jobsystem::Dispatch(ctx, vertexCount, chunk_width * 4, [&](pf::jobsystem::JobArgs args) {
						const uint32_t index = args.jobIndex;
						...
						float region_low_altitude = region2 == 0 ? 1 : smoothstep(0.0f, region2, pf::math::InverseLerp(0, bottomLevel, height));
						float region_high_altitude = region3 == 0 ? 1 : smoothstep(0.0f, region3, pf::math::InverseLerp(0, topLevel, height));
					...
					pf::jobsystem::Wait(ctx); // wait until chunk's vertex buffer is fully generated
					...
					if (IsPhysicsEnabled())
					{
						...
						newrigidbody.friction = 0.8f;
						//newrigidbody.mesh_lod = 2;
						pf::physics::CreateRigidBodyShape(newrigidbody, transform.scale_local, &mesh);
					}

					pf::jobsystem::Wait(ctx); // wait until mesh.CreateRenderData() async task finishes
				...
				if (dist <= grass_chunk_dist && IsGrassEnabled())
				{
					...
					if (it != chunks.end() && it->second.entity != INVALID_ENTITY)
					{
						...
						if (chunk_data.grass_entity == INVALID_ENTITY && chunk_data.grass.meshID != INVALID_ENTITY)
						{
							...
							pf::HairParticleSystem& grass = generator->scene.hairs.Create(chunk_data.grass_entity);
				...
				if (dist <= prop_generation)
				{
					...
					if (it != chunks.end() && it->second.entity != INVALID_ENTITY)
					{
						...
						if (prop_density > 0 && chunk_data.props_entity == INVALID_ENTITY && chunk_data.mesh_vertex_positions != nullptr)
						{
							...
							pf::random::RNG rng(chunk.compute_hash());
							...
							for (const auto& prop : props)
							{
								...
								for (int i = 0; i < gen_count; ++i)
								{
									...
									XMFLOAT4 region0 = pf::Color(chunk_data.blendmap_layers[0].pixels[ind0], chunk_data.blendmap_layers[1].pixels[ind0], chunk_data.blendmap_layers[2].pixels[ind0], chunk_data.blendmap_layers[3].pixels[ind0]);
									XMFLOAT4 region1 = pf::Color(chunk_data.blendmap_layers[0].pixels[ind1], chunk_data.blendmap_layers[1].pixels[ind1], chunk_data.blendmap_layers[2].pixels[ind1], chunk_data.blendmap_layers[3].pixels[ind1]);
									XMFLOAT4 region2 = pf::Color(chunk_data.blendmap_layers[0].pixels[ind2], chunk_data.blendmap_layers[1].pixels[ind2], chunk_data.blendmap_layers[2].pixels[ind2], chunk_data.blendmap_layers[3].pixels[ind2]);
									weight_norm(region0);
									weight_norm(region1);
									weight_norm(region2);
									...
									{
										pf::Archive archive = pf::Archive(prop.data.data(), prop.data.size());
										EntitySerializer seri;
										Entity entity = generator->scene.Entity_Serialize(
											archive,
											seri,
											INVALID_ENTITY,
											pf::scene::Scene::EntitySerializeFlags::RECURSIVE |
											pf::scene::Scene::EntitySerializeFlags::KEEP_INTERNAL_ENTITY_REFERENCES
										);
										...
										transform->translation_local.y += pf::math::Lerp(prop.min_y_offset, prop.max_y_offset, rng.next_float());
										const float scaling = pf::math::Lerp(prop.min_size, prop.max_size, rng.next_float());
	...
	void Terrain::Generation_Cancel()
	{
		...
		pf::jobsystem::Wait(generator->workload); // waits until generation thread exits
	...
	void Terrain::CreateChunkRegionTexture(ChunkData& chunk_data)
	{
		...
		if (!chunk_data.heightmap.IsValid() && !chunk_data.heightmap_data.empty())
		{
			...
			assert(success);
			device->SetName(&chunk_data.heightmap, "pf::terrain::ChunkData::heightmap");
		}
		...
		if (!chunk_data.blendmap.IsValid() || chunk_data.blendmap.desc.array_size != required_layers)
		{
			...
			pf::vector<SubresourceData> data(required_layers);
			...
			assert(success);
			device->SetName(&chunk_data.blendmap, "pf::terrain::ChunkData::blendmap");
		}
	}
	...
	{
		pf::jobsystem::Wait(virtual_texture_ctx);
		virtual_texture_ctx.priority = pf::jobsystem::Priority::Low;
		...
		pf::jobsystem::Execute(virtual_texture_ctx, [this](pf::jobsystem::JobArgs args) {
	...
	void Terrain::UpdateVirtualTexturesGPU(CommandList cmd) const
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Terrain - UpdateVirtualTexturesGPU", cmd);

		device->EventBegin("Update Residency Maps", cmd);
		device->BindComputeShader(pf::renderer::GetShader(pf::enums::CSTYPE_VIRTUALTEXTURE_RESIDENCYUPDATE), cmd);
		...
		for (const VirtualTexture* vt : virtual_textures_in_use)
		{
			...
			for (uint32_t map_type = 0; map_type < arraysize(atlas.maps); map_type++)
			{
				...
				{
				case MaterialComponent::BASECOLORMAP:
					device->BindComputeShader(pf::renderer::GetShader(pf::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_BASECOLORMAP), cmd);
					...
					break;
				case MaterialComponent::NORMALMAP:
					device->BindComputeShader(pf::renderer::GetShader(pf::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_NORMALMAP), cmd);
					...
					break;
				case MaterialComponent::SURFACEMAP:
					device->BindComputeShader(pf::renderer::GetShader(pf::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_SURFACEMAP), cmd);
					...
					break;
				case MaterialComponent::EMISSIVEMAP:
					device->BindComputeShader(pf::renderer::GetShader(pf::enums::CSTYPE_TERRAIN_VIRTUALTEXTURE_UPDATE_EMISSIVEMAP), cmd);
	...
	void Terrain::AllocateVirtualTextureTileRequestsGPU(CommandList cmd) const
	{
...

		{
			...
			device->BindComputeShader(pf::renderer::GetShader(pf::enums::CSTYPE_VIRTUALTEXTURE_TILEREQUESTS), cmd);
			for (const VirtualTexture* vt : virtual_textures_in_use)
			{
				if (vt->residency == nullptr)
					continue;
...

		{
			...
			device->BindComputeShader(pf::renderer::GetShader(pf::enums::CSTYPE_VIRTUALTEXTURE_TILEALLOCATE), cmd);
			for (const VirtualTexture* vt : virtual_textures_in_use)
			{
				if (vt->residency == nullptr)
					continue;
	...
	void Terrain::WritebackTileRequestsGPU(CommandList cmd) const
	{
		...
		for (const VirtualTexture* vt : virtual_textures_in_use)
		{
			...
			pf::renderer::PushBarrier(GPUBarrier::Buffer(&vt->residency->allocationBuffer, ResourceState::COPY_SRC, ResourceState::COPY_DST));
		}
		...
		for (const VirtualTexture* vt : virtual_textures_in_use)
		{
			...
			static pf::unordered_map<uint32_t, VirtualTextureAtlas::Residency> clear_residencies;
			...
			pf::renderer::PushBarrier(GPUBarrier::Image(&vt->residency->residencyMap, ResourceState::COPY_DST, ResourceState::COPY_SRC));
		}
		pf::renderer::FlushBarriers(cmd);
		device->EventEnd(cmd);
	}
	...
	void Terrain::Serialize(pf::Archive& archive, pf::ecs::EntitySerializer& seri)
	{
		Generation_Cancel();
		...
		const uint64_t grass_version = seri.GetVersion("pf::scene::Scene::hairs");
		const uint64_t material_version = seri.GetVersion("pf::scene::Scene::materials");
		const uint64_t weather_version = seri.GetVersion("pf::scene::Scene::weathers");
			...
			for (size_t i = 0; i < props.size(); ++i)
			{
				...
				if (terrain_version >= 1)
				{
					...
					if (!prop.data.empty())
					{
						...
						Scene tmp_scene;
						pf::Archive tmp_archive = pf::Archive(prop.data.data(), prop.data.size());
				...
				else
				{
					...
					tmp_scene.Entity_Serialize(
						...
						seri,
						object_entity,
						pf::scene::Scene::EntitySerializeFlags::RECURSIVE | pf::scene::Scene::EntitySerializeFlags::KEEP_INTERNAL_ENTITY_REFERENCES
					);
		...
		else
		{
			...
			pf::scene::MaterialComponent materials[MATERIAL_COUNT];
			...
			pf::jobsystem::Wait(seri.ctx); // wait for material CreateRenderData() that was asynchronously launched in Serialize()!
			...
			ComponentManager<MaterialComponent>* scene_materials = library.Get<MaterialComponent>("pf::scene::Scene::materials");
			...
			ComponentManager<NameComponent>* scene_names = library.Get<NameComponent>("pf::scene::Scene::names");
		...
		if (terrain_version < 4)
		{
			...
			ComponentManager<pf::HairParticleSystem>* scene_hairs = library.Get<pf::HairParticleSystem>("pf::scene::Scene::hairs");
