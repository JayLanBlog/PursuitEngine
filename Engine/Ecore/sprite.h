...
namespace pf {
	...
	public:
		...
		virtual void Draw(pf::graphics::CommandList cmd) const;
		...
		pf::Resource textureResource;
		pf::Resource maskResource;
		pf::Resource backgroundResource;

		const pf::graphics::Texture* GetTexture() const;
		...
		struct Anim
		{
			...
			struct WobbleAnim
			{
				...
				WobbleAnim()
				{
					for (int i = 0; i < 4; ++i)
					{
						corner_angles[i] = pf::random::GetRandom(0, 1000) / 1000.0f * XM_2PI;
						corner_speeds[i] = pf::random::GetRandom(500, 1000) / 1000.0f;
						if (pf::random::GetRandom(0, 1) == 0)
						{
							corner_speeds[i] *= -1;
						}
						corner_angles2[i] = pf::random::GetRandom(0, 1000) / 1000.0f * XM_2PI;
						corner_speeds2[i] = pf::random::GetRandom(500, 1000) / 1000.0f;
						if (pf::random::GetRandom(0, 1) == 0)
						{
							corner_speeds2[i] *= -1;
						}
