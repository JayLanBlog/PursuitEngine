...
#include "Shader/ShaderInterop_Ocean.h"
#include "scene.h"
...
namespace pf {
	...
	namespace ocean_internal
	{
		...
		void LoadShaders()
		{
			pf::renderer::LoadShader(ShaderStage::CS, updateSpectrumCS, "oceanSimulatorCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, updateDisplacementMapCS, "oceanUpdateDisplacementMapCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, updateGradientFoldingCS, "oceanUpdateGradientFoldingCS.cso");

			pf::renderer::LoadShader(ShaderStage::VS, oceanSurfVS, "oceanSurfaceVS.cso");

			pf::renderer::LoadShader(ShaderStage::PS, oceanSurfPS, "oceanSurfacePS.cso");
			pf::renderer::LoadShader(ShaderStage::PS, wireframePS, "oceanSurfaceSimplePS.cso");
...
#define GRAV_ACCEL	981.0f	// The acceleration of gravity, cm/s^2
	...
	void Ocean::Create(const OceanParameters& params)
	{
		...
		pf::vector<XMFLOAT2> h0_data(height_map_size);
		pf::vector<float> omega_data(height_map_size);
		...
		pf::vector<XMFLOAT4> displacementdata(tex_desc.width * tex_desc.height); // zero init the heightmap to be valid before first simulation
	...
	void Ocean::UpdateDisplacementMap(CommandList cmd) const
	{
		...
		// ------------------------------------ Perform FFT -------------------------------------------
		pf::fftgenerator::fft_512x512_c2c(buffer_Float_Dxyz, buffer_Float_Dxyz, buffer_Float2_Ht, cmd);
		...
		pf::renderer::GenerateMipChain(gradientMap, pf::renderer::MIPGENFILTER_LINEAR, cmd);

		device->EventEnd(cmd);
	}

	void Ocean::CopyDisplacementMapReadback(pf::graphics::CommandList cmd) const
	{
		GraphicsDevice* device = pf::graphics::GetDevice();
	...
	void Ocean::RenderForOcclusionTest(const CameraComponent& camera, CommandList cmd) const
	{
		...
		{
			pf::vector<uint16_t> index_data(index_count);
			size_t counter = 0;
	...
	void Ocean::Render(const CameraComponent& camera, CommandList cmd) const
	{
		...
		bool wire = pf::renderer::IsWireRender();
		...
		{
			pf::vector<uint32_t> index_data(index_count);
			size_t counter = 0;
	...
	void Ocean::Initialize()
	{
		...
		static pf::eventhandler::Handle handle = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); pf::fftgenerator::LoadShaders(); });

		LoadShaders();

		pf::fftgenerator::LoadShaders();

		p_log("pf::Ocean Initialized (%d ms)", (int)std::round(timer.elapsed()));
	}
