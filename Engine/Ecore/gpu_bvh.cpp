...
namespace pf
{
	...
	void GPUBVH::Update(const pf::scene::Scene& scene)
	{
	...
	void GPUBVH::Build(const Scene& scene, CommandList cmd) const
	{
		...
		auto range = pf::profiler::BeginRangeGPU("BVH Rebuild", cmd);
		...
		pf::gpusortlib::Sort(primitiveCount, primitiveMortonBuffer, primitiveCounterBuffer, 0, primitiveIDBuffer, cmd);
		device->EventEnd(cmd);
	...
	namespace GPUBVH_Internal
	{
		void LoadShaders()
		{
			pf::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_PRIMITIVES], "bvh_primitivesCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_HIERARCHY], "bvh_hierarchyCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, computeShaders[CSTYPE_BVH_PROPAGATEAABB], "bvh_propagateaabbCS.cso");
		}
	}
	...
	void GPUBVH::Initialize()
	{
		...
		static pf::eventhandler::Handle handle = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { GPUBVH_Internal::LoadShaders(); });
		GPUBVH_Internal::LoadShaders();

		p_log("pf::GPUBVH Initialized (%d ms)", (int)std::round(timer.elapsed()));
	}
}
