...
namespace pf
{
	...
	void RenderPath2D::Update(float dt)
	{
		...
		for (auto& x : layers)
		{
			for (auto& y : x.items)
			{
				...
				{
					pf::video::UpdateVideo(y.videoinstance, dt);
					if (pf::video::IsDecodingRequired(y.videoinstance))
					{
	...
	void RenderPath2D::Render() const
	{
		...
		if (!video_decodes.empty())
		{
			...
			{
				pf::video::DecodeVideo(x, video_cmd);
			}
		...
		pf::image::SetCanvas(*this);
		pf::font::SetCanvas(*this);
		...
		if (!video_decodes.empty())
		{
			...
			for (auto& x : video_decodes)
			{
				pf::video::ResolveVideoToRGB(x, cmd);
			}
			device->EventEnd(cmd);
		}

		pf::renderer::ProcessDeferredTextureRequests(cmd);

		if (GetGUIBlurredBackground() != nullptr)
		{
			pf::image::SetBackground(*GetGUIBlurredBackground());
		}
		...
		{
			pf::renderer::ExtractStencil(*dsv, rtStencilExtracted, cmd);
		}
		...
		for (auto& x : layers)
		{
			for (auto& y : x.items)
			{
				if (y.sprite != nullptr)
				{
					if (!stencil_scaled && y.sprite->params.stencilComp != pf::image::STENCILMODE_DISABLED && stencilScaled.IsValid())
					{
						// Only need a scaled stencil mask if there are any stenciled sprites, and only once is enough before the first one
						stencil_scaled = true;
						pf::renderer::ScaleStencilMask(vp, rtStencilExtracted, cmd);
					}
					if (y.videoinstance != nullptr)
					{
	...
	void RenderPath2D::Compose(CommandList cmd) const
	{
		...
		pf::image::Params fx;
		fx.enableFullScreen();
		fx.blendFlag = pf::enums::BLENDMODE_PREMULTIPLIED;
		...
		pf::image::Draw(&GetRenderResult(), fx, cmd);
	...
	void RenderPath2D::AddSprite(pf::Sprite* sprite, const std::string& layer)
	{
		for (auto& x : layers)
		{
			if (x.name == layer)
	...
	void RenderPath2D::RemoveSprite(pf::Sprite* sprite)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	int RenderPath2D::GetSpriteOrder(pf::Sprite* sprite)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::AddFont(pf::SpriteFont* font, const std::string& layer)
	{
		for (auto& x : layers)
		{
			if (x.name == layer)
	...
	void RenderPath2D::RemoveFont(pf::SpriteFont* font)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	int RenderPath2D::GetFontOrder(pf::SpriteFont* font)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::AddVideoSprite(pf::video::VideoInstance* videoinstance, pf::Sprite* sprite, const std::string& layer)
	{
		for (auto& x : layers)
		{
			if (x.name == layer)
	...
	void RenderPath2D::SetSpriteOrder(pf::Sprite* sprite, int order)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::SetFontOrder(pf::SpriteFont* font, int order)
	{
		for (auto& x : layers)
		{
			for (auto& y : x.items)
	...
	void RenderPath2D::CleanLayers()
	{
		for (auto& x : layers)
		{
			...
			pf::vector<RenderItem2D> itemsToRetain(0);
			for (auto& y : x.items)
			{
