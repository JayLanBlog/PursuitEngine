...
namespace pf {
	...
	Sprite::Sprite(const std::string& newTexture, const std::string& newMask)
	{
		if (!newTexture.empty())
		{
			...
			textureResource = pf::resourcemanager::Load(newTexture);
		}
		if (!newMask.empty())
		{
			maskName = newMask;
			maskResource = pf::resourcemanager::Load(newMask);
	...
	void Sprite::Draw(CommandList cmd) const
	{
		...
		if ((maskResource.IsValid() && params.maskMap == nullptr) || (backgroundResource.IsValid() && params.backgroundMap == nullptr))
		{
			...
			pf::image::Draw(GetTexture(), temp, cmd);
		}
		else
		{
			pf::image::Draw(GetTexture(), params, cmd);
		}
	}
	...
	void Sprite::Update(float dt)
	{
		...
		if (anim.wobbleAnim.amount.x > 0 || anim.wobbleAnim.amount.y > 0)
		{
			...
			pf::image::Params default_params; // contains corners in idle positions
			for (int i = 0; i < 4; ++i)
			{
	...
	const pf::graphics::Texture* Sprite::GetTexture() const
	{
		return textureResource.IsValid() ? &textureResource.GetTexture() : nullptr;
	}

	void Sprite::Serialize(pf::Archive& archive, pf::ecs::EntitySerializer& seri)
	{
			...
			if (!textureName.empty() || !maskName.empty())
			{
				pf::jobsystem::Execute(seri.ctx, [&](pf::jobsystem::JobArgs args) {
					if (!textureName.empty())
					{
						...
						textureResource = pf::resourcemanager::Load(textureName);
					}
					if (!maskName.empty())
					{
						maskName = dir + maskName;
						maskResource = pf::resourcemanager::Load(maskName);
					}
					});
			}
		}
		else
		{
			...
			archive << pf::helper::GetPathRelative(dir, maskName);
			archive << params._flags;
			archive << params.pos;
			archive << params.siz;
			archive << params.scale;
