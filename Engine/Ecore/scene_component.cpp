...
namespace pf::scene {
	...
	{
		return pf::math::GetPosition(world);
	}
	...
	void MaterialComponent::WriteShaderMaterial(ShaderMaterial* dest) const
	{
		...
		material.options_stencilref |= pf::renderer::CombineStencilrefs(engineStencilRef, userStencilRef) << 24u;
		...
		{
			material.sampler_descriptor = device->GetDescriptorIndex(pf::renderer::GetSampler(pf::enums::SAMPLER_OBJECTSHADER));
		}
		else
		{
			material.sampler_descriptor = sampler_descriptor;
		}
		material.sampler_clamp_descriptor = device->GetDescriptorIndex(pf::renderer::GetSampler(pf::enums::SAMPLER_OBJECTSHADER_CLAMP));
	...
	void MaterialComponent::WriteTextures(const pf::graphics::GPUResource** dest, int count) const
	{
	...
	{
		if (IsCustomShader() && customShaderID < (int)pf::renderer::GetCustomShaders().size())
		{
			auto& customShader = pf::renderer::GetCustomShaders()[customShaderID];
	...
	pf::resourcemanager::Flags MaterialComponent::GetTextureSlotResourceFlags(TEXTURESLOT slot)
	{
		pf::resourcemanager::Flags flags = pf::resourcemanager::Flags::NONE;
		if (!IsPreferUncompressedTexturesEnabled())
		{
			flags |= pf::resourcemanager::Flags::IMPORT_BLOCK_COMPRESSED;
		}
		if (!IsTextureStreamingDisabled())
		{
			flags |= pf::resourcemanager::Flags::STREAMING;
		}
		switch (slot)
		{
		case NORMALMAP:
		case CLEARCOATNORMALMAP:
			flags |= pf::resourcemanager::Flags::IMPORT_NORMALMAP;
			break;
		default:
			break;
		}
	...
	void MaterialComponent::CreateRenderData(bool force_recreate)
	{
		...
		for (uint32_t slot = 0; slot < TEXTURESLOT_COUNT; ++slot)
		{
			...
			{
				pf::resourcemanager::Flags flags = GetTextureSlotResourceFlags(TEXTURESLOT(slot));
				textureslot.resource = pf::resourcemanager::Load(textureslot.name, flags);
			}
		}
	}
	uint32_t MaterialComponent::GetStencilRef() const
	{
		return pf::renderer::CombineStencilrefs(engineStencilRef, userStencilRef);
	}
	...
	void MeshComponent::CreateRenderData()
	{
		...
		pf::vector<unsigned int> provoke(indices.size()); // mesh needs to be drawn with this as index buffer when SV_PrimitiveID is required
		pf::vector<unsigned int> reorder(vertex_positions.size() + indices.size() / 3); // This can be used as vertexID lookup from current index
		...
		for (size_t i = 0; i < vertex_positions.size(); ++i)
		{
			...
			_min = pf::math::Min(_min, pos);
			_max = pf::math::Max(_max, pos);
		}
		aabb = AABB(_min, _max);
		...
		if (!vertex_uvset_0.empty() || !vertex_uvset_1.empty())
		{
			...
			{
				uv_range_max = pf::math::Max(uv_range_max, uv0_stream[i]);
				uv_range_max = pf::math::Max(uv_range_max, uv1_stream[i]);
				uv_range_min = pf::math::Min(uv_range_min, uv0_stream[i]);
				uv_range_min = pf::math::Min(uv_range_min, uv1_stream[i]);
			}
		...
		pf::vector<ShaderCluster> clusters;
		pf::vector<ShaderClusterBounds> cluster_bounds;
		...
		pf::renderer::BufferSuballocation suballoc = pf::renderer::SuballocateGPUBuffer(bd.size);
	...
	void MeshComponent::BuildBVH()
	{
		...
		for (uint32_t subsetIndex = first_subset; subsetIndex < last_subset; ++subsetIndex)
		{
			...
			for (uint32_t triangleIndex = 0; triangleIndex < triangleCount; ++triangleIndex)
			{
				...
				AABB aabb = pf::primitive::AABB(pf::math::Min(p0, pf::math::Min(p1, p2)), pf::math::Max(p0, pf::math::Max(p1, p2)));
	...
	void MeshComponent::ComputeNormals(COMPUTE_NORMALS compute)
	{
		...
		if (compute != COMPUTE_NORMALS_SMOOTH_FAST)
		{
			...
			pf::vector<uint32_t> newIndexBuffer;
			pf::vector<XMFLOAT3> newPositionsBuffer;
			pf::vector<XMFLOAT3> newNormalsBuffer;
			pf::vector<XMFLOAT2> newUV0Buffer;
			pf::vector<XMFLOAT2> newUV1Buffer;
			pf::vector<XMFLOAT2> newAtlasBuffer;
			pf::vector<XMUINT4> newBoneIndicesBuffer;
			pf::vector<XMFLOAT4> newBoneWeightsBuffer;
			pf::vector<uint32_t> newColorsBuffer;
		...
		case MeshComponent::COMPUTE_NORMALS_SMOOTH:
		{
			...
			for (size_t i = 0; i < vertex_positions.size(); i++)
			{
				...
				for (size_t ind = 0; ind < indices.size() / 3; ++ind)
				{
					...
					bool match_pos0 =
						pf::math::float_equal(v_search_pos.x, v0.x) &&
						pf::math::float_equal(v_search_pos.y, v0.y) &&
						pf::math::float_equal(v_search_pos.z, v0.z);

					bool match_pos1 =
						pf::math::float_equal(v_search_pos.x, v1.x) &&
						pf::math::float_equal(v_search_pos.y, v1.y) &&
						pf::math::float_equal(v_search_pos.z, v1.z);

					bool match_pos2 =
						pf::math::float_equal(v_search_pos.x, v2.x) &&
						pf::math::float_equal(v_search_pos.y, v2.y) &&
						pf::math::float_equal(v_search_pos.z, v2.z);
				...
				for (uint32_t i = 0; i < subset.indexCount - 1; i++)
				{
					...
					for (uint32_t j = i + 1; j < subset.indexCount; j++)
					{
						...
						const bool duplicated_pos =
							pf::math::float_equal(p0.x, p1.x) &&
							pf::math::float_equal(p0.y, p1.y) &&
							pf::math::float_equal(p0.z, p1.z);

						const bool duplicated_uv0 =
							pf::math::float_equal(u00.x, u01.x) &&
							pf::math::float_equal(u00.y, u01.y);

						const bool duplicated_uv1 =
							pf::math::float_equal(u10.x, u11.x) &&
							pf::math::float_equal(u10.y, u11.y);

						const bool duplicated_atl =
							pf::math::float_equal(at0.x, at1.x) &&
							pf::math::float_equal(at0.y, at1.y);
	...
	size_t MeshComponent::GetMemoryUsageBVH() const
	{
		return
			...
			bvh_leaf_aabbs.size() * sizeof(pf::primitive::AABB);
	}
	...
	{
		pf::vector<MeshSubset> new_subsets;
		int ret = 0;
		const uint32_t lod_count = GetLODCount();
		for (uint32_t lod = 0; lod < lod_count; ++lod)
		{
		...
		if (pf::renderer::IsMeshShaderAllowed() || !cluster_ranges.empty())
		{
	...
	void ObjectComponent::SaveLightmap()
	{
		...
		if (lightmap.IsValid() && has_flag(lightmap.desc.bind_flags, BindFlag::UNORDERED_ACCESS))
		{
			...
			bool success = pf::helper::saveTextureToMemory(lightmap, lightmapTextureData);
			assert(success);
			...
			if (success)
			{
				...
				size_t height = (size_t)lightmapHeight;
				{
					...
					{
						pf::backlog::post(std::string("[OpenImageDenoise error] ") + errorMessage);
					}
					else
					{
			...
			pf::texturehelper::CreateTexture(lightmap, lightmapTextureData.data(), lightmapWidth, lightmapHeight, lightmap.desc.format);
			pf::graphics::GetDevice()->SetName(&lightmap, "lightmap");
		}
	}
	void ObjectComponent::CompressLightmap()
	{
		...
		else
		{
			...
			pf::renderer::BlockCompress(lightmap, bc6tex, cmd);
			pf::helper::saveTextureToMemory(bc6tex, lightmapTextureData); // internally waits for GPU completion
			lightmap.desc = desc;
		}
	}
	...
	void EnvironmentProbeComponent::CreateRenderData()
	{
		...
		{
			resource = pf::resourcemanager::Load(textureName);
		}
		if (resource.IsValid())
		{
		...
		resolution = pf::math::GetNextPowerOfTwo(resolution);
	...
	AnimationComponent::AnimationChannel::PathDataType AnimationComponent::AnimationChannel::GetPathDataType() const
	{
		switch (path)
		{
		case pf::scene::AnimationComponent::AnimationChannel::Path::TRANSLATION:
			return PathDataType::Float3;
		case pf::scene::AnimationComponent::AnimationChannel::Path::ROTATION:
			return PathDataType::Float4;
		case pf::scene::AnimationComponent::AnimationChannel::Path::SCALE:
			return PathDataType::Float3;
		case pf::scene::AnimationComponent::AnimationChannel::Path::WEIGHTS:
			return PathDataType::Weights;

		case pf::scene::AnimationComponent::AnimationChannel::Path::LIGHT_COLOR:
			return PathDataType::Float3;
		case pf::scene::AnimationComponent::AnimationChannel::Path::LIGHT_INTENSITY:
		case pf::scene::AnimationComponent::AnimationChannel::Path::LIGHT_RANGE:
		case pf::scene::AnimationComponent::AnimationChannel::Path::LIGHT_INNERCONE:
		case pf::scene::AnimationComponent::AnimationChannel::Path::LIGHT_OUTERCONE:
			return PathDataType::Float;

		case pf::scene::AnimationComponent::AnimationChannel::Path::SOUND_PLAY:
		case pf::scene::AnimationComponent::AnimationChannel::Path::SOUND_STOP:
			return PathDataType::Event;
		case pf::scene::AnimationComponent::AnimationChannel::Path::SOUND_VOLUME:
			return PathDataType::Float;

		case pf::scene::AnimationComponent::AnimationChannel::Path::EMITTER_EMITCOUNT:
			return PathDataType::Float;

		case pf::scene::AnimationComponent::AnimationChannel::Path::CAMERA_FOV:
		case pf::scene::AnimationComponent::AnimationChannel::Path::CAMERA_FOCAL_LENGTH:
		case pf::scene::AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SIZE:
			return PathDataType::Float;
		case pf::scene::AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SHAPE:
			return PathDataType::Float2;

		case pf::scene::AnimationComponent::AnimationChannel::Path::SCRIPT_PLAY:
		case pf::scene::AnimationComponent::AnimationChannel::Path::SCRIPT_STOP:
			return PathDataType::Event;

		case pf::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_COLOR:
		case pf::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_EMISSIVE:
		case pf::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_TEXMULADD:
			return PathDataType::Float4;
		case pf::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_ROUGHNESS:
		case pf::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_REFLECTANCE:
		case pf::scene::AnimationComponent::AnimationChannel::Path::MATERIAL_METALNESS:
			return PathDataType::Float;
	...
	void SoftBodyPhysicsComponent::CreateFromMesh(MeshComponent& mesh)
	{
		...
		if (physicsIndices.empty())
		{
			...
			if (pinning_required)
			{
				...
				pf::vector<XMFLOAT4> vertices(mesh.vertex_positions.size());
				...
				pf::vector<uint32_t> shadow_indices(source.size());
			...
			pf::unordered_map<uint32_t, size_t> physicsVertices;
			...
			pf::jobsystem::Dispatch(ctx, (uint32_t)mesh.vertex_positions.size(), 64, [&](pf::jobsystem::JobArgs args) {
				...
				for (size_t physicsInd = 0; physicsInd < physicsToGraphicsVertexMapping.size(); ++physicsInd)
				{
					...
					const float dist = pf::math::DistanceSquared(position, position2);
	...
	void CameraComponent::Lerp(const CameraComponent& a, const CameraComponent& b, float t)
	{
		...
		width = pf::math::Lerp(a.width, b.width, t);
		height = pf::math::Lerp(a.height, b.height, t);
		zNearP = pf::math::Lerp(a.zNearP, b.zNearP, t);
		zFarP = pf::math::Lerp(a.zFarP, b.zFarP, t);
		fov = pf::math::Lerp(a.fov, b.fov, t);
		focal_length = pf::math::Lerp(a.focal_length, b.focal_length, t);
		aperture_size = pf::math::Lerp(a.aperture_size, b.aperture_size, t);
		aperture_shape = pf::math::Lerp(a.aperture_shape, b.aperture_shape, t);
	}

	void ScriptComponent::CreateFromFile(const std::string& filename)
	{
		this->filename = filename;
		resource = pf::resourcemanager::Load(filename);
	...
	void SoundComponent::SetLooped(bool value)
	{
		...
		{
			_flags |= LOOPED;
			pf::audio::CreateSoundInstance(&soundResource.GetSound(), &soundinstance);
		}
		else
		{
			_flags &= ~LOOPED;
			pf::audio::ExitLoop(&soundinstance);
		}
	}
	...
	{
		pf::video::Seek(&videoinstance, timerSeconds);
	}
	...
	{
		return pf::math::Lerp(position_prev, position, alpha);
	}
	...
	XMMATRIX SplineComponent::EvaluateSplineAt(float t) const
	{
		...
		XMVECTOR P = pf::math::CatmullRomCentripetal(P0, P1, P2, P3, tmid);
		XMVECTOR P_prev = pf::math::CatmullRomCentripetal(P0, P1, P2, P3, saturate(tmid - 0.01f));
		XMVECTOR P_next = pf::math::CatmullRomCentripetal(P0, P1, P2, P3, saturate(tmid + 0.01f));
	...
	XMVECTOR SplineComponent::ClosestPointOnSpline(const XMVECTOR& P, int steps) const
	{
		...
		XMVECTOR A = pf::math::GetPosition(EvaluateSplineAt(0.0f));
		for (int i = 1; i < steps; ++i)
		{
			const float t = float(i) / float(steps - 1);
			const XMMATRIX M = EvaluateSplineAt(t);
			XMVECTOR R = pf::math::GetRight(M);
			float nodewidth = XMVectorGetX(XMVector3Length(R));
			nodewidth *= width;
			R = XMVector3Normalize(R) * nodewidth;
			const XMVECTOR B = pf::math::GetPosition(M);
			XMVECTOR C = pf::math::ClosestPointOnLineSegment(A, B, P); // point on spline center
			C = pf::math::ClosestPointOnLineSegment(C - R, C + R, P); // extruded segment by spline width
			const float dist = pf::math::Distance(P, C);
			if (dist < mindist)
			{
				mindist = dist;
				MIN = C;
	...
	XMVECTOR SplineComponent::TraceSplinePlane(const XMVECTOR& ORIGIN, const XMVECTOR& DIRECTION, int steps) const
	{
		...
		for (int i = 0; i < steps; ++i)
		{
			const float t = float(i) / float(steps - 1);
			const XMMATRIX M = EvaluateSplineAt(t);
			const XMVECTOR P = pf::math::GetPosition(M);
			const XMVECTOR N = pf::math::GetUp(M);
			...
			const float dist = pf::math::Distance(P, I);
			if (dist < mindist)
			{
				mindist = dist;
				MIN = I;
	...
	AABB SplineComponent::ComputeAABB(int steps) const
	{
		...
		for (int i = 0; i < steps; ++i)
		{
			const float t = float(i) / float(steps - 1);
			const XMMATRIX M = EvaluateSplineAt(t);
			const float nodewidth = XMVectorGetX(XMVector3Length(pf::math::GetRight(M)));
			const float range = nodewidth * rangemod;
			const XMVECTOR P = pf::math::GetPosition(M);
			const XMVECTOR R = XMVector3Normalize(pf::math::GetRight(M)) * range;
			const XMVECTOR N = XMVector3Normalize(pf::math::GetUp(M)) * range;
			const XMVECTOR F = XMVector3Normalize(pf::math::GetForward(M)) * range;
			ret.AddPoint(P - R);
			ret.AddPoint(P + R);
			ret.AddPoint(P - N);
			ret.AddPoint(P + N);
	...
	void SplineComponent::PrecomputeSplineNodeDistances()
	{
		...
		for (int i = 0; i < beforelast; ++i)
		{
			...
			const float distance = pf::math::Distance(pos0, pos1);
