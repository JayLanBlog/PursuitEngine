...
namespace pf
{
	struct VoxelGrid
	{
		...
		pf::vector<uint64_t> voxels; // 1 array element stores 4 * 4 * 4 = 64 voxels
		...
		void inject_aabb(const pf::primitive::AABB& aabb, bool subtract = false);
		void inject_sphere(const pf::primitive::Sphere& sphere, bool subtract = false);
		void inject_capsule(const pf::primitive::Capsule& capsule, bool subtract = false);
		...
		pf::primitive::AABB get_aabb() const;
		void from_aabb(const pf::primitive::AABB& aabb);
		...
		bool is_visible(const XMFLOAT3& observer, const pf::primitive::AABB& subject) const;
		void add(const VoxelGrid& other);
		void subtract(const VoxelGrid& other);
		void flood_fill();
		void debugdraw(const XMFLOAT4X4& ViewProjection, pf::graphics::CommandList cmd) const;
		...
		void create_mesh(
			...
			bool simplify,
			const pf::scene::Scene* navscene = nullptr
		);

		//#define DEBUG_VOXEL_OCCLUSION
#ifdef DEBUG_VOXEL_OCCLUSION
		mutable pf::vector<XMUINT3> debug_subject_coords;
		mutable pf::vector<XMUINT3> debug_visible_coords;
		mutable pf::vector<XMUINT3> debug_occluded_coords;
