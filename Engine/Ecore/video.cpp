...
namespace pf::video {
	...
	void CalculatePictureOrder(Video* video)
	{
		...
		pf::vector<size_t> frame_display_order(video->frame_infos.size());
		for (size_t i = 0; i < video->frame_infos.size(); ++i)
		{
	...
	bool CreateVideo(const std::string& filename, Video* video)
	{
		...
		if (!pf::helper::FileRead(filename, filedata))
			return false;
		std::string ext = pf::helper::toUpper(pf::helper::GetExtensionFromFileName(filename));
			...
			for (uint32_t ntrack = 0; ntrack < mp4.track_count; ntrack++)
			{
				...
				if (track.handler_type == MP4D_HANDLER_TYPE_VIDE)
				{
					...
					case MP4_OBJECT_TYPE_HEVC:
						...
						p_log_warning("H265 (HEVC) video format is not supported yet!");
						
						return false;
					default:
						p_log_warning("Unknown video format! track.object_type_indication = %d", (int)track.object_type_indication);
						return false;
					}
...
#ifdef DEBUG_DUMP_H264
					pf::helper::FileWrite("dump.h264", dump.data(), dump.size());

					// validate dump:
					if (pf::helper::FileRead("dump.h264", dump))
					{
					...
					assert(success);
					device->SetName(&video->data_stream, "pf::Video::data_stream");
				}
				else if (track.handler_type == MP4D_HANDLER_TYPE_SOUN)
				{
					pf::backlogger::postin("Audio from video file is not implemented yet!");
				}
			}
		}
		else
		{
			p_log_warning("MP4 parsing failure! MP4D_open result = %d", result);
			return false;
		}
		MP4D_close(&mp4);
		return success;
		...
		while (h264::find_next_nal(&bs))
		{
			...
			if (h264::read_nal_header(&nal, &bs))
			{
				...
				case h264::NAL_UNIT_TYPE_CODED_SLICE_NON_IDR:
				{
					...
					frame_info.type = pf::graphics::VideoFrameType::Predictive;
				...
				case h264::NAL_UNIT_TYPE_CODED_SLICE_IDR:
				{
					Video::FrameInfo& frame_info = video->frame_infos.emplace_back();
					frame_info.offset = nal_offset;
					frame_info.type = pf::graphics::VideoFrameType::Intra;
					frame_info.reference_priority = nal.idc;
					h264::SliceHeader slice_header = {};
					h264::read_slice_header(&slice_header, &nal, (const h264::PPS*)video->pps_datas.data(), (const h264::SPS*)video->sps_datas.data(), &bs);
					video->slice_header_datas.resize(video->slice_header_datas.size() + sizeof(h264::SliceHeader));
			...
			{
				p_log_warning("CreateVideoH264RAW: found invalid NAL unit!\n");
			}
		}
		...
		while (IsDecodingRequired(instance))
		{
			...
			if (decode.display_order >= instance->target_display_order || frame_info.reference_priority > 0) // this check can skip decoding for non-reference frame that is a low enough display order
			{
				...
				if (has_flag(instance->decoder.support, VideoDecoderSupportFlags::DPB_AND_OUTPUT_COINCIDE))
				{
					...
					{
						pf::renderer::PushBarrier(GPUBarrier::Image(&instance->dpb.texture, instance->dpb.resource_states[instance->dpb.current_slot], ResourceState::VIDEO_DECODE_DPB, 0, instance->dpb.current_slot, &aspect_luma));
						pf::renderer::PushBarrier(GPUBarrier::Image(&instance->dpb.texture, instance->dpb.resource_states[instance->dpb.current_slot], ResourceState::VIDEO_DECODE_DPB, 0, instance->dpb.current_slot, &aspect_chroma));
					...
					for (size_t i = 0; i < instance->dpb.reference_usage.size(); ++i)
					{
						...
						{
							pf::renderer::PushBarrier(GPUBarrier::Image(&instance->dpb.texture, instance->dpb.resource_states[ref], ResourceState::VIDEO_DECODE_SRC, 0, ref, &aspect_luma));
							pf::renderer::PushBarrier(GPUBarrier::Image(&instance->dpb.texture, instance->dpb.resource_states[ref], ResourceState::VIDEO_DECODE_SRC, 0, ref, &aspect_chroma));
				...
				else
				{
					...
					pf::renderer::PushBarrier(GPUBarrier::Image(&decode.src, decode.src.desc.layout, ResourceState::VIDEO_DECODE_DST, -1, -1, &aspect_luma));
					pf::renderer::PushBarrier(GPUBarrier::Image(&decode.src, decode.src.desc.layout, ResourceState::VIDEO_DECODE_DST, -1, -1, &aspect_chroma));
				}
	...
	void ResolveVideoToRGB(VideoInstance* instance, CommandList cmd)
	{
		...
		for (auto& resolve_request : instance->output_textures_resolve_request)
		{
			...
			if (has_flag(instance->flags, VideoInstance::Flags::Mipmapped))
			{
				pf::renderer::GenerateMipChain(resolve_request.texture, pf::renderer::MIPGENFILTER_LINEAR, cmd);
			}
		}
		instance->output_textures_resolve_request.clear();
	...
	void Seek(VideoInstance* instance, float timerSeconds)
	{
		...
		for (size_t i = 0; i < video->frame_infos.size(); ++i)
		{
			...
			if (frame_info.type == pf::graphics::VideoFrameType::Intra)
			{
				max_iframe = (int)i;
			}
		...
		if (instance->current_decode_frame != max_iframe)
		{
			...
			instance->flags |= pf::video::VideoInstance::Flags::DecoderReset;
			instance->flags &= ~pf::video::VideoInstance::Flags::InitialFirstFrameDecoded;
			instance->output_textures_free.clear();
			instance->output_textures_used.clear();
			instance->output_textures_resolve_request.clear();
		}
