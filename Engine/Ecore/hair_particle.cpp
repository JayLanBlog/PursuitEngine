...
#include "scene.h"
#include "Shader/ShaderInterop_HairParticle.h"
...
namespace pf
{
	...
	void HairParticleSystem::CreateFromMesh(const pf::scene::MeshComponent& mesh)
	{
	...
	void HairParticleSystem::CreateRenderData()
	{
		...
		if (!vertex_lengths.empty())
		{
			auto pack_lengths = [&](void* dest) {
				...
				{
					uint8_t packed = uint8_t(pf::math::Clamp(vertex_lengths[i], 0, 1) * 255.0f);
	...
	)
	{
		...
		for (uint32_t i = 0; i < itemCount; ++i)
		{
			...
			pf::renderer::PushBarrier(GPUBarrier::Buffer(&hair.constantBuffer, ResourceState::COPY_DST, ResourceState::CONSTANT_BUFFER));
			...
			pf::renderer::PushBarrier(GPUBarrier::Buffer(&hair.generalBuffer, ResourceState::COPY_DST, ResourceState::UNORDERED_ACCESS));
		...
		for (uint32_t i = 0; i < itemCount; ++i)
		{
			...
			pf::renderer::PushBarrier(GPUBarrier::Buffer(&hair.generalBuffer, ResourceState::UNORDERED_ACCESS, ResourceState::INDIRECT_ARGUMENT | ResourceState::INDEX_BUFFER | ResourceState::SHADER_RESOURCE));
		}
	...
	void HairParticleSystem::Draw(const MaterialComponent& material, pf::enums::RENDERPASS renderPass, CommandList cmd) const
	{
	...
	void HairParticleSystem::Serialize(pf::Archive& archive, pf::ecs::EntitySerializer& seri)
	{
	...
	namespace HairParticleSystem_Internal
	{
		void LoadShaders()
		{
			pf::renderer::LoadShader(ShaderStage::VS, vs, "hairparticleVS.cso");

			pf::renderer::LoadShader(ShaderStage::PS, ps_simple, "hairparticlePS_simple.cso");
			pf::renderer::LoadShader(ShaderStage::PS, ps_prepass, "hairparticlePS_prepass.cso");
			pf::renderer::LoadShader(ShaderStage::PS, ps_prepass_depthonly, "hairparticlePS_prepass_depthonly.cso");
			pf::renderer::LoadShader(ShaderStage::PS, ps_shadow, "hairparticlePS_shadow.cso");
			pf::renderer::LoadShader(ShaderStage::PS, ps, "hairparticlePS.cso");

			pf::renderer::LoadShader(ShaderStage::CS, cs_simulate, "hairparticle_simulateCS.cso");
	...
	void HairParticleSystem::Initialize()
	{
		...
		if (IsFormatUnorm(pf::renderer::format_depthbuffer_shadowmap))
		{
		...
		static pf::eventhandler::Handle handle = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { HairParticleSystem_Internal::LoadShaders(); });
		HairParticleSystem_Internal::LoadShaders();

		p_log("pf::HairParticleSystem Initialized (%d ms)", (int)std::round(timer.elapsed()));
	}
