...
namespace pf
{
	...
	public:
		...
		void UpdateDisplacementMap(pf::graphics::CommandList cmd) const;
		void RenderForOcclusionTest(const pf::scene::CameraComponent& camera, pf::graphics::CommandList cmd) const;
		void Render(const pf::scene::CameraComponent& camera, pf::graphics::CommandList cmd) const;

		void CopyDisplacementMapReadback(pf::graphics::CommandList cmd) const;

		const pf::graphics::Texture* getDisplacementMap() const;
		const pf::graphics::Texture* getGradientMap() const;
		...
		mutable int occlusionQueries[pf::graphics::GraphicsDevice::GetBufferCount()];
	...
	protected:
		...
		pf::graphics::Texture displacementMap;		// (RGBA32F)
		pf::graphics::Texture gradientMap;			// (RGBA16F)

		pf::graphics::Texture displacementMap_readback[pf::graphics::GraphicsDevice::GetBufferCount()];		// (RGBA32F)
		...
		pf::graphics::GPUBuffer buffer_Float2_H0;

		// Angular frequency
		pf::graphics::GPUBuffer buffer_Float_Omega;

		// Height field H(t), choppy field Dx(t) and Dy(t) in frequency domain, updated each frame.
		pf::graphics::GPUBuffer buffer_Float2_Ht;

		// Height & choppy buffer in the space domain, corresponding to H(t), Dx(t) and Dy(t)
		pf::graphics::GPUBuffer buffer_Float_Dxyz;


		pf::graphics::GPUBuffer constantBuffer;
		mutable pf::graphics::GPUBuffer indexBuffer;
		mutable pf::graphics::GPUBuffer indexBuffer_occlusionTest;
	};
}
