...
namespace pf::scene {
	...
	void Scene::Update(float dt)
	{
		...
		pf::jobsystem::Execute(collider_bvh_workload, [this](pf::jobsystem::JobArgs args) {
		...
		if (pf::renderer::GetOcclusionCullingEnabled() && !pf::renderer::GetFreezeCullingCameraEnabled())
		{
		...
		if (dt > 0)
		{
			...
			pf::jobsystem::Dispatch(ctx, (uint32_t)objects.GetCount(), small_subtask_groupsize, [this](pf::jobsystem::JobArgs args) {
			...
			pf::jobsystem::Dispatch(ctx, (uint32_t)meshes.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
			...
			pf::jobsystem::Dispatch(ctx, (uint32_t)armatures.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
		...
		if (pf::renderer::GetDDGIEnabled())
		{
			...
			if (!ddgi.ray_buffer.IsValid()) // Check the ray_buffer here because that is invalid with serialized DDGI data, and we can detect if dynamic resources need recreation when serialized is loaded
			{
				...
				pf::vector<uint8_t> zerodata(ComputeTextureMemorySizeInBytes(tex));
			...
			ddgi.grid_min = pf::math::Lerp(ddgi.grid_min, grid_min, bounds_blend);
			ddgi.grid_max = pf::math::Lerp(ddgi.grid_max, grid_max, bounds_blend);
		}
		...
		shaderscene.weather.sun_color = pf::math::pack_half3(weather.sunColor);
		shaderscene.weather.sun_direction = pf::math::pack_half3(weather.sunDirection);
		shaderscene.weather.most_important_light_index = weather.most_important_light_index;
		shaderscene.weather.ambient = pf::math::pack_half3(weather.ambient);
		...
		shaderscene.weather.horizon = pf::math::pack_half3(weather.horizon);
		shaderscene.weather.zenith = pf::math::pack_half3(weather.zenith);
	...
	void Scene::Merge(Scene& other)
	{
		...
		CountCPUandGPUColliders();
		const size_t size =
			sizeof(pf::primitive::AABB) * collider_count_cpu +
			sizeof(pf::primitive::AABB) * collider_count_gpu +
		...
		static_assert(sizeof(pf::primitive::AABB) % 16 == 0);
		...
		aabb_colliders_cpu = reinterpret_cast<pf::primitive::AABB*>(colliders_gpu + collider_count_gpu);
	...
	Entity Scene::Instantiate(Scene& prefab, bool attached)
	{
		...
		archive = pf::Archive(prefab.optimized_instatiation_data.GetData(), prefab.optimized_instatiation_data.GetSize());
		...
		pf::jobsystem::Wait(seri.ctx); // wait for completion of component serializations background threads here
	...
	void Scene::FindAllEntities(pf::unordered_set<pf::ecs::Entity>& entities) const
	{
		for (auto& entry : componentLibrary.entries)
		{
	...
	void Scene::Entity_Remove(Entity entity, bool recursive, bool keep_sorted)
	{
		if (recursive)
		{
			pf::vector<Entity> entities_to_remove;
	...
	bool Scene::Entity_IsDescendant(pf::ecs::Entity entity, pf::ecs::Entity ancestor) const
	{
	...
	)
	{
		...
		if (!filename.empty())
		{
			...
			sound.soundResource = pf::resourcemanager::Load(filename);
			sound.soundinstance.SetLooped(sound.IsLooped());
			pf::audio::CreateSoundInstance(&sound.soundResource.GetSound(), &sound.soundinstance);
		}
	...
	)
	{
		...
		if (!filename.empty())
		{
			...
			video.videoResource = pf::resourcemanager::Load(filename);
			if (video.videoResource.IsValid())
			{
				pf::video::CreateVideoInstance(&video.videoResource.GetVideo(), &video.videoinstance);
			}
		}
	...
	void Scene::Component_TransformWorldToHierarchy(pf::ecs::Entity entity)
	{
		const HierarchyComponent* hier = hierarchy.GetComponent(entity);
		if (hier == nullptr)
			return;
	...
	void Scene::GatherChildren(Entity parent, pf::vector<Entity>& children) const
	{
		for (size_t i = 0; i < hierarchy.GetCount(); ++i)
		{
			Entity child = hierarchy.GetEntity(i);
	...
	void Scene::RunAnimationUpdateSystem(pf::jobsystem::context& ctx)
	{
		auto range = pf::profiler::BeginRangeCPU("Animations");

		pf::jobsystem::Wait(animation_dependency_scan_workload);

		pf::jobsystem::Dispatch(ctx, (uint32_t)animation_queue_count, 1, [&](pf::jobsystem::JobArgs args) {
				...
				for (const AnimationComponent::AnimationChannel& channel : animation.channels)
				{
					...
					else
					{
						...
						case AnimationComponent::AnimationSampler::Mode::STEP:
						{
							...
							const int key = pf::math::InverseLerp(timeLeft, timeRight, animation.timer) > 0.5f ? keyRight : keyLeft;
						...
						case AnimationComponent::AnimationSampler::Mode::LINEAR:
						{
							...
							case AnimationComponent::AnimationChannel::PathDataType::Float:
							{
								...
								float vAnim = pf::math::Lerp(vLeft, vRight, t);
					...
					const bool isRootBone = (animation.IsRootMotion() && animation.rootMotionBone != pf::ecs::INVALID_ENTITY && (target_transform == transforms.GetComponent(animation.rootMotionBone)));
					...
					if (target_mesh != nullptr)
					{
						...
						{
							target_mesh->morph_targets[j].weight = pf::math::Lerp(target_mesh->morph_targets[j].weight, animation.morph_weights_temp[j], t);
						}
					}
					...
					if (target_light != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::LIGHT_COLOR:
						{
							target_light->color = pf::math::Lerp(target_light->color, interpolator.f3, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_INTENSITY:
						{
							target_light->intensity = pf::math::Lerp(target_light->intensity, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_RANGE:
						{
							target_light->range = pf::math::Lerp(target_light->range, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_INNERCONE:
						{
							target_light->innerConeAngle = pf::math::Lerp(target_light->innerConeAngle, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::LIGHT_OUTERCONE:
						{
							target_light->outerConeAngle = pf::math::Lerp(target_light->outerConeAngle, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_sound != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::SOUND_VOLUME:
						{
							target_sound->volume = pf::math::Lerp(target_sound->volume, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_emitter != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::EMITTER_EMITCOUNT:
						{
							target_emitter->count = pf::math::Lerp(target_emitter->count, interpolator.f, t);
						}
						break;
						default:
							break;
					...
					if (target_camera != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::CAMERA_FOV:
						{
							target_camera->fov = pf::math::Lerp(target_camera->fov, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_FOCAL_LENGTH:
						{
							target_camera->focal_length = pf::math::Lerp(target_camera->focal_length, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SIZE:
						{
							target_camera->aperture_size = pf::math::Lerp(target_camera->aperture_size, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::CAMERA_APERTURE_SHAPE:
						{
							target_camera->aperture_shape = pf::math::Lerp(target_camera->aperture_shape, interpolator.f2, t);
						}
						break;
						default:
							break;
					...
					if (target_material != nullptr)
					{
						...
						case AnimationComponent::AnimationChannel::Path::MATERIAL_COLOR:
						{
							target_material->baseColor = pf::math::Lerp(target_material->baseColor, interpolator.f4, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_EMISSIVE:
						{
							target_material->emissiveColor = pf::math::Lerp(target_material->emissiveColor, interpolator.f4, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_ROUGHNESS:
						{
							target_material->roughness = pf::math::Lerp(target_material->roughness, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_METALNESS:
						{
							target_material->metalness = pf::math::Lerp(target_material->metalness, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_REFLECTANCE:
						{
							target_material->reflectance = pf::math::Lerp(target_material->reflectance, interpolator.f, t);
						}
						break;
						case AnimationComponent::AnimationChannel::Path::MATERIAL_TEXMULADD:
						{
							target_material->texMulAdd = pf::math::Lerp(target_material->texMulAdd, interpolator.f4, t);
						}
						break;
						default:
							break;
	...
	void Scene::RunTransformUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)transforms.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {

			TransformComponent& transform = transforms[args.jobIndex];
			transform.UpdateTransform();
			});
	}
	void Scene::RunHierarchyUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)hierarchy.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunExpressionUpdateSystem(pf::jobsystem::context& ctx)
	{
		for (size_t i = 0; i < expressions.GetCount(); ++i)
		{
			...
			if (expression_mastering.blink_timer >= 1)
			{
				...
				if (blink >= 0 && blink < expression_mastering.expressions.size())
				{
					...
					float blink_index = std::floor(pf::math::Lerp(0, (float)expression_mastering.blink_count, (expression_mastering.blink_timer - 1) / all_blink_length));
					float blink_trim = 1 + one_blink_length * blink_index;
					float blink_state = pf::math::InverseLerp(0, one_blink_length, expression_mastering.blink_timer - blink_trim);
					if (blink_state < 0.5f)
					{
						// closing
						expression.weight = pf::math::Lerp(0, 1, saturate(blink_state * 2));
					}
					else
					{
						// opening
						expression.weight = pf::math::Lerp(1, 0, saturate((blink_state - 0.5f) * 2));
					}
					if (expression_mastering.blink_timer >= 1 + all_blink_length)
					{
						expression.weight = 0;
						expression_mastering.blink_timer = -pf::random::GetRandom(0.0f, 1.0f);
					}
					expression.SetDirty();
				}
			}
			...
			if (expression_mastering.look_timer == 0)
			{
				...
				float vertical = pf::random::GetRandom(-1.0f, 1.0f);
				float horizontal = pf::random::GetRandom(-1.0f, 1.0f);
			...
			if (expression_mastering.look_timer >= 1)
			{
				...
				for (int idx = 0; idx < arraysize(looks); ++idx)
				{
					...
					if (look >= 0 && look < expression_mastering.expressions.size())
					{
						...
						float look_state = pf::math::InverseLerp(0, expression_mastering.look_length * expression_mastering.look_frequency, expression_mastering.look_timer - 1);
						if (look_state < 0.25f)
						{
							expression.weight = pf::math::Lerp(0, weight, saturate(look_state * 4));
						}
						else
						{
							expression.weight = pf::math::Lerp(weight, 0, saturate((look_state - 0.75f) * 4));
						}
						expression.SetDirty();
					}
				}
				...
				{
					expression_mastering.look_timer = -pf::random::GetRandom(0.0f, 1.0f);
				}
			}
			...
			if (voice_playing || expression_mastering.IsForceTalkingEnabled())
			{
				...
				for (int phoneme = (int)ExpressionComponent::Preset::Aa; phoneme <= (int)ExpressionComponent::Preset::Oh; phoneme++)
				{
					if (phoneme != (int)expression_mastering.talking_phoneme) // don't allow to select the current phoneme next
					{
						...
						expression.weight = pf::math::Lerp(expression.weight, 0, 0.4f); // fade out unused
						expression.SetDirty();
					}
				}
				...
				if (voice_playing)
				{
					...
					pf::audio::SampleInfo info = pf::audio::GetSampleInfo(&sound->soundResource.GetSound());
					uint32_t sample_frequency = info.sample_rate * info.channel_count;
					uint64_t current_sample = pf::audio::GetTotalSamplesPlayed(&sound->soundinstance);
					if (sound->IsLooped())
					{
					...
					if (voice > 0.1f)
					{
						expression.weight = pf::math::Lerp(expression.weight, 1, strength);
					}
					else
					{
						expression.weight = pf::math::Lerp(expression.weight, 0, strength);
					}
				}
				else
				{
				...
				if (prev_slope < 0 && curr_slope > 0)
				{
					...
					expression_mastering.talking_phoneme = unused_phonemes[pf::random::GetRandom(0, (int)arraysize(unused_phonemes) - 1)];
				}
			...
			else if (expression_mastering._flags & ExpressionComponent::TALKING_ENDED)
			{
				...
				for (auto& phoneme : phonemes)
				{
					...
					expression.weight = pf::math::Lerp(expression.weight, 0, 0.4f);
					expression.SetDirty();
			...
			for (ExpressionComponent::Expression& expression : expression_mastering.expressions)
			{
				...
				for (const ExpressionComponent::Expression::MorphTargetBinding& morph_target_binding : expression.morph_target_bindings)
				{
					...
					{
						MeshComponent::MorphTarget& morph_target = mesh->morph_targets[morph_target_binding.index];
						morph_target.weight = pf::math::Lerp(morph_target.weight, morph_target_binding.weight, blend);
					}
				}
			}
		}
	}
	void Scene::RunProceduralAnimationUpdateSystem(pf::jobsystem::context& ctx)
	{
		auto range = pf::profiler::BeginRangeCPU("Procedural Animations");
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)characters.GetCount(), 1, [&](pf::jobsystem::JobArgs args) {
			...
			if (character.IsFootPlacementEnabled() && character.ground_intersect)
			{
				...
				character.foot_offset = pf::math::Lerp(character.foot_offset, diff, saturate(5 * dt));
			}
			else
			{
				character.foot_offset = pf::math::Lerp(character.foot_offset, 0.0f, saturate(5 * dt));
			}
			...
			if (inverse_kinematics.Contains(character.left_foot))
			{
				...
				ik.target_position = pf::math::Lerp(ik.target_position, left_pos, foot_blend);
			}
			if (inverse_kinematics.Contains(character.right_foot))
			{
				InverseKinematicsComponent& ik = *inverse_kinematics.GetComponent(character.right_foot);
				ik.target_position = pf::math::Lerp(ik.target_position, right_pos, foot_blend);
			}
			...
			if (ik_foot != INVALID_ENTITY && inverse_kinematics.Contains(ik_foot))
			{
				...
				ik_pos.y += 0.16f;
				ik.target_position = pf::math::Lerp(ik.target_position, ik_pos, foot_blend);
#if 0
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)inverse_kinematics.GetCount(), 1, [this, &recompute_hierarchy](pf::jobsystem::JobArgs args) {
		...
		pf::jobsystem::Wait(ctx); // sync needed when there is IK on character arm/leg, and also arm/leg spacing!

		pf::jobsystem::Dispatch(ctx, (uint32_t)humanoids.GetCount(), 1, [this, &recompute_hierarchy](pf::jobsystem::JobArgs args) {
			...
			for (auto& source : sources)
			{
				...
				if (boneIndex < transforms_temp.size())
				{
					...
					if (humanoid.IsLookAtEnabled())
					{
						...
						const float angle_horizontal = pf::math::GetAngle(FORWARD, TARGET_HORIZONTAL, UP, source.rotation_max->x);
						const float angle_vertical = pf::math::GetAngle(FORWARD, TARGET_VERTICAL, SIDE, source.rotation_max->y);
						...
						pf::renderer::DebugTextParams textparams;
						textparams.flags |= pf::renderer::DebugTextParams::CAMERA_FACING;
						textparams.flags |= pf::renderer::DebugTextParams::CAMERA_SCALING;
						textparams.position = humanoid.lookAt;
						textparams.scaling = 0.8f;
						pf::renderer::DrawDebugText(text.c_str(), textparams);
#endif
					}
		...
		// Colliders:
		pf::jobsystem::Wait(collider_bvh_workload); // waits for BVH build and collider counts
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)colliders.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
		...
		// Springs:
		pf::jobsystem::Wait(spring_dependency_scan_workload);
		if (dt > 0)
		{
			pf::jobsystem::Dispatch(ctx, (uint32_t)spring_queues.size(), 1, [this](pf::jobsystem::JobArgs args) {
		...
		if (collider_count_cpu > 0)
		{
			...
			collider_bvh_workload.priority = pf::jobsystem::Priority::Low;
			pf::jobsystem::Execute(collider_bvh_workload, [this](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunArmatureUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)armatures.GetCount(), 1, [&](pf::jobsystem::JobArgs args) {
			...
			for (Entity boneEntity : armature.boneCollection)
			{
				...
				_min = pf::math::Min(_min, boneAABB._min);
				_max = pf::math::Max(_max, boneAABB._max);
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)softbodies.GetCount(), 1, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunMeshUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)meshes.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
			...
			if (geometryArrayMapped != nullptr)
			{
				...
				if (pf::renderer::IsMeshShaderAllowed())
				{
				...
				for (auto& subset : mesh.subsets)
				{
					...
					if (pf::renderer::IsMeshShaderAllowed() && subsetIndex < mesh.cluster_ranges.size())
					{
	...
	void Scene::RunMaterialUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)materials.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
			...
			{
				material.cached_wrapSampler = device->GetDescriptorIndex(pf::renderer::GetSampler(pf::enums::SAMPLER_OBJECTSHADER));
			}
			else
			{
				material.cached_wrapSampler = material.sampler_descriptor;
			}
			material.cached_clampSampler = device->GetDescriptorIndex(pf::renderer::GetSampler(pf::enums::SAMPLER_OBJECTSHADER_CLAMP));
	...
	void Scene::RunImpostorUpdateSystem(pf::jobsystem::context& ctx)
	{
	...
	void Scene::RunObjectUpdateSystem(pf::jobsystem::context& ctx)
	{
		...
		parallel_bounds.resize((size_t)pf::jobsystem::DispatchGroupCount((uint32_t)objects.GetCount(), small_subtask_groupsize));

		pf::jobsystem::Dispatch(ctx, (uint32_t)objects.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
			...
			if (!pf::renderer::GetFreezeCullingCameraEnabled())
			{
			...
			if (object.meshID != INVALID_ENTITY && meshes.Contains(object.meshID) && transforms.Contains(entity))
			{
				...
				inst.color = pf::math::pack_half4(object.color);
				inst.emissive = pf::math::pack_half3(XMFLOAT3(object.emissiveColor.x * object.emissiveColor.w, object.emissiveColor.y * object.emissiveColor.w, object.emissiveColor.z * object.emissiveColor.w));
				...
				inst.alphaTest_size = pf::math::pack_half2(XMFLOAT2(1 - object.alphaRef, size));
				inst.SetUserStencilRef(object.userStencilRef);
				inst.rimHighlight = pf::math::pack_half4(XMFLOAT4(object.rimHighlightColor.x * object.rimHighlightColor.w, object.rimHighlightColor.y * object.rimHighlightColor.w, object.rimHighlightColor.z * object.rimHighlightColor.w, object.rimHighlightFalloff));
				...
				if (TLAS_instancesMapped != nullptr)
				{
					...
					{
						instance.instance_mask &= ~pf::renderer::raytracing_inclusion_mask_shadow;
					}
					if (object.IsNotVisibleInReflections())
					{
						instance.instance_mask &= ~pf::renderer::raytracing_inclusion_mask_reflection;
					}
				...
				if (object.IsLightmapRenderRequested() && dt > 0)
				{
					...
					{
						object.lightmapWidth = pf::math::GetNextPowerOfTwo(object.lightmapWidth + 1) / 2;
						object.lightmapHeight = pf::math::GetNextPowerOfTwo(object.lightmapHeight + 1) / 2;

						// align to BC6 block size:
						object.lightmapWidth = pf::graphics::AlignTo(object.lightmapWidth, 4u);
						object.lightmapHeight = pf::graphics::AlignTo(object.lightmapHeight, 4u);
				...
				if (!object.lightmapTextureData.empty() && !object.lightmap.IsValid())
				{
					...
					{
						pf::backlogger::postin("Invalid or old lightmap data found, it will be discarded. Re-render the lightmap if required.", LogLevel::Warning);
						object.lightmapTextureData.clear();
					}
					if (object.lightmap.desc.format != Format::UNKNOWN)
					{
						pf::texturehelper::CreateTexture(object.lightmap, object.lightmapTextureData.data(), object.lightmapWidth, object.lightmapHeight, object.lightmap.desc.format);
						device->SetName(&object.lightmap, "lightmap");
					}
				}
	...
	void Scene::RunCameraUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)cameras.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunDecalUpdateSystem(pf::jobsystem::context& ctx)
	{
	...
	void Scene::RunProbeUpdateSystem(pf::jobsystem::context& ctx)
	{
	...
	void Scene::RunForceUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)forces.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunLightUpdateSystem(pf::jobsystem::context& ctx)
	{
		aabb_lights.resize(lights.GetCount());

		pf::jobsystem::Dispatch(ctx, (uint32_t)lights.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunParticleUpdateSystem(pf::jobsystem::context& ctx)
	{
		//pf::jobsystem::Dispatch(ctx, (uint32_t)hairs.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
		...
		//	inst.emissive = pf::math::pack_half3(XMFLOAT3(1, 1, 1));
		//	inst.color = pf::math::pack_half4(XMFLOAT4(1, 1, 1, 1));
		...
		//	XMFLOAT4X4 remapMatrix = pf::math::IDENTITY_MATRIX;
		...
		//pf::jobsystem::Dispatch(ctx, (uint32_t)emitters.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
		...
		//			for (int j = 0; j < arraysize(instance.transform[i]); ++j)
		//			{
		//				instance.transform[i][j] = pf::math::IDENTITY_MATRIX.m[j][i];
		//			}
		//		}
		//		instance.instance_id = (uint32_t)instanceIndex;
	...
	void Scene::RunWeatherUpdateSystem(pf::jobsystem::context& ctx)
	{
		...
		if (weather.rain_amount > 0)
		{
			...
			rainEmitter.count = pf::math::Lerp(0, (float)target_max_particle_count, weather.rain_amount);
			rainEmitter.life = 1;
			...
			{
				Texture gradientTex = pf::texturehelper::CreateGradientTexture(
					pf::texturehelper::GradientType::Circular,
					64, 64,
					XMFLOAT2(0.5f, 0.5f), XMFLOAT2(0.5f, 0),
					pf::texturehelper::GradientFlags::Inverse //TO DO : | pf::texturehelper::GradientFlags::Inverse
				);
				Texture gradientTexBC;
				TextureDesc desc = gradientTex.GetDesc();
				desc.format = Format::BC4_UNORM;
				desc.swizzle = { pf::graphics::ComponentSwizzle::ONE,pf::graphics::ComponentSwizzle::ONE,pf::graphics::ComponentSwizzle::ONE,pf::graphics::ComponentSwizzle::R };
				bool success = device->CreateTexture(&desc, nullptr, &gradientTexBC);
				assert(success);
				pf::renderer::AddDeferredBlockCompression(gradientTex, gradientTexBC);
			...
			{
				Texture gradientTex = pf::texturehelper::CreateLensDistortionNormalMap(
					32, 32
				);
				Texture gradientTexBC;
				TextureDesc desc = gradientTex.GetDesc();
				desc.format = Format::BC5_UNORM;
				desc.swizzle = { pf::graphics::ComponentSwizzle::R,pf::graphics::ComponentSwizzle::G,pf::graphics::ComponentSwizzle::ONE,pf::graphics::ComponentSwizzle::ONE };
				bool success = device->CreateTexture(&desc, nullptr, &gradientTexBC);
				assert(success);
				pf::renderer::AddDeferredBlockCompression(gradientTex, gradientTexBC);
	...
	void Scene::RunSoundUpdateSystem(pf::jobsystem::context& ctx)
	{
		...
		for (size_t i = 0; i < sounds.GetCount(); ++i)
		{
			...
			if (!sound.IsDisable3D())
			{
				...
				if (transform != nullptr)
				{
					...
					pf::audio::Update3D(&sound.soundinstance, instance3D);
				}
			}
			if (sound.IsPlaying())
			{
				pf::audio::Play(&sound.soundinstance);
			}
			else
			{
				pf::audio::Stop(&sound.soundinstance);
			}
			pf::audio::SetVolume(sound.volume, &sound.soundinstance);
		}
	}
	void Scene::RunVideoUpdateSystem(pf::jobsystem::context& ctx)
	{
		for (size_t i = 0; i < videos.GetCount(); ++i)
		{
			...
			{
				if (pf::video::CreateVideoInstance(&video.videoResource.GetVideo(), &video.videoinstance))
				{
			...
			if (video.videoResource.IsValid())
			{
				const pf::video::Video& vid = video.videoResource.GetVideo();
	...
	void Scene::RunScriptUpdateSystem(pf::jobsystem::context& ctx)
	{
		if (dt == 0)
			return; // not allowed to be run when dt == 0 as it could be on separate thread!
		auto range = pf::profiler::BeginRangeCPU("Script Components");
		for (size_t i = 0; i < scripts.GetCount(); ++i)
		{
			...
			if (script.IsPlaying())
			{
				if (script.resource.IsValid() && (script.script.empty() || script.script_hash != script.resource.GetScriptHash()))
				{
					...
					//pf::lua::AttachScriptParameters(str, script.filename, pf::lua::GeneratePID(), "local function GetEntity() return " + std::to_string(entity) + "; end;", "");
					//pf::lua::CompileText(str, script.script);
				}
				if (!script.script.empty())
				{
					//pf::lua::RunBinaryData(script.script.data(), script.script.size(), script.filename.c_str());
				}
	...
	void Scene::RunSpriteUpdateSystem(pf::jobsystem::context& ctx)
	{
		pf::jobsystem::Dispatch(ctx, (uint32_t)sprites.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunFontUpdateSystem(pf::jobsystem::context& ctx)
	{
		aabb_fonts.resize(fonts.GetCount());
		pf::jobsystem::Dispatch(ctx, (uint32_t)fonts.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunCharacterUpdateSystem(pf::jobsystem::context& ctx)
	{
		if (dt == 0)
			return;
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)characters.GetCount(), 1, [&](pf::jobsystem::JobArgs args) {
			...
			if (character.IsActive())
			{
				...
				// Smooth facing:
				float faceangle = -pf::math::GetAngleSigned(facing_next, facing, up);
				...
				if (character.pathfinding_thread)
				{
					...
					/*if (character.process_goal && character.voxelgrid != nullptr && !pf::jobsystem::IsBusy(character.pathfinding_thread->ctx))
					{
						...
						pf::jobsystem::Execute(character.pathfinding_thread->ctx, [&](pf::jobsystem::JobArgs args) {
	...
	void Scene::RunSplineUpdateSystem(pf::jobsystem::context& ctx)
	{
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)splines.GetCount(), small_subtask_groupsize, [&](pf::jobsystem::JobArgs args) {
				...
				if (spline.mesh_generation_subdivision > 0)
				{
					...
					if (mesh != nullptr)
					{
						...
						for (size_t i = 0; i < segmentCount; ++i)
						{
							...
							const float width = pf::math::Distance(currentRIGHT, currentLEFT);
							if (i > 0)
							{
								dist += pf::math::Distance(prevMID, currentMID) / std::max(0.01f, width);
							}
							prevMID = currentMID;
		...
		if (filterMask & FILTER_OBJECT_ALL)
		{
			...
			for (size_t objectIndex = 0; objectIndex < objectCount; ++objectIndex)
			{
				...
				auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset, uint32_t triangleIndex)
				{
					...
					float distance;
					XMFLOAT2 bary;
					if (pf::math::RayTriangleIntersects(rayOrigin_local, rayDirection_local, p0, p1, p2, distance, bary))
					{
						...
						distance = pf::math::Distance(pos, rayOrigin);
	...
	void Scene::IntersectsAll(pf::vector<RayIntersectionResult>& results, const Ray& ray, uint32_t filterMask, uint32_t layerMask, uint32_t lod) const
	{
		const XMVECTOR rayOrigin = XMLoadFloat3(&ray.origin);
		const XMVECTOR rayDirection = XMVector3Normalize(XMLoadFloat3(&ray.direction));
	...
	void Scene::IntersectsAll(pf::vector<SphereIntersectionResult>& results, const Sphere& sphere, uint32_t filterMask, uint32_t layerMask, uint32_t lod) const
	{
		const XMVECTOR Center = XMLoadFloat3(&sphere.center);
		const XMVECTOR Radius = XMVectorReplicate(sphere.radius);
		const XMVECTOR RadiusSq = XMVectorMultiply(Radius, Radius);
				...
				auto intersect_triangle = [&](uint32_t subsetIndex, uint32_t indexOffset, uint32_t triangleIndex, bool doubleSided)
				{
					...
					else
					{
						...
						else
						{
							...
							// Edge 0,1
							XMVECTOR Point1 = pf::math::ClosestPointOnLineSegment(p0, p1, LinePlaneIntersection);

							// Edge 1,2
							XMVECTOR Point2 = pf::math::ClosestPointOnLineSegment(p1, p2, LinePlaneIntersection);

							// Edge 2,0
							XMVECTOR Point3 = pf::math::ClosestPointOnLineSegment(p2, p0, LinePlaneIntersection);
					...
					XMVECTOR Center = pf::math::ClosestPointOnLineSegment(A, B, ReferencePoint);
					...
					// Edge 0,1
					XMVECTOR Point1 = pf::math::ClosestPointOnLineSegment(p0, p1, Center);
					...
					// Edge 1,2
					XMVECTOR Point2 = pf::math::ClosestPointOnLineSegment(p1, p2, Center);
					...
					// Edge 2,0
					XMVECTOR Point3 = pf::math::ClosestPointOnLineSegment(p2, p0, Center);
	...
	void Scene::IntersectsAll(pf::vector<CapsuleIntersectionResult>& results, const Capsule& capsule, uint32_t filterMask, uint32_t layerMask, uint32_t lod) const
	{
		const XMVECTOR Base = XMLoadFloat3(&capsule.base);
		const XMVECTOR Tip = XMLoadFloat3(&capsule.tip);
		const XMVECTOR Radius = XMVectorReplicate(capsule.radius);
	...
	void Scene::VoxelizeObject(size_t objectIndex, pf::VoxelGrid& grid, bool subtract, uint32_t lod)
	{
		if (objectIndex >= objects.GetCount() || objectIndex >= aabb_objects.size())
			return;
		if (aabb_objects[objectIndex].intersects(grid.get_aabb()) == pf::primitive::AABB::OUTSIDE)
			return;
		const ObjectComponent& object = objects[objectIndex];
		const MeshComponent* mesh = meshes.GetComponent(object.meshID);
		if (mesh == nullptr)
	...
	void Scene::VoxelizeScene(pf::VoxelGrid& voxelgrid, bool subtract, uint32_t filterMask, uint32_t layerMask, uint32_t lod)
	{
		pf::jobsystem::context ctx;
		if ((filterMask & FILTER_COLLIDER))
		{
			for (size_t i = 0; i < collider_count_cpu; ++i)
			{
				...
				case ColliderComponent::Shape::Sphere:
				{
					...
					pf::jobsystem::Execute(ctx, [&voxelgrid, subtract, sphere](pf::jobsystem::JobArgs args) {
				...
				case ColliderComponent::Shape::Capsule:
				{
					...
					// TODO: fix heap allocating lambda capture!
					pf::jobsystem::Execute(ctx, [&voxelgrid, subtract, capsule](pf::jobsystem::JobArgs args) {
				...
				case ColliderComponent::Shape::Plane:
				{
					...
					// TODO: fix heap allocating lambda capture!
					pf::jobsystem::Execute(ctx, [&voxelgrid, subtract, P0, P1, P2, P3](pf::jobsystem::JobArgs args) {
		...
		if (filterMask & FILTER_OBJECT_ALL)
		{
			for (size_t i = 0; i < objects.GetCount(); ++i)
			{
				...
				if ((layerMask & aabb.layerMask) == 0)
					continue;
				// TODO: fix heap allocating lambda capture!
				pf::jobsystem::Execute(ctx, [this, &voxelgrid, subtract, lod, i](pf::jobsystem::JobArgs args) {
	...
	void Scene::PutWaterRipple(const std::string& image, const XMFLOAT3& pos)
	{
		...
		img.params.pos = pos;
		img.params.rotation = (pf::random::GetRandom(0, 1000) * 0.001f) * 2 * 3.1415f;
		img.params.siz = XMFLOAT2(1, 1);
		img.params.quality = pf::image::QUALITY_ANISOTROPIC;
	...
	{
		pf::Sprite img;
		img.textureResource.SetTexture(*pf::texturehelper::getWaterRipple());
		img.params.enableExtractNormalMap();
		img.params.blendFlag = BLENDMODE_ADDITIVE;
		img.anim.fad = 0.01f;
		img.anim.scaleX = 0.1f;
	...
	XMVECTOR SkinVertex(const MeshComponent& mesh, const pf::vector<ShaderTransform>& boneData, uint32_t index, XMVECTOR* N)
	{
	...
	{
		pf::Archive archive(fileName, true);
		if (!archive.IsOpen())
			return;
	...
	PickResult Pick(const pf::primitive::Ray& ray, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
		return scene.Intersects(ray, filterMask, layerMask, lod);
	}
	SceneIntersectSphereResult SceneIntersectSphere(const pf::primitive::Sphere& sphere, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
		return scene.Intersects(sphere, filterMask, layerMask, lod);
	}
	SceneIntersectCapsuleResult SceneIntersectCapsule(const pf::primitive::Capsule& capsule, uint32_t filterMask, uint32_t layerMask, const Scene& scene, uint32_t lod)
	{
		return scene.Intersects(capsule, filterMask, layerMask, lod);
	}


	XMMATRIX Scene::ComputeEntityMatrixRecursive(pf::ecs::Entity entity) const
	{
		const TransformComponent* transform = transforms.GetComponent(entity);
		if (transform == nullptr)
	...
	XMMATRIX Scene::GetRestPose(pf::ecs::Entity entity) const
	{
	...
	{
		WaitBuildTopDownHierarchy();
		pf::jobsystem::Execute(topdown_hierarchy_workload, [&](pf::jobsystem::JobArgs args) {
	...
	{
		pf::jobsystem::Wait(topdown_hierarchy_workload);
	}
	void Scene::RefreshHierarchyTopdownFromParent(pf::ecs::Entity entity)
	{
	...
	void Scene::ScanAnimationDependencies()
	{
		...
		pf::jobsystem::Execute(animation_dependency_scan_workload, [&](pf::jobsystem::JobArgs args) {
			auto range = pf::profiler::BeginRangeCPU("Animation Dependencies");
	...
	{
		pf::jobsystem::Execute(spring_dependency_scan_workload, [this](pf::jobsystem::JobArgs args) {
			auto range = pf::profiler::BeginRangeCPU("Spring Dependencies");
			spring_queues.clear();
	...
	void Scene::UpdateSpringsTopDownRecursive(SpringComponent* parent_spring, SpringComponent& spring)
	{
		...
		pf::primitive::Sphere tail_sphere;
	...
	void Scene::UpdateHumanoidFacings()
	{
		...
		for (size_t i = 0; i < humanoids.GetCount(); ++i)
		{
			...
			if (humanoid.knee_bending == 0)
			{
				...
				if (transform != nullptr)
				{
					XMFLOAT3 roll_pitch_yaw = pf::math::QuaternionToRollPitchYaw(transform->rotation_local);
