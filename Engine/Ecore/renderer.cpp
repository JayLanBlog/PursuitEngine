...
#include "Shader/ShaderInterop_Postprocess.h"
#include "Shader/ShaderInterop_Raytracing.h"
#include "Shader/ShaderInterop_BVH.h"
#include "Shader/ShaderInterop_DDGI.h"
#include "Shader/ShaderInterop_VXGI.h"
#include "Shader/ShaderInterop_FSR2.h"
#include "Shader/uvsphere.hlsli"
#include "Shader/cone.hlsli"
...
#include <Engine/Shaders/ShaderCompiler.h>
#include <Engine/Shaders/ShaderCompiler.h>
#include <Engine/Ecore/sheen_lut.h>
#define SHADERDUMP_ENABLED
	...
	std::string SHADERPATH = pf::helper::GetCurrentPath() + "/Res/shaders/";
	...
	static thread_local pf::vector<GPUBarrier> barrier_stack;
	...
	pf::vector<std::pair<XMFLOAT4X4, XMFLOAT4>> renderableBoxes;
	pf::vector<std::pair<XMFLOAT4X4, XMFLOAT4>> renderableBoxes_depth;
	pf::vector<std::pair<Sphere, XMFLOAT4>> renderableSpheres;
	pf::vector<std::pair<Sphere, XMFLOAT4>> renderableSpheres_depth;
	pf::vector<std::pair<Capsule, XMFLOAT4>> renderableCapsules;
	pf::vector<std::pair<Capsule, XMFLOAT4>> renderableCapsules_depth;
	pf::vector<RenderableLine> renderableLines;
	pf::vector<RenderableLine> renderableLines_depth;
	pf::vector<RenderableLine2D> renderableLines2D;
	pf::vector<RenderablePoint> renderablePoints;
	pf::vector<RenderablePoint> renderablePoints_depth;
	pf::vector<RenderableTriangle> renderableTriangles_solid;
	pf::vector<RenderableTriangle> renderableTriangles_wireframe;
	pf::vector<RenderableTriangle> renderableTriangles_solid_depth;
	pf::vector<RenderableTriangle> renderableTriangles_wireframe_depth;
	pf::vector<uint8_t> debugTextStorage; // A stream of DebugText struct + text characters
	pf::vector<PaintRadius> paintrads;
	pf::vector<PaintTextureParams> painttextures;
	pf::vector<PaintDecalParams> paintdecals;
	pf::vector<const pf::VoxelGrid*> renderableVoxelgrids;
	pf::vector<const pf::PathQuery*> renderablePathqueries;
	pf::vector<const pf::TrailRenderer*> renderableTrails;

	pf::SpinLock deferredMIPGenLock;
	pf::vector<std::pair<Texture, bool>> deferredMIPGens;
	pf::vector<std::pair<Texture, Texture>> deferredBCQueue;
	...
	union ObjectRenderingVariant
	{
		struct
		{
			uint32_t renderpass : 4;	// pf::enums::RENDERPASS
			uint32_t shadertype : 8;	// MaterialComponent::SHADERTYPE
			uint32_t blendmode : 4;		// pf::enums::BLENDMODE
			uint32_t cullmode : 2;		// pf::graphics::CullMode
	...
	{
		static pf::unordered_map<uint32_t, PipelineState> PSO_object[RENDERPASS_COUNT][MaterialComponent::SHADERTYPE_COUNT][OBJECT_MESH_SHADER_PSO_COUNT];
		return &PSO_object[variant.bits.renderpass][variant.bits.shadertype][variant.bits.mesh_shader][variant.value];
	}
	pf::jobsystem::context mesh_shader_ctx;
	pf::jobsystem::context object_pso_job_ctx;
	PipelineState PSO_object_wire;
	PipelineState PSO_object_wire_tessellation;
	PipelineState PSO_object_wire_mesh_shader;

	pf::jobsystem::context raytracing_ctx;
	pf::jobsystem::context objectps_ctx;

	pf::vector<CustomShader> customShaders;
	...
	)
	{
		...
		{
			std::string ext = pf::helper::GetExtensionFromFileName(shaderbinaryfilename);
			shaderbinaryfilename = pf::helper::RemoveExtension(shaderbinaryfilename);
		...
		if (device != nullptr)
		{
			...
			if (it != wiShaderDump::shaderdump.end())
			{
				...
				pf::backlogger::postin("shader dump decompression failure: " + shaderbinaryfilename, LogLevel::Error);
			}
			else
			{
				pf::backlogger::postin("shader dump doesn't contain shader: " + shaderbinaryfilename, LogLevel::Error);
			}
#endif // SHADERDUMP_ENABLED
		}

		pf::shadercompiler::RegisterShader(shaderbinaryfilename);

		if (pf::shadercompiler::IsShaderOutdated(shaderbinaryfilename))
		{
			pf::shadercompiler::CompilerInput input;
			...
			pf::helper::MakePathAbsolute(sourcedir);
			input.include_directories.push_back(sourcedir);
			input.include_directories.push_back(sourcedir + pf::helper::GetDirectoryFromPath(filename));
			input.shadersourcefilename = pf::helper::ReplaceExtension(sourcedir + filename, "hlsl");

			pf::shadercompiler::CompilerOutput output;
			pf::shadercompiler::Compile(input, output);

			if (output.IsValid())
			{
				pf::shadercompiler::SaveShaderAndMetadata(shaderbinaryfilename, output);

				if (!output.error_message.empty())
				{
					pf::backlogger::postin(output.error_message, LogLevel::Warning);
				}
				pf::backlogger::postin("shader compiled: " + shaderbinaryfilename);
				return device->CreateShader(stage, output.shaderdata, output.shadersize, &shader);
			}
			else
			{
				pf::backlogger::postin("shader compile FAILED: " + shaderbinaryfilename + "\n" + output.error_message, LogLevel::Error);
		...
		if (device != nullptr)
		{
			pf::vector<uint8_t> buffer;
			if (pf::helper::FileRead(shaderbinaryfilename, buffer))
			{
	...
	const GPUBuffer& GetIndexBufferForQuads(uint32_t max_quad_count)
	{
		...
		if (required_max_index < 65536u)
		{
			...
			if (!indexBufferForQuads16.IsValid() || indexBufferForQuads16.desc.size / indexBufferForQuads16.desc.stride < required_index_count)
			{
				...
				device->SetName(&indexBufferForQuads16, "pf::renderer::indexBufferForQuads16bit");
			}
			return indexBufferForQuads16;
		}

		// 32-bit request below:
		max_quad_count = pf::math::GetNextPowerOfTwo(max_quad_count); // reduce allocations by making larger fitting allocations
		const size_t required_index_count = max_quad_count * 6u;
		...
		if (!indexBufferForQuads32.IsValid() || indexBufferForQuads32.desc.size / indexBufferForQuads32.desc.stride < required_index_count)
		{
			...
			device->SetName(&indexBufferForQuads32, "pf::renderer::indexBufferForQuads32bit");
		}
	...
	struct GPUSubAllocator
	{
		...
		struct Block
		{
			pf::allocator::PageAllocator allocator;
			GPUBuffer buffer;
		};
		pf::vector<Block> blocks;
		std::mutex locker;
	} static suballocator;
		...
		// scoped for locker
		{
			...
			for (auto& block : suballocator.blocks)
			{
				...
				if (allocation.allocation.IsValid())
				{
					...
					//p_log("SuballocateGPUBuffer allocated size: %s, pages: %d, free space remaining: %s", pf::helper::GetMemorySizeText(size).c_str(), block.allocator.page_count_from_bytes(size), pf::helper::GetMemorySizeText(allocation.allocation.allocator->allocator.storageReport().totalFreeSpace * block.allocator.page_size).c_str());
					return allocation;
				}
			}
			...
			p_log("SuballocateGPUBuffer created buffer block with size: %s, with page size: %s, page count: %d", pf::helper::GetMemorySizeText(block.allocator.total_size_in_bytes()).c_str(), pf::helper::GetMemorySizeText(block.allocator.page_size).c_str(), (int)block.allocator.page_count);
		}
	...
	void ReloadShaders()
	{
		...
		pf::eventhandler::FireEvent(pf::eventhandler::EVENT_RELOAD_SHADERS, 0);
	}
	...
	void Initialize()
	{
		...
		static pf::eventhandler::Handle handle2 = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
		LoadShaders();
		int ti = (int)std::round(timer.elapsed());
		p_log("pf::renderer Initialized (%d ms)", ti);
	...
	inline void CreateDirLightShadowCams(const LightComponent& light, CameraComponent camera, SHCAM* shcams, size_t shcam_count, const pf::rectpacker::Rect& shadow_rect, const Sphere* dedicated_shadows = nullptr, size_t dedicated_shadow_count = 0)
	{
	...
	void UpdateVisibility(Visibility& vis)
	{
		...
		pf::jobsystem::context ctx;
		auto range = pf::profiler::BeginRangeCPU("Frustum Culling");
		...
		if (vis.flags & Visibility::ALLOW_LIGHTS)
		{
			...
			pf::jobsystem::Dispatch(ctx, light_loop, groupSize, [&](pf::jobsystem::JobArgs args) {
		...
		if (vis.flags & Visibility::ALLOW_OBJECTS)
		{
			...
			pf::jobsystem::Dispatch(ctx, object_loop, groupSize, [&](pf::jobsystem::JobArgs args) {
				...
				if ((aabb.layerMask & vis.layerMask) && vis.frustum.CheckBoxFast(aabb))
				{
					...
					if ((vis.flags & Visibility::ALLOW_REQUEST_REFLECTION) && object.IsRequestPlanarReflection() && !occluded)
					{
						...
						float dist = pf::math::DistanceEstimated(vis.camera->Eye, object.center);
						vis.locker.lock();
		...
		if (vis.flags & Visibility::ALLOW_EMITTERS)
		{
			pf::jobsystem::Execute(ctx, [&](pf::jobsystem::JobArgs args) {
				...
				{
					const pf::EmittedParticleSystem& emitter = vis.scene->emitters[i];
		...
		if (vis.flags & Visibility::ALLOW_HAIRS)
		{
			pf::jobsystem::Execute(ctx, [&](pf::jobsystem::JobArgs args) {
				...
				{
					const pf::HairParticleSystem& hair = vis.scene->hairs[i];
					...
					{
						const float dist = pf::math::Distance(vis.camera->Eye, hair.aabb.getCenter());
		...
		if (vis.flags & Visibility::ALLOW_COLLIDERS)
		{
			pf::jobsystem::Execute(ctx, [&](pf::jobsystem::JobArgs args) {
				for (size_t i = 0; i < vis.scene->collider_count_gpu; ++i)
				{
					...
					collider.dist = pf::math::DistanceSquared(vis.scene->aabb_colliders_gpu[i].getCenter(), vis.camera->Eye);
				...
				for (size_t i = 0; i < vis.scene->humanoids.GetCount(); ++i)
				{
					...
					for (auto& bodypart : humanoid.ragdoll_bodyparts)
					{
						...
						collider.dist = pf::math::DistanceSquared(sphere.center, vis.camera->Eye);
						vis.visibleColliders.push_back(collider);
					}
				}
		...
		{
			auto range = pf::profiler::BeginRangeCPU("Shadowmap packing");
			...
			while (iterative_scaling > 0.03f)
			{
				...
				for (uint32_t lightIndex : vis.visibleLights)
				{
					...
					const float dist = pf::math::Distance(vis.camera->Eye, light.position);
				...
				if (!vis.shadow_packer.rects.empty())
				{
					if (vis.shadow_packer.pack(8192))
					{
						for (auto& rect : vis.shadow_packer.rects)
						{
							...
							pf::rectpacker::Rect& lightrect = vis.visibleLightShadowRects[lightIndex];
							const LightComponent& light = vis.scene->lights[lightIndex];
							if (rect.was_packed)
							{
								lightrect = rect;
		...
		pf::profiler::EndRange(range); // Frustum Culling
	}
	...
	)
	{
		...
		frameCB.giboost_packed = pf::math::pack_half2(GetGIBoost(), 0);
		...
		frameCB.texture_random64x64_index = device->GetDescriptorIndex(pf::texturehelper::getRandom64x64(), SubresourceType::SRV);
		frameCB.texture_bluenoise_index = device->GetDescriptorIndex(pf::texturehelper::getBlueNoise(), SubresourceType::SRV);
		...
		const Texture& shadowMap = shadowMapAtlas.IsValid() ? shadowMapAtlas : *pf::texturehelper::getBlack();
		const Texture& shadowMapTransparent = shadowMapAtlas_Transparent.IsValid() ? shadowMapAtlas_Transparent : *pf::texturehelper::getWhite();
		...
		frameCB.entity_culling_count = 0;
		{
			...
			for (uint32_t lightIndex : vis.visibleLights)
			{
				...
				const pf::rectpacker::Rect& shadow_rect = vis.visibleLightShadowRects[lightIndex];
	...
	)
	{
		...
		auto prof_updatebuffer_cpu = pf::profiler::BeginRangeCPU("Update Buffers (CPU)");
		auto prof_updatebuffer_gpu = pf::profiler::BeginRangeGPU("Update Buffers (GPU)", cmd);
		...
		pf::profiler::EndRange(prof_updatebuffer_cpu);
		pf::profiler::EndRange(prof_updatebuffer_gpu);

		BindCommonResources(cmd);

		{
			auto range = pf::profiler::BeginRangeGPU("Wind", cmd);
		...
		{
			auto range = pf::profiler::BeginRangeGPU("Caustics", cmd);
...

		{
			...
			auto range = pf::profiler::BeginRangeGPU("Skinning and Morph", cmd);
			...
			pf::profiler::EndRange(range); // Skinning and Morph
		...
		static thread_local pf::vector<HairParticleSystem::UpdateGPUItem> hair_updates;
		if (!vis.visibleHairs.empty())
		{
			auto range = pf::profiler::BeginRangeGPU("HairParticles - Simulate", cmd);
			for (uint32_t hairIndex : vis.visibleHairs)
			{
				const pf::HairParticleSystem& hair = vis.scene->hairs[hairIndex];
		...
		if (vis.scene->impostors.GetCount() > 0 && vis.scene->objects.GetCount() > 0 && vis.scene->impostorBuffer.IsValid())
		{
			...
			auto range = pf::profiler::BeginRangeGPU("Impostor prepare", cmd);
		...
		if (vis.scene->instanceArraySize > 0 && vis.scene->meshletBuffer.IsValid())
		{
			...
			auto range = pf::profiler::BeginRangeGPU("Meshlet prepare", cmd);
	...
	)
	{
		...
		if (vis.scene->weather.IsRealisticSky())
		{
			pf::renderer::ComputeSkyAtmosphereTextures(cmd);
			pf::renderer::ComputeSkyAtmosphereSkyViewLut(cmd);
			if (vis.scene->weather.IsRealisticSkyAerialPerspective())
			{
				pf::renderer::ComputeSkyAtmosphereCameraVolumeLut(cmd);
			}
		}
		...
		{
			auto range = pf::profiler::BeginRangeGPU("EmittedParticles - Simulate", cmd);
			for (uint32_t emitterIndex : vis.visibleEmitters)
			{
				const pf::EmittedParticleSystem& emitter = vis.scene->emitters[emitterIndex];
	...
	)
	{
		if (!vis.scene->weather.IsOceanEnabled() || !vis.scene->ocean.IsValid())
			return;
		bool occluded = false;
		if (vis.flags & pf::renderer::Visibility::ALLOW_OCCLUSION_CULLING)
		{
			occluded = vis.scene->ocean.IsOccluded();
		}
		if (!occluded)
		{
			auto range = pf::profiler::BeginRangeGPU("Ocean - Simulate", cmd);
	...
	void UpdateRaytracingAccelerationStructures(const Scene& scene, CommandList cmd)
	{
		if (device->CheckCapability(GraphicsDeviceCapability::RAYTRACING))
		{
			...
			// BLAS:
			{
				auto rangeCPU = pf::profiler::BeginRangeCPU("BLAS Update (CPU)");
				auto range = pf::profiler::BeginRangeGPU("BLAS Update (GPU)", cmd);
				...
				{
					const pf::HairParticleSystem& hair = scene.hairs[i];
				...
				{
					const pf::EmittedParticleSystem& emitter = scene.emitters[i];
				...
				device->EventEnd(cmd);
				pf::profiler::EndRange(range);
				pf::profiler::EndRange(rangeCPU);
			}

			// TLAS:
			{
				auto rangeCPU = pf::profiler::BeginRangeCPU("TLAS Update (CPU)");
				auto range = pf::profiler::BeginRangeGPU("TLAS Update (GPU)", cmd);
	...
	void OcclusionCulling_Render(const CameraComponent& camera, const Visibility& vis, CommandList cmd)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Occlusion Culling Render", cmd);
		...
		pf::profiler::EndRange(range); // Occlusion Culling Render
	}
	...
	)
	{
		auto range = distortion ?
			pf::profiler::BeginRangeGPU("EmittedParticles - Render (Distortion)", cmd) :
			pf::profiler::BeginRangeGPU("EmittedParticles - Render", cmd);
		...
		if (emitterCount > 0)
		{
			...
			static thread_local pf::vector<uint32_t> emitterSortingHashes;
			emitterSortingHashes.resize(emitterCount);
			for (size_t i = 0; i < emitterCount; ++i)
			{
				const uint32_t emitterIndex = vis.visibleEmitters[i];
				const pf::EmittedParticleSystem& emitter = vis.scene->emitters[emitterIndex];
				float distance = pf::math::DistanceEstimated(emitter.center, vis.camera->Eye);
			...
			for (size_t i = 0; i < emitterCount; ++i)
			{
				...
				if (distortion && emitter.shaderType == pf::EmittedParticleSystem::SOFT_DISTORTION)
				{
					emitter.Draw(*material, cmd);
				}
				else if (!distortion && (emitter.shaderType == pf::EmittedParticleSystem::SOFT || emitter.shaderType == pf::EmittedParticleSystem::SOFT_LIGHTING || emitter.shaderType == pf::EmittedParticleSystem::SIMPLE || IsWireRender()))
				{
					emitter.Draw(*material, cmd);
				}
			}
	...
	)
	{
		...
		static thread_local pf::vector<uint64_t> distance_sorter;
		distance_sorter.clear();
		for (size_t i = 0; i < scene.sprites.GetCount(); ++i)
		{
			const pf::Sprite& sprite = scene.sprites[i];
			if (sprite.IsHidden())
				continue;
			...
			{
				sorter.bits.distance = XMConvertFloatToHalf(pf::math::DistanceEstimated(transform->GetPosition(), camera.Eye));
			}
		...
		if (!distortion)
		{
			...
			{
				const pf::SpriteFont& font = scene.fonts[i];
				if (font.IsHidden())
					continue;
				...
				if (font.IsCameraScaling())
				{
					if (transform != nullptr)
					{
						scale *= 0.05f * pf::math::Distance(transform->GetPosition(), camera.Eye);
					}
				}
				aabb = aabb * scale;
		...
		for (auto& x : distance_sorter)
		{
			...
			case SPRITE:
			{
				const pf::Sprite& sprite = scene.sprites[sorter.bits.id];
				pf::image::Params params = sprite.params;
				...
				if (transform != nullptr)
				{
					...
					{
						float scale = 0.05f * pf::math::Distance(transform->GetPosition(), camera.Eye);
				...
				pf::image::Draw(sprite.GetTexture(), params, cmd);
			}
			break;
			case FONT:
			{
				const pf::SpriteFont& font = scene.fonts[sorter.bits.id];
				pf::font::Params params = font.params;
				...
				params.customProjection = &M;
				pf::font::Draw(font.GetText().c_str(), font.GetCurrentTextLength(), params, cmd);
			}
			break;
			}
		}
			...
			for (int type = LightComponent::POINT; type < LightComponent::LIGHTTYPE_COUNT; ++type)
			{
				...
				for (uint32_t lightIndex : vis.visibleLights)
				{
					...
					if (light.GetType() == type && light.IsVisualizerEnabled())
					{
						...
						{
							const float sphere_volume = std::max(1.0f, pf::math::SphereVolume(light.radius));
		...
		for (int type = 0; type < LightComponent::LIGHTTYPE_COUNT; ++type)
		{
			...
			for (size_t i = 0; i < vis.visibleLights.size(); ++i)
			{
				...
				case LightComponent::SPOT:
				{
					...
					uint packed = pf::math::pack_half2(std::pow(std::sin(light.outerConeAngle), 2.0f), std::pow(std::cos(light.outerConeAngle), 2.0f));
		...
		for (uint32_t lightIndex : vis.visibleLights)
		{
			...
			if (!light.lensFlareRimTextures.empty())
			{
				...
				if (light.GetType() == LightComponent::DIRECTIONAL)
				{
					...
					{
						device->BindResource(pf::texturehelper::getWhite(), 0, cmd);
					}
					else
					{
	...
	)
	{
		...
		auto range_cpu = pf::profiler::BeginRangeCPU("Shadowmap Rendering");
		auto range_gpu = pf::profiler::BeginRangeGPU("Shadowmap Rendering", cmd);
			...
			case LightComponent::DIRECTIONAL:
			{
				...
				for (size_t i = 0; i < vis.scene->aabb_objects.size(); ++i)
				{
					...
					if (aabb.layerMask & vis.layerMask)
					{
						...
						{
							const float distanceSq = pf::math::DistanceSquared(EYE, object.center);
	...
	)
	{
		...
		if (opaque || transparent)
		{
			...
			for (uint32_t instanceIndex : vis.visibleObjects)
			{
				...
				const float distance = pf::math::Distance(vis.camera->Eye, object.center);
	...
	)
	{
		...
		{
			const XMFLOAT4 colliderColor = pf::Color(88, 218, 83, 255);
		...
		{
			const XMFLOAT4 springDebugColor = pf::Color(255, 70, 165, 255);
			...
			)
		{
			...
			auto draw_triangle = [&](pf::vector<RenderableTriangle>& dbg) {
				if (dbg.empty())
					return;
				MiscCB sb;
				XMStoreFloat4x4(&sb.g_xTransform, camera.GetViewProjection());
		...
		if (!renderableLines.empty() || !renderableLines_depth.empty())
		{
			...
			auto draw_line = [&](pf::vector<RenderableLine>& dbg) {
				if (dbg.empty())
					return;
				MiscCB sb;
				XMStoreFloat4x4(&sb.g_xTransform, camera.GetViewProjection());
		...
		if (!renderablePoints.empty() || !renderablePoints_depth.empty())
		{
			...
			auto draw_point = [&](pf::vector<RenderablePoint>& dbg) {
				if (dbg.empty())
					return;
				MiscCB sb;
		...
		if (!renderableBoxes.empty() || !renderableBoxes_depth.empty())
		{
			...
			auto draw_box = [&](pf::vector<std::pair<XMFLOAT4X4, XMFLOAT4>>& dbg) {
				if (dbg.empty())
					return;
		...
		if (!renderableSpheres.empty() || !renderableSpheres_depth.empty())
		{
			...
			auto draw_sphere = [&](pf::vector<std::pair<Sphere, XMFLOAT4>>& dbg) {
				if (dbg.empty())
					return;
		...
		if (!renderableCapsules.empty() || !renderableCapsules_depth.empty())
		{
			...
			auto draw_capsule = [&](pf::vector<std::pair<Capsule, XMFLOAT4>>& dbg) {
				if (dbg.empty())
					return;
				MiscCB sb;
				XMStoreFloat4x4(&sb.g_xTransform, camera.GetViewProjection());
		...
		if (!paintrads.empty())
		{
			...
			for (auto& x : paintrads)
			{
				...
				push.wrapSamplerIndex = device->GetDescriptorIndex(pf::renderer::GetSampler(pf::enums::SAMPLER_OBJECTSHADER));
				push.clampSamplerIndex = device->GetDescriptorIndex(pf::renderer::GetSampler(pf::enums::SAMPLER_OBJECTSHADER_CLAMP));
				device->PushConstants(&push, sizeof(push), cmd);
		...
		if (!debugTextStorage.empty())
		{
			...
			static thread_local pf::vector<DebugTextSorter> sorted_texts;
			sorted_texts.clear();
			size_t offset = 0;
			while (offset < debugTextStorage.size())
			{
				...
				offset += x.text_len + 1;
				x.distance = pf::math::Distance(x.params.position, camera.Eye);
			...
			for (auto& x : sorted_texts)
			{
				...
				params.color = pf::Color::fromFloat4(x.params.color);
				...
				params.shadowColor = pf::Color::Black();
				...
				pf::font::Draw(x.text, x.text_len, params, cmd);
			}
		...
		const Texture* weatherMapSecond)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Volumetric Clouds Shadow", cmd);
	...
	void ComputeSkyAtmosphereTextures(CommandList cmd)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("SkyAtmosphere Textures", cmd);
	...
	void RefreshEnvProbes(const Visibility& vis, CommandList cmd)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Environment Probe Refresh", cmd);
		...
		auto render_probe = [&](const EnvironmentProbeComponent& probe, const AABB& probe_aabb) {
			...
			static pf::unordered_map<uint32_t, Texture> render_textures;
			static std::mutex locker;
			{
				...
				if (!envrenderingDepthBuffer.IsValid())
				{
					...
					info += "\n\tMemory = " + pf::helper::GetMemorySizeText(ComputeTextureMemorySizeInBytes(desc)) + "\n";
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("VXGI - Voxelize", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("VXGI - Resolve", cmd);
	...
	{
		auto range = pf::profiler::BeginRangeGPU("Entity Culling", cmd);
	...
	)
	{
		...
		if (pf::jobsystem::IsBusy(raytracing_ctx))
			return;

		device->EventBegin("RayTraceScene", cmd);
		auto range = pf::profiler::BeginRangeGPU("RayTraceScene", cmd);

		const TextureDesc& desc = output.GetDesc();

		BindCommonResources(cmd);

		const XMFLOAT4& halton = pf::math::GetHaltonSequence(accumulation_sample);
		RaytracingCB cb = {};
	...
	void RefreshLightmaps(const Scene& scene, CommandList cmd)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Lightmap Processing", cmd);
		...
		{
			TextureDesc desc;
			desc.width = pf::math::GetNextPowerOfTwo(max_width);
			desc.height = pf::math::GetNextPowerOfTwo(max_height);
			desc.format = Format::R16G16B16A16_FLOAT;
		...
		for (uint32_t requestIndex = 0; requestIndex < lightmap_request_count; ++requestIndex)
		{
			...
			if (object.IsLightmapRenderRequested())
			{
				...
				cb.xTraceUserData.x = raytraceBounceCount;
				XMFLOAT4 halton = pf::math::GetHaltonSequence(object.lightmapIterationCount); // for jittering the rasterization (good for eliminating atlas border artifacts)
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Luminance", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Bloom", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("ComputeShadingRateClassification", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Visibility_Prepare", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Visibility_Surface", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Visibility_Surface_Reduced", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Visibility_Shade", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Visibility_Velocity", cmd);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("SurfelGI - Coverage", cmd);
		...
		pf::profiler::EndRange(prof_range);
		device->EventEnd(cmd);
	}
	void SurfelGI(
		const SurfelGIResources& res,
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("SurfelGI", cmd);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("DDGI", cmd);
		...
		// Raytracing:
		{
			...
			MiscCB cb = {};
			float angle = pf::random::GetRandom(0.0f, 1.0f) * XM_2PI;
			XMVECTOR axis = XMVectorSet(
				pf::random::GetRandom(-1.0f, 1.0f),
				pf::random::GetRandom(-1.0f, 1.0f),
				pf::random::GetRandom(-1.0f, 1.0f),
				0
			);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("SSAO", cmd);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("HBAO", cmd);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("MSAO", cmd);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("RTAO", cmd);
	...
	)
	{
		...
		auto profilerRange = pf::profiler::BeginRangeGPU("RTDiffuse", cmd);
		...
		pf::profiler::EndRange(profilerRange);
		device->EventEnd(cmd);
	}
	...
	)
	{
		...
		auto profilerRange = pf::profiler::BeginRangeGPU("SSGI", cmd);
	...
	)
	{
		...
		auto profilerRange = pf::profiler::BeginRangeGPU("RTReflection", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("SSR", cmd);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("RTShadow", cmd);
	...
	)
	{
		...
		auto prof_range = pf::profiler::BeginRangeGPU("ScreenSpaceShadow", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("LightShafts", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Depth of Field", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Outline", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("MotionBlur", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Aerial Perspective", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Volumetric Clouds", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Volumetric Clouds Upsample", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("FXAA", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Temporal AA Resolve", cmd);
	...
	{
#define A_CPU
#include "Shader/ffx-fsr/ffx_a.h"
#include "Shader/ffx-fsr/ffx_fsr1.h"
	}
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Postprocess_FSR", cmd);
	...
	namespace fsr2
	{
#include "Shader/ffx-fsr2/ffx_core.h"
#include "Shader/ffx-fsr2/ffx_fsr1.h"
#include "Shader/ffx-fsr2/ffx_spd.h"
#include "Shader/ffx-fsr2/ffx_fsr2_callbacks_hlsl.h"
#include "Shader/ffx-fsr2/ffx_fsr2_common.h"
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Postprocess_FSR2", cmd);
		...
		if (resetAccumulation)
		{
			...
			uint32_t clear_lock_pk = pf::math::Pack_R11G11B10_FLOAT(XMFLOAT3(clearValuesLockStatus[0], clearValuesLockStatus[1], clearValuesLockStatus[2]));
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Underwater", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Mesh Blend - Edge procesing (Async compute)", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU("Mesh Blend - Resolve (Gfx)", cmd);
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeGPU(debug_name, cmd);
	...
	Ray GetPickRay(long cursorX, long cursorY, const pf::Canvas& canvas, const CameraComponent& camera)
	{
	...
	void DrawBox(const pf::primitive::AABB& aabb, const XMFLOAT4& color, bool depth)
	{
	...
	pf::Resource CreatePaintableTexture(uint32_t width, uint32_t height, uint32_t mips, pf::Color initialColor)
	{
		TextureDesc desc;
		desc.width = width;
		desc.height = height;
		...
		pf::vector<pf::Color> data(desc.width * desc.height);
