...
#include <Shader/ShaderInterop_Image.h>
...
namespace pf::image {
	...
	void Draw(const Texture* texture, const Params& params, CommandList cmd)
	{
		...
		else
		{
			...
			// vertex buffer:
			XMMATRIX S = DirectX::XMMatrixScaling(params.scale.x * params.siz.x, params.scale.y * params.siz.y, 1);
			XMMATRIX M = DirectX::XMMatrixRotationZ(params.rotation);
			...
			{
				S = DirectX::XMMatrixScaling(1, -1, 1) * S; // reason: screen projection is Y down (like UV-space) and that is the common case for image rendering. But custom projections will use the "world space"
		...
		if (params.gradient != Params::Gradient::None)
		{
			...
			image.gradient_color = math::pack_half4(params.gradient_color);
			image.gradient_uv_start = math::pack_half2(params.gradient_uv_start);
			image.gradient_uv_end = math::pack_half2(params.gradient_uv_end);
		}
		...
		{
			stencilRef = renderer::CombineStencilrefs(STENCILREF_EMPTY, (uint8_t)stencilRef);
		}
	...
	void LoadShaders()
	{
		renderer::LoadShader(ShaderStage::VS, vertexShader, "imageVS.cso");
		renderer::LoadShader(ShaderStage::PS, pixelShader, "imagePS.cso");
