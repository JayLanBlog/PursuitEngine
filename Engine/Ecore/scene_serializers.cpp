...
namespace pf::scene
{
	...
	{
		void Serialize(pf::Archive& archive, pf::ecs::EntitySerializer& seri) { /*this never serialized any data*/ }
	};

	void NameComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
	...
	void LayerComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> layerMask;
	...
	void TransformComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void HierarchyComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
	...
	void MaterialComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		...
		if (archive.IsReadMode())
		{
			...
			pf::jobsystem::Execute(seri.ctx, [&](pf::jobsystem::JobArgs args) {
				CreateRenderData();
				});
		}
		else
		{
			...
			archive << pf::helper::GetPathRelative(dir, textures[BASECOLORMAP].name);
			archive << pf::helper::GetPathRelative(dir, textures[SURFACEMAP].name);
			archive << pf::helper::GetPathRelative(dir, textures[NORMALMAP].name);
			archive << pf::helper::GetPathRelative(dir, textures[DISPLACEMENTMAP].name);

			if (archive.GetVersion() >= 24)
			{
				archive << pf::helper::GetPathRelative(dir, textures[EMISSIVEMAP].name);
			}

			if (archive.GetVersion() >= 28)
			{
				archive << pf::helper::GetPathRelative(dir, textures[OCCLUSIONMAP].name);
			...
			{
				archive << transmission;
				archive << pf::helper::GetPathRelative(dir, textures[TRANSMISSIONMAP].name);
				archive << textures[TRANSMISSIONMAP].uvset;
			}
			...
			if (archive.GetVersion() >= 61)
			{
				archive << sheenColor;
				archive << sheenRoughness;
				archive << pf::helper::GetPathRelative(dir, textures[SHEENCOLORMAP].name);
				archive << pf::helper::GetPathRelative(dir, textures[SHEENROUGHNESSMAP].name);
				archive << textures[SHEENCOLORMAP].uvset;
				archive << textures[SHEENROUGHNESSMAP].uvset;

				archive << clearcoat;
				archive << clearcoatRoughness;
				archive << pf::helper::GetPathRelative(dir, textures[CLEARCOATMAP].name);
				archive << pf::helper::GetPathRelative(dir, textures[CLEARCOATROUGHNESSMAP].name);
				archive << pf::helper::GetPathRelative(dir, textures[CLEARCOATNORMALMAP].name);
				archive << textures[CLEARCOATMAP].uvset;
				archive << textures[CLEARCOATROUGHNESSMAP].uvset;
				archive << textures[CLEARCOATNORMALMAP].uvset;
			}
			...
			if (archive.GetVersion() >= 68)
			{
				archive << pf::helper::GetPathRelative(dir, textures[SPECULARMAP].name);
				archive << textures[SPECULARMAP].uvset;
			}
			...
			if (seri.GetVersion() >= 2)
			{
				archive << anisotropy_strength;
				archive << anisotropy_rotation;
				archive << pf::helper::GetPathRelative(dir, textures[ANISOTROPYMAP].name);
				archive << textures[ANISOTROPYMAP].uvset;
			}

			if (seri.GetVersion() >= 3)
			{
				archive << pf::helper::GetPathRelative(dir, textures[TRANSPARENCYMAP].name);
				archive << textures[TRANSPARENCYMAP].uvset;
			}
			if (seri.GetVersion() >= 4)
			{
	...
	void MeshComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ImpostorComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ObjectComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void RigidBodyPhysicsComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void PhysicsConstraintComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void SoftBodyPhysicsComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		pf::vector<uint32_t> graphicsToPhysicsVertexMapping;
		...
		if (archive.IsReadMode())
		{
			...
			else
			{
				...
				pf::vector<float> weights2(graphicsToPhysicsVertexMapping.size());
	...
	void ArmatureComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void LightComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
			...
			pf::jobsystem::Execute(seri.ctx, [&](pf::jobsystem::JobArgs args) {
				...
				for (size_t i = 0; i < lensFlareNames.size(); ++i)
				{
					if (!lensFlareNames[i].empty())
					{
						...
						lensFlareRimTextures[i] = pf::resourcemanager::Load(lensFlareNames[i]);
					}
				}
				});
		}
		else
		{
			...
			pf::vector<std::string> lensFlareNamesRelative = lensFlareNames;
			if (!dir.empty())
			{
				for (size_t i = 0; i < lensFlareNamesRelative.size(); ++i)
				{
					pf::helper::MakePathRelative(dir, lensFlareNamesRelative[i]);
				}
			}
	...
	void CameraComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void EnvironmentProbeComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		else
		{
			...
			{
				archive << resolution;
				archive << pf::helper::GetPathRelative(dir, textureName);
			}
		}
	}
	void ForceFieldComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void DecalComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void AnimationComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void AnimationDataComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void WeatherComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		...
		/*	//TO DO:
			...
			if (archive.GetVersion() >= 32)
			{
				...
				if (!skyMapName.empty())
				{
					...
					skyMap = pf::resourcemanager::Load(skyMapName);
				}
			}
			...
			if (archive.GetVersion() >= 62)
			{
				...
				if (!colorGradingMapName.empty())
				{
					...
					colorGradingMap = pf::resourcemanager::Load(colorGradingMapName, pf::resourcemanager::Flags::IMPORT_COLORGRADINGLUT);
				}
			}
			...
			if (archive.GetVersion() >= 86)
			{
				...
				if (!volumetricCloudsWeatherMapFirstName.empty())
				{
					...
					volumetricCloudsWeatherMapFirst = pf::resourcemanager::Load(volumetricCloudsWeatherMapFirstName);
				}
			}
			...
			if (archive.GetVersion() >= 88)
			{
				...
				if (!volumetricCloudsWeatherMapSecondName.empty())
				{
					...
					volumetricCloudsWeatherMapSecond = pf::resourcemanager::Load(volumetricCloudsWeatherMapSecondName);
				}
		...
		/*	TO DO:
			...
			if (archive.GetVersion() >= 32)
			{
				archive << pf::helper::GetPathRelative(dir, skyMapName);
			}
			if (archive.GetVersion() >= 40)
			{
				archive << windSpeed;
			...
			if (archive.GetVersion() >= 62)
			{
				archive << pf::helper::GetPathRelative(dir, colorGradingMapName);
			}
			...
			if (archive.GetVersion() >= 86)
			{
				archive << pf::helper::GetPathRelative(dir, volumetricCloudsWeatherMapFirstName);
			}

			if (archive.GetVersion() >= 88)
			{
				archive << pf::helper::GetPathRelative(dir, volumetricCloudsWeatherMapSecondName);
	...
	void SoundComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		if (archive.IsReadMode())
		{
			...
			pf::jobsystem::Execute(seri.ctx, [&](pf::jobsystem::JobArgs args) {
				if (!filename.empty())
				{
					...
					soundResource = pf::resourcemanager::Load(filename);
		...
		else
		{
			...
			archive << _flags;
			archive << pf::helper::GetPathRelative(dir, filename);
			archive << volume;
	...
	void VideoComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
		...
		if (archive.IsReadMode())
		{
			...
			pf::jobsystem::Execute(seri.ctx, [&](pf::jobsystem::JobArgs args) {
				...
				{
					filename = dir + filename;
					videoResource = pf::resourcemanager::Load(filename);
	...
	void InverseKinematicsComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
	...
	void SpringComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ColliderComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void ScriptComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		const std::string& dir = archive.GetSourceDirectory();
	...
	void ExpressionComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void HumanoidComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
	...
	void MetadataComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void CharacterComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void SplineComponent::Serialize(pf::Archive& archive, EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
	...
	void Scene::Serialize(pf::Archive& archive)
	{
		pf::Timer timer;
		...
		pf::resourcemanager::ResourceSerializer resource_seri;
		if (archive.IsReadMode() && archive.GetVersion() >= 63)
		{
			//pf::resourcemanager::Serialize_READ(archive, resource_seri);
			if (archive.GetVersion() >= 90)
			{
		...
		seri.ctx.priority = pf::jobsystem::Priority::Low; // serialization tasks will be low priority to not block rendering if scene loading is asynchronous
		...
		else
		{
			...
			ComponentManager<pf::primitive::AABB> aabbs_tmp; // no longer needed from serializer
	...
	void Scene::DDGI::Serialize(pf::Archive& archive)
	{
		...
		if (archive.IsReadMode())
		{
			...
			if (archive.GetVersion() < 93)
			{
				...
				if (!data.empty())
				{
					pf::backlogger::postin("Found older DDGI data in the scene which is now incompatible. Please recompute the DDGI if you need it.", LogLevel::Warning);
					depth_texture = {};
					probe_buffer = {};
				}
			}
		...
		else
		{
			...
			{
			//	pf::helper::saveBufferToMemory(probe_buffer, data);
			}
			archive << data;
			...
			{
				bool success = pf::helper::saveTextureToMemory(depth_texture, data);
				assert(success);
			}
			archive << data;
		}
		...
		else
		{
			...
			if (archive.IsReadMode())
			{
				...
				}
				{
					...
					if (component_exists)
					{
						auto component = pf::primitive::AABB(); // no longer needed to be serialized
						component.Serialize(archive, seri);
					}
				}
				{
