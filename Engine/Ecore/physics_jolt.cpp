...
namespace pf::physics {
	...
	namespace jolt
	{
		...
		struct RigidBody
		{
			...
			XMFLOAT4X4 parentMatrix = pf::math::IDENTITY_MATRIX;
			XMFLOAT4X4 parentMatrixInverse = pf::math::IDENTITY_MATRIX;
		...
		struct SoftBody
		{
			...
			pf::vector<Neighbors> physicsNeighbors;
		...
		RigidBody& GetRigidBody(pf::scene::RigidBodyPhysicsComponent& physicscomponent)
		{
		...
		const RigidBody& GetRigidBody(const pf::scene::RigidBodyPhysicsComponent& physicscomponent)
		{
			return *(RigidBody*)physicscomponent.physicsobject.get();
		}
		SoftBody& GetSoftBody(pf::scene::SoftBodyPhysicsComponent& physicscomponent)
		{
			if (physicscomponent.physicsobject == nullptr)
			{
		...
		const SoftBody& GetSoftBody(const pf::scene::SoftBodyPhysicsComponent& physicscomponent)
		{
			return *(SoftBody*)physicscomponent.physicsobject.get();
		}
		Constraint& GetConstraint(pf::scene::PhysicsConstraintComponent& physicscomponent)
		{
			if (physicscomponent.physicsobject == nullptr)
			{
		...
		const Constraint& GetConstraint(const pf::scene::PhysicsConstraintComponent& physicscomponent)
		{
			return *(Constraint*)physicscomponent.physicsobject.get();
		}
		...
		void AddRigidBody(
			...
			const pf::scene::TransformComponent& _transform,
		...
		)
		{
			...
			if (physicsobject.shape != nullptr)
			{
				...
				{
					pf::backlogger::postin("AddRigidBody failed: body couldn't be created! This could mean that there are too many physics objects.", LogLevel::Error);
					return;
				}
		...
		)
		{
			...
			{
				pf::backlogger::postin("AddSoftBody failed: physics faces are empty, this means generating physics mesh has failed, try to change settings.", LogLevel::Error);
				return;
			}
			...
			{
				pf::backlogger::postin("AddSoftBody failed: body couldn't be created! This could mean that there are too many physics objects.", LogLevel::Error);
				return;
			}
		}
		void AddConstraint(
			pf::scene::Scene& scene,
			Entity entity,
			pf::scene::PhysicsConstraintComponent& physicscomponent,
		...
		)
		{
			...
			else
			{
				...
				if (!lock.Succeeded())
				{
					p_log_error("AddConstraint error: bodyA lock did not succeed!");
					return;
				}
			...
			else
			{
				...
				if (!lock.Succeeded())
				{
					p_log_error("AddConstraint error: bodyB lock did not succeed!");
					return;
				}
				Body& body = lock.GetBody();
				body2 = &body;
			...
			{
				p_log("Constraint creation failed: constraint type is not valid!");
				return;
			}

			if (physicsobject.constraint == nullptr)
			{
				p_log("Constraint creation failed: constraint is not valid!");
				return;
			}
		...
		struct Ragdoll
		{
			...
			Ragdoll(Scene& scene, HumanoidComponent& humanoid, Entity humanoidEntity, float scale)
			{
				...
				for (int c = 0; c < BODYPART_COUNT; ++c)
				{
					...
					if (entityA == INVALID_ENTITY)
					{
						p_log_warning("Ragdoll creation aborted because of a missing body part: %d", c);
						return;
					}
					...
					case BODYPART_RIGHT_UPPER_ARM:
					case BODYPART_RIGHT_LOWER_ARM:
						physicsobject.capsule = pf::primitive::Capsule(XMFLOAT3(-capsule_height * 0.5f - capsule_radius, 0, 0), XMFLOAT3(capsule_height * 0.5f + capsule_radius, 0, 0), capsule_radius);
						rtshape_settings.mRotation = Quat::sRotation(Vec3::sAxisZ(), 0.5f * JPH_PI).Normalized();
						break;
					case BODYPART_LEFT_FOOT:
					case BODYPART_RIGHT_FOOT:
						physicsobject.capsule = pf::primitive::Capsule(XMFLOAT3(0, 0, -capsule_height * 0.5f - capsule_radius), XMFLOAT3(0, 0, capsule_height * 0.5f + capsule_radius), capsule_radius);
						break;
					default:
						physicsobject.capsule = pf::primitive::Capsule(XMFLOAT3(0, -capsule_height * 0.5f - capsule_radius, 0), XMFLOAT3(0, capsule_height * 0.5f + capsule_radius, 0), capsule_radius);
						break;
					}
	...
	void Initialize()
	{
		...
		p_log("pf::physics Initialized [Jolt Physics %d.%d.%d] (%d ms)", JPH_VERSION_MAJOR, JPH_VERSION_MINOR, JPH_VERSION_PATCH, (int)std::round(timer.elapsed()));
	}
	...
	)
	{
		...
		case RigidBodyPhysicsComponent::CollisionShape::CONVEX_HULL:
			...
			{
				pf::backlogger::postin("CreateRigidBodyShape failed: convex hull physics requested, but no MeshComponent provided!", LogLevel::Error);
				return;
			}
			break;
		...
		case RigidBodyPhysicsComponent::CollisionShape::TRIANGLE_MESH:
			...
			{
				pf::backlogger::postin("CreateRigidBodyShape failed: triangle mesh physics requested, but no MeshComponent provided!", LogLevel::Error);
				return;
			}
			break;
		...
		case RigidBodyPhysicsComponent::CollisionShape::HEIGHTFIELD:
			if (mesh != nullptr)
			{
				...
				p_log_assert(dim >= 2, "Height field shape dimension must be at least 2!");
			...
			{
				pf::backlogger::postin("CreateRigidBodyShape failed: height field physics requested, but no MeshComponent provided!", LogLevel::Error);
				return;
			}
			break;
		...
		{
			p_log_error("CreateRigidBodyShape failed, shape_result: %s", shape_result.GetError().c_str());
			return;
		}
	...
	)
	{
		...
		auto range = pf::profiler::BeginRangeCPU("Physics");
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.rigidbodies.GetCount(), dispatchGroupSize, [&scene](pf::jobsystem::JobArgs args) {
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.softbodies.GetCount(), 1, [&scene](pf::jobsystem::JobArgs args) {
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.humanoids.GetCount(), 1, [&scene](pf::jobsystem::JobArgs args) {
			...
			if (humanoid.ragdoll != nullptr)
			{
				...
				if (!pf::math::float_equal(ragdoll.scale, scale))
				{
		...
		pf::jobsystem::Wait(ctx); // wait for rigidbody creations
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.constraints.GetCount(), dispatchGroupSize, [&scene, &physics_scene](pf::jobsystem::JobArgs args) {
		...
		pf::jobsystem::Wait(ctx); // wait for all creations

		// Now do the property updating
		//	These will be non-locking updates and perfromed potentially every frame
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.rigidbodies.GetCount(), dispatchGroupSize, [&scene, &physics_scene](pf::jobsystem::JobArgs args) {
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.softbodies.GetCount(), 1, [&scene, &physics_scene](pf::jobsystem::JobArgs args) {
			...
			for (size_t ind = 0; ind < physicscomponent.physicsToGraphicsVertexMapping.size(); ++ind)
			{
				...
				{
					XMFLOAT3 position = mesh->vertex_positions[graphicsInd];
					XMVECTOR P = armature == nullptr ? XMLoadFloat3(&position) : pf::scene::SkinVertex(*mesh, *armature, graphicsInd);
					P = XMVector3Transform(P, worldMatrix);
					XMStoreFloat3(&position, P);
		...
		// Ragdoll management:
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.humanoids.GetCount(), 1, [&scene, &physics_scene](pf::jobsystem::JobArgs args) {
			HumanoidComponent& humanoid = scene.humanoids[args.jobIndex];
			if (humanoid.ragdoll == nullptr)
				return;
			Entity humanoidEntity = scene.humanoids.GetEntity(args.jobIndex);
						...
						if (rb.vehicle_constraint != nullptr)
						{
							...
							for (uint32_t i = 0; i < count; ++i)
							{
								...
								if (wheel_transform != nullptr)
								{
									...
									Vec3 right = cast(pf::math::GetRight(localMatrix)).Normalized();
									Vec3 up = cast(pf::math::GetUp(localMatrix)).Normalized();
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.softbodies.GetCount(), 1, [&scene, &physics_scene](pf::jobsystem::JobArgs args) {
			...
			physicscomponent.aabb = pf::primitive::AABB();
...
#if 0
				scene.locker.lock();
				pf::renderer::DrawAxis(W, 0.05f, false);
				scene.locker.unlock();
#endif

				physicscomponent.aabb._min = pf::math::Min(physicscomponent.aabb._min, p0);
				physicscomponent.aabb._max = pf::math::Max(physicscomponent.aabb._max, p0);
			}
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.humanoids.GetCount(), 1, [&scene, &physics_scene](pf::jobsystem::JobArgs args) {
			...
			humanoid.ragdoll_bounds = pf::primitive::AABB();
			int bodypart = 0;
			...
			for (auto& rb : ragdoll.rigidbodies)
			{
				...
				humanoid.ragdoll_bounds = pf::primitive::AABB::Merge(humanoid.ragdoll_bounds, bp.capsule.getAABB());

#if 0
				scene.locker.lock();
				pf::renderer::DrawCapsule(bp.capsule, XMFLOAT4(1, 1, 1, 1), false);
				scene.locker.unlock();
#endif
			...
			pf::renderer::DrawBox(mat, XMFLOAT4(1, 1, 1, 1), false);
			scene.locker.unlock();
#endif
			});
		...
		if (IsDebugDrawEnabled())
		{
			class JoltDebugRenderer : public DebugRendererSimple
			{
				void DrawLine(RVec3Arg inFrom, RVec3Arg inTo, ColorArg inColor) override
				{
					...
					line.color_start = line.color_end = pf::Color(inColor.r, inColor.g, inColor.b, inColor.a);
				...
				void DrawText3D(RVec3Arg inPosition, const string_view& inString, ColorArg inColor = JPH::Color::sWhite, float inHeight = 0.5f) override
				{
					...
					params.scaling = 0.6f;
					params.flags |= pf::renderer::DebugTextParams::CAMERA_FACING;
					params.flags |= pf::renderer::DebugTextParams::CAMERA_SCALING;
					pf::renderer::DrawDebugText(inString.data(), params);
				}
			};
		...
		pf::profiler::EndRange(range); // Physics
	}
		...
		pf::jobsystem::Dispatch(ctx, (uint32_t)scene.rigidbodies.GetCount(), dispatchGroupSize, [&scene, &physics_scene](pf::jobsystem::JobArgs args) {
			...
			for (uint32_t i = 0; i < count; ++i)
			{
				...
				if (wheel_transform != nullptr)
				{
					...
					pf::renderer::DrawAxis(XMLoadFloat4x4(&tmp), 1);
#endif
				}
			}
			});
	...
	void SetConstraintBroken(pf::scene::PhysicsConstraintComponent& physicscomponent, bool broken)
	{
		if (physicscomponent.physicsobject == nullptr)
			return;
		Constraint& constraint = GetConstraint(physicscomponent);
	...
	)
	{
		...
		switch (state)
		{
		case pf::physics::ActivationState::Active:
			body_interface.ActivateBody(physicsobject.bodyID);
			break;
		case pf::physics::ActivationState::Inactive:
	...
	)
	{
		...
		else
		{
			...
			internal_state->pick_distance = pf::math::Distance(ray.origin, result.position);
