...
namespace pf
{
	...
	void TrailRenderer::Draw(const CameraComponent& camera, CommandList cmd) const
	{
		...
		sb.g_xTrailTextureIndex1 = device->GetDescriptorIndex(texture.IsValid() ? &texture : pf::texturehelper::getWhite(), SubresourceType::SRV);
		sb.g_xTrailTextureIndex2 = device->GetDescriptorIndex(texture2.IsValid() ? &texture2 : pf::texturehelper::getWhite(), SubresourceType::SRV);
		...
		while (next_cut <= cuts.size())
		{
			...
			for (; i < last; ++i)
			{
				...
				for (uint32_t j = 0; j < subdivision; ++j)
				{
					...
					const XMVECTOR P = pf::math::CatmullRomCentripetal(P0, P1, P2, P3, t);
					const XMVECTOR P_prev = pf::math::CatmullRomCentripetal(P0, P1, P2, P3, t - resolution_rcp);
					const XMVECTOR P_next = pf::math::CatmullRomCentripetal(P0, P1, P2, P3, t + resolution_rcp);
	...
	namespace TrailRenderer_Internal
	{
		void LoadShaders()
		{
			pf::renderer::LoadShader(ShaderStage::VS, vertexShader, "trailVS.cso");
			pf::renderer::LoadShader(ShaderStage::PS, pixelShader, "trailPS.cso");
	...
	void TrailRenderer::Initialize()
	{
		...
		static pf::eventhandler::Handle handle = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { TrailRenderer_Internal::LoadShaders(); });
		TrailRenderer_Internal::LoadShaders();

		p_log("pf::TrailRenderer Initialized (%d ms)", (int)std::round(timer.elapsed()));
	}
}
