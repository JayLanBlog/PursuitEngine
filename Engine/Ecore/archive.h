...
namespace pf {
	...
	private:
		...
		vector<uint8_t> DATA; // data suitable for read/write operations
		...
		void WriteCompressedData(vector<uint8_t>& final_data) const;
	...
	public:
		...
		void WriteData(vector<uint8_t>& dest) const;
		...
		inline Archive& operator<<(const Color& data)
		{
			_write(data.rgba);
			return *this;
		}
		...
		template<typename T>
		inline Archive& operator<<(const vector<T>& data)
		{
		...
		inline Archive& operator>>(Color& data)
		{
			_read(data.rgba);
			return *this;
		}
		...
		template<typename T>
		inline Archive& operator>>(vector<T>& data)
		{
			// Here we will use the >> operator so that non-specified types will have compile error!
			size_t count;
			(*this) >> count;
