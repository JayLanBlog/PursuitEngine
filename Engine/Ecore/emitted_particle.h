...
#include "primitive.h"
#include "Shader/ShaderInterop_EmittedParticle.h"
#include  "enums.h"
...
namespace pf
{
	...
	public:
		...
		pf::graphics::GPUBuffer statisticsReadbackBuffer[pf::graphics::GraphicsDevice::GetBufferCount()];
		pf::graphics::GPUBuffer particleBuffer;
		pf::graphics::GPUBuffer aliveList[2];
		pf::graphics::GPUBuffer deadList;
		pf::graphics::GPUBuffer distanceBuffer; // for sorting
		pf::graphics::GPUBuffer sphGridCells;  // for SPH
		pf::graphics::GPUBuffer sphParticleCells;  // for SPH
		pf::graphics::GPUBuffer densityBuffer; // for SPH
		pf::graphics::GPUBuffer counterBuffer;
		pf::graphics::GPUBuffer indirectBuffers; // kickoffUpdate, simulation, draw
		pf::graphics::GPUBuffer constantBuffer;
		pf::graphics::GPUBuffer generalBuffer;
		pf::scene::MeshComponent::BufferView vb_pos;
		pf::scene::MeshComponent::BufferView vb_nor;
		pf::scene::MeshComponent::BufferView vb_uvs;
		pf::scene::MeshComponent::BufferView vb_col;
		pf::graphics::GPUBuffer primitiveBuffer; // raytracing
		pf::graphics::GPUBuffer culledIndirectionBuffer; // rasterization
		pf::graphics::GPUBuffer culledIndirectionBuffer2; // rasterization
		pf::graphics::Texture opacityCurveTex;
	...
	private:
		...
		mutable pf::vector<EmitLocation> emit_locations;

	public:
		void UpdateCPU(const pf::scene::TransformComponent& transform, float dt);
		void Burst(int num);
		void Burst(int num, const XMFLOAT3& position, const pf::Color& color = pf::Color::White());
		void Burst(int num, const XMFLOAT4X4& transform, const pf::Color& color = pf::Color::White());
		void Restart();

		// Must have a transform and material component, but mesh is optional
		void UpdateGPU(uint32_t instanceIndex, const pf::scene::MeshComponent* mesh, pf::graphics::CommandList cmd) const;
		void Draw(const pf::scene::MaterialComponent& material, pf::graphics::CommandList cmd, const PARTICLESHADERTYPE* shadertype_override = nullptr) const;
		...
		const pf::graphics::Texture* GetOpacityCurveTex() const { return &opacityCurveTex; }
