...
namespace pf::ecs {
	...
	struct EntitySerializer
	{
		...
		unordered_map<uint64_t, Entity> remap;
		...
		unordered_set<std::string> resource_registration; // register for resource manager serialization
		ComponentLibrary* componentlibrary = nullptr;
		unordered_map<std::string, uint64_t> library_versions;
...
#pragma GCC diagnostic pop
	...
	public:
		...
		virtual void Component_Serialize(Entity entity, Archive& archive, EntitySerializer& seri) = 0;
		...
		virtual const vector<Entity>& GetEntityArray() const = 0;
	};
	...
	public:
		...
		inline void Component_Serialize(Entity entity, Archive& archive, EntitySerializer& seri)
		{
			if (archive.IsReadMode())
			{
		...
		inline const vector<Entity>& GetEntityArray() const { return entities; }

		// Returns the tightly packed [read only] component array
		inline const vector<Component>& GetComponentArray() const { return components; }
...
//#define LOOKUP_HASH
		...
		struct LookupTable
		{
			...
			vector<BlockL2*> blocks_l2;
			allocator::BlockAllocator<BlockData, 8> block_allocator; // block allocator manages memory per 8 blocks here (sizeof(BlockData) * 8 = 4 KB pages)
			allocator::BlockAllocator<BlockL2, 4> block_allocator_l2;
			...
			unordered_map<uint64_t, Block> table;
			...
			unordered_map<Entity, size_t> table;
	...
	public:
		...
		unordered_map<std::string, LibraryEntry> entries;
		...
		inline void Entity_Serialize(Entity entity, Archive& archive, EntitySerializer& seri)
		{
			seri.componentlibrary = this;
			if (archive.IsReadMode())
			{
