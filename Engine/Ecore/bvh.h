...
namespace pf
{
	...
	struct BVH
	{
		...
		void Build(const pf::primitive::AABB* aabbs, uint32_t aabb_count)
		{
			node_count = 0;
			if (aabb_count == 0)
				return;
			...
			{
				node.aabb = pf::primitive::AABB::Merge(node.aabb, aabbs[i]);
				leaf_indices[i] = i;
			}
			Subdivide(0, aabbs);
		}
		...
		void Update(const pf::primitive::AABB* aabbs, uint32_t aabb_count)
		{
			if (node_count == 0)
				return;
			if (aabb_count == 0)
			...
			{
				Node& node = nodes[i];
				node.aabb = pf::primitive::AABB();
				if (node.isLeaf())
				{
					for (uint32_t j = 0; j < node.count; ++j)
					{
						node.aabb = pf::primitive::AABB::Merge(node.aabb, aabbs[leaf_indices[node.offset + j]]);
					}
				}
				else
				{
					node.aabb = pf::primitive::AABB::Merge(node.aabb, nodes[node.left].aabb);
					node.aabb = pf::primitive::AABB::Merge(node.aabb, nodes[node.left + 1].aabb);
				}
			}
		}
	...
	private:
		void UpdateNodeBounds(uint32_t nodeIndex, const pf::primitive::AABB* leaf_aabb_data)
		{
			Node& node = nodes[nodeIndex];
			node.aabb = {};
			for (uint32_t i = 0; i < node.count; ++i)
			{
				...
				node.aabb = pf::primitive::AABB::Merge(node.aabb, leaf_aabb_data[index]);
			}
		}

		void Subdivide(uint32_t nodeIndex, const pf::primitive::AABB* leaf_aabb_data)
		{
			Node& node = nodes[nodeIndex];
			if (node.count <= 2)
				return;
