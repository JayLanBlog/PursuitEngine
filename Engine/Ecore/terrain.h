...
{
	template <>
	struct hash<pf::terrain::Chunk>
	{
		constexpr uint64_t operator()(const pf::terrain::Chunk& chunk) const
		{
...
namespace pf::terrain
{
	...
	struct VirtualTextureAtlas
	{
		struct Map
		{
			...
			pf::graphics::Texture texture_raw_block;
		};
		Map maps[4];
		pf::graphics::GPUBuffer tile_pool;
		...
		pf::vector<PhysicalTile> physical_tiles;

		struct Residency
		{
			pf::graphics::Texture feedbackMap;
			pf::graphics::Texture residencyMap;
			pf::graphics::GPUBuffer requestBuffer;
			pf::graphics::GPUBuffer allocationBuffer;
			pf::graphics::GPUBuffer allocationBuffer_CPU_readback[pf::graphics::GraphicsDevice::GetBufferCount()];
			pf::graphics::GPUBuffer pageBuffer;
			pf::graphics::GPUBuffer pageBuffer_CPU_upload[pf::graphics::GraphicsDevice::GetBufferCount()];
			bool data_available_CPU[pf::graphics::GraphicsDevice::GetBufferCount()] = {};
		...
		pf::unordered_map<uint32_t, pf::vector<std::shared_ptr<Residency>>> free_residencies; // per resolution residencies
	...
	struct VirtualTexture
	{
		...
		pf::vector<VirtualTextureAtlas::Tile> tiles;
		...
		pf::vector<AllocationRequest> allocation_requests;
		...
		mutable pf::vector<UpdateRequest> update_requests;
		pf::graphics::Texture blendmap;
	};

	struct BlendmapLayer
	{
		pf::vector<uint8_t> pixels;
	};

	struct ChunkData
	{
		pf::ecs::Entity entity = pf::ecs::INVALID_ENTITY;
		pf::ecs::Entity grass_entity = pf::ecs::INVALID_ENTITY;
		pf::ecs::Entity props_entity = pf::ecs::INVALID_ENTITY;
		float prop_density_current = 1;
		float grass_density_current = 1;
		const XMFLOAT3* mesh_vertex_positions = nullptr;
		pf::HairParticleSystem grass;
		pf::vector<BlendmapLayer> blendmap_layers;
		pf::vector<BlendmapLayer> spline_blendmap_layers;
		pf::graphics::Texture blendmap;
		pf::primitive::Sphere sphere;
		XMFLOAT3 position = XMFLOAT3(0, 0, 0);
		bool visible = true;
		std::shared_ptr<VirtualTexture> vt;
		pf::vector<uint16_t> heightmap_data;
		pf::graphics::Texture heightmap;
	...
	struct Prop
	{
		pf::vector<uint8_t> data; // serialized component data storage
	...
	struct Terrain
	{
		...
		pf::ecs::Entity terrainEntity = pf::ecs::INVALID_ENTITY;
		pf::ecs::Entity chunkGroupEntity = pf::ecs::INVALID_ENTITY;
		pf::scene::Scene* scene = nullptr;
		pf::vector<pf::ecs::Entity> materialEntities;
		pf::vector<pf::ecs::Entity> splineMaterialEntities;
		pf::ecs::Entity grassEntity = pf::ecs::INVALID_ENTITY;
		pf::scene::WeatherComponent weather;
		pf::HairParticleSystem grass_properties;
		pf::scene::MaterialComponent grass_material;
		pf::unordered_map<Chunk, ChunkData> chunks;
		Chunk center_chunk;
		pf::noise::Perlin perlin_noise;
		pf::vector<Prop> props;
		...
		pf::vector<VirtualTexture*> virtual_textures_in_use;
		pf::graphics::Sampler sampler;
		VirtualTextureAtlas atlas;

		pf::graphics::GPUBuffer chunk_buffer;
		...
		pf::vector<std::shared_ptr<Modifier>> modifiers;
		pf::vector<Modifier*> modifiers_to_remove;
		...
		void Generation_Update(const pf::scene::CameraComponent& camera);
		...
		void UpdateVirtualTexturesGPU(pf::graphics::CommandList cmd) const;
		void CopyVirtualTexturePageStatusGPU(pf::graphics::CommandList cmd) const;
		void AllocateVirtualTextureTileRequestsGPU(pf::graphics::CommandList cmd) const;
		void WritebackTileRequestsGPU(pf::graphics::CommandList cmd) const;
	...
	private:
		pf::vector<pf::scene::MaterialComponent> materials; // temp storage allocation
	};
	...
	struct Modifier
	{
		...
		constexpr void Blend(float& height, float value)
		{
			...
			case BlendMode::Normal:
				height = pf::math::Lerp(height, value, weight);
				break;
	...
	struct VoronoiModifier : public Modifier
	{
		...
		void Apply(const XMFLOAT2& world_pos, float& height) override
		{
			...
			pf::noise::voronoi::Result res = pf::noise::voronoi::compute(p.x, p.y, (float)seed);
