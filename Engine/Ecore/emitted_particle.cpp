...
namespace pf
{
	...
	void EmittedParticleSystem::CreateSelfBuffers()
	{
		...
		if (particleBuffer.desc.size < MAX_PARTICLES * sizeof(Particle))
		{
			...
			primitiveBuffer = pf::renderer::GetIndexBufferForQuads(MAX_PARTICLES);
		}
	...
	void EmittedParticleSystem::UpdateCPU(const TransformComponent& transform, float dt)
	{
		...
		const uint32_t oldest_stat_index = pf::graphics::GetDevice()->GetBufferIndex();
	...
	void EmittedParticleSystem::Burst(int num, const XMFLOAT3& position, const pf::Color& color)
	{
		if (IsPaused() || num <= 0)
			return;
	...
	void EmittedParticleSystem::Burst(int num, const XMFLOAT4X4& transform, const pf::Color& color)
	{
		if (IsPaused() || num <= 0)
			return;
	...
	void EmittedParticleSystem::UpdateGPU(uint32_t instanceIndex, const MeshComponent* mesh, CommandList cmd) const
	{
		...
		if (!IsPaused() && dt > 0)
		{
			...
			cb.xEmitterRandomness = pf::random::GetRandom(0.0f, 1.0f);
			...
			if (IsSPHEnabled())
			{
				auto range = pf::profiler::BeginRangeGPU("SPH - Simulation", cmd);
		...
		if (IsSorted())
		{
			pf::gpusortlib::Sort(MAX_PARTICLES, distanceBuffer, counterBuffer, PARTICLECOUNTER_OFFSET_CULLEDCOUNT, culledIndirectionBuffer, cmd);
		}
	...
	void EmittedParticleSystem::Draw(const MaterialComponent& material, CommandList cmd, const PARTICLESHADERTYPE* shadertype_override) const
	{
		...
		if (pf::renderer::IsWireRender())
		{
			device->BindPipelineState(&PSO_wire, cmd);
		}
		else
		{
			const pf::enums::BLENDMODE blendMode = material.GetBlendMode();
	...
	namespace EmittedParticleSystem_Internal
	{
		void LoadShaders()
		{
			pf::renderer::LoadShader(ShaderStage::VS, vertexShader, "emittedparticleVS.cso");

			if (ALLOW_MESH_SHADER && pf::graphics::GetDevice()->CheckCapability(GraphicsDeviceCapability::MESH_SHADER))
			{
				pf::renderer::LoadShader(ShaderStage::MS, meshShader, "emittedparticleMS.cso");
			}

			pf::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT], "emittedparticlePS_soft.cso");
			pf::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT_DISTORTION], "emittedparticlePS_soft_distortion.cso");
			pf::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SIMPLE], "emittedparticlePS_simple.cso");
			pf::renderer::LoadShader(ShaderStage::PS, pixelShader[EmittedParticleSystem::SOFT_LIGHTING], "emittedparticlePS_soft_lighting.cso");

			pf::renderer::LoadShader(ShaderStage::CS, kickoffUpdateCS, "emittedparticle_kickoffUpdateCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, finishUpdateCS, "emittedparticle_finishUpdateCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, emitCS, "emittedparticle_emitCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, emitCS_VOLUME, "emittedparticle_emitCS_volume.cso");
			pf::renderer::LoadShader(ShaderStage::CS, emitCS_FROMMESH, "emittedparticle_emitCS_FROMMESH.cso");
			pf::renderer::LoadShader(ShaderStage::CS, sphpartitionCS, "emittedparticle_sphpartitionCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, sphcellallocationCS, "emittedparticle_sphcellallocationCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, sphbinningCS, "emittedparticle_sphbinningCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, sphdensityCS, "emittedparticle_sphdensityCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, sphforceCS, "emittedparticle_sphforceCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, simulateCS, "emittedparticle_simulateCS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, simulateCS_SORTING, "emittedparticle_simulateCS_SORTING.cso");
			pf::renderer::LoadShader(ShaderStage::CS, simulateCS_DEPTHCOLLISIONS, "emittedparticle_simulateCS_DEPTHCOLLISIONS.cso");
			pf::renderer::LoadShader(ShaderStage::CS, simulateCS_SORTING_DEPTHCOLLISIONS, "emittedparticle_simulateCS_SORTING_DEPTHCOLLISIONS.cso");
	...
	void EmittedParticleSystem::Initialize()
	{
		...
		static pf::eventhandler::Handle handle = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { EmittedParticleSystem_Internal::LoadShaders(); });
		EmittedParticleSystem_Internal::LoadShaders();

		p_log("pf::EmittedParticleSystem Initialized (%d ms)", (int)std::round(timer.elapsed()));
	}


	void EmittedParticleSystem::Serialize(pf::Archive& archive, pf::ecs::EntitySerializer& seri)
	{
		if (archive.IsReadMode())
		{
			archive >> _flags;
			archive >> (uint32_t&)shaderType;
			pf::ecs::SerializeEntity(archive, meshID, seri);
