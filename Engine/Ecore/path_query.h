...
namespace pf
{
	struct PathQuery
	{
		...
		std::priority_queue<Node, pf::vector<Node>, std::greater<Node>> frontier;
		pf::unordered_map<uint64_t, Node> came_from;
		pf::unordered_map<uint64_t, uint16_t> cost_so_far;
		pf::vector<XMFLOAT3> result_path_goal_to_start;
		pf::vector<XMFLOAT3> result_path_goal_to_start_simplified;
