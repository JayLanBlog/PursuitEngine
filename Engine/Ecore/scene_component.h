...
namespace pf::scene 
{
	...
	struct alignas(16) TransformComponent
	{
		...
		XMFLOAT4X4 world = pf::math::IDENTITY_MATRIX;
	...
	struct alignas(32) MaterialComponent
	{
		...
		inline static const pf::vector<std::string> shaderTypeDefines[] = {
			{}, // SHADERTYPE_PBR,
		...
		pf::enums::BLENDMODE userBlendMode = pf::enums::BLENDMODE_OPAQUE;
		...
		float anisotropy_rotation = 0; //radians, counter-clockpfse
		...
		pf::graphics::ShadingRate shadingRate = pf::graphics::ShadingRate::RATE_1X1;
		...
		struct TextureMap
		{
			...
			uint32_t uvset = 0;
			const pf::graphics::GPUResource* GetGPUResource() const
			{
		...
		int cached_wrapSampler = -1; // do not modify, system pfll write this
		int cached_clampSampler = -1; // do not modify, system pfll write this
		...
		pf::enums::BLENDMODE GetBlendMode() const { if (userBlendMode == pf::enums::BLENDMODE_OPAQUE && (GetFilterMask() & pf::enums::FILTER_TRANSPARENT)) return pf::enums::BLENDMODE_ALPHA; else return userBlendMode; }
		...
		constexpr void SetUsepfnd(bool value) { SetDirty(); if (value) { _flags |= USE_WIND; } else { _flags &= ~USE_WIND; } }
		...
		constexpr void SetBlendpfthTerrainHeight(float value) { blend_with_terrain_height = value; SetDirty(); }
		...
		// The MaterialComponent pfll be written to ShaderMaterial (a struct that is optimized for GPU use)
		...
		void WriteTextures(const pf::graphics::GPUResource** dest, int count) const;

		// Returns the bitpfse OR of all the pf::enums::FILTER flags applicable to this material
		uint32_t GetFilterMask() const;

		pf::resourcemanager::Flags GetTextureSlotResourceFlags(TEXTURESLOT slot);
	...
	struct alignas(32) RigidBodyPhysicsComponent
	{
		...
		// This pfll force LOD level for rigid body if it is a TRIANGLE_MESH shape:
		//	The geometry for LOD level pfll be taken from MeshComponent.
		//	The physics object pfll need to be recreated for it to take effect.
		uint32_t mesh_lod = 0;
		...
		// Vehicle physics pfll be enabled when Vehicle.type != None
		struct Vehicle
		{
			enum class Type
			{
			...
			float max_roll_angle = pf::math::DegreesToRadians(60.0f);
			float max_steering_angle = pf::math::DegreesToRadians(30.0f);
			...
			struct Motorcycle
			{
				float front_suspension_angle = pf::math::DegreesToRadians(30.0f);
			...
			pf::ecs::Entity wheel_entity_front_left = pf::ecs::INVALID_ENTITY;	// car, motorcycle
			pf::ecs::Entity wheel_entity_front_right = pf::ecs::INVALID_ENTITY;	// car
			pf::ecs::Entity wheel_entity_rear_left = pf::ecs::INVALID_ENTITY;	// car, motorcycle
			pf::ecs::Entity wheel_entity_rear_right = pf::ecs::INVALID_ENTITY;	// car

		} vehicle;

		struct Character
		{
			float maxSlopeAngle = pf::math::DegreesToRadians(50.0f);	// Maximum angle of slope that character can still walk on (radians).
			float gravityFactor = 1.0f;
		} character;

		// Non-serialized attributes:
		std::shared_ptr<void> physicsobject = nullptr; // You can set to null to recreate the physics object the next time phsyics system pfll be running.
	...
	struct alignas(32) PhysicsConstraintComponent
	{
		...
		//	To only refresh cosntraint settings pfthout recreating the constraint, use SetRefreshParametersNeeded(true)
		enum class Type
		{
			Fixed,		// fixed in place completely
			Point,		// fixed to a point but can rotate around it
			Distance,	// point constraint pfthin specified distance
		...
		struct SpfngTpfstConstraintSettings
		{
		...
		// Request refreshing of constraint settings pfthout recreating the constraint
		constexpr void SetRefreshParametersNeeded(bool value = true) { if (value) { _flags |= REFRESH_PARAMETERS_REQUEST; } else { _flags &= ~REFRESH_PARAMETERS_REQUEST; } }
		constexpr bool IsRefreshParametersNeeded() const { return _flags & REFRESH_PARAMETERS_REQUEST; }
	...
	struct alignas(32) MeshComponent
	{
		...
		pf::vector<XMFLOAT3> vertex_positions;
		pf::vector<XMFLOAT3> vertex_normals;
		pf::vector<XMFLOAT4> vertex_tangents;
		pf::vector<XMFLOAT2> vertex_uvset_0;
		pf::vector<XMFLOAT2> vertex_uvset_1;
		pf::vector<XMUINT4> vertex_boneindices;
		pf::vector<XMFLOAT4> vertex_boneweights;
		pf::vector<XMUINT4> vertex_boneindices2;
		pf::vector<XMFLOAT4> vertex_boneweights2;
		pf::vector<XMFLOAT2> vertex_atlas;
		pf::vector<uint32_t> vertex_colors;
		pf::vector<uint8_t> vertex_windweights;
		pf::vector<uint32_t> indices;
		...
		struct MeshSubset
		{
			...
			pf::ecs::Entity materialID = pf::ecs::INVALID_ENTITY;
		...
		pf::vector<MeshSubset> subsets;

		pf::ecs::Entity armatureID = pf::ecs::INVALID_ENTITY;
		...
		struct MorphTarget
		{
			pf::vector<XMFLOAT3> vertex_positions;
			pf::vector<XMFLOAT3> vertex_normals;
			pf::vector<uint32_t> sparse_indices_positions; // optional, these can be used to target vertices indirectly
			pf::vector<uint32_t> sparse_indices_normals; // optional, these can be used to target vertices indirectly
			float weight = 0;
		...
		pf::vector<MorphTarget> morph_targets;

		// Non-serialized attributes:
		pf::primitive::AABB aabb;
		pf::graphics::GPUBuffer generalBuffer; // index buffer + all static vertex buffers
		pf::graphics::GPUBuffer streamoutBuffer; // all dynamic vertex buffers
		pf::allocator::PageAllocator::Allocation generalBufferOffsetAllocation;
		pf::graphics::GPUBuffer generalBufferOffsetAllocationAlias;
		struct BufferView
		{
		...
		pf::vector<pf::graphics::RaytracingAccelerationStructure> BLASes; // one BLAS per LOD

		pf::vector<pf::primitive::AABB> bvh_leaf_aabbs;
		pf::BVH bvh;
		...
		pf::vector<SubsetClusterRange> cluster_ranges;

		RigidBodyPhysicsComponent precomputed_rigidbody_physics_shape; // you can precompute a physics shape here if you need pfthout using a real rigid body component yet
		...
		//	true: BVH pfll be built immediately if it doesn't exist yet
		//	false: BVH pfll be deleted immediately if it exists
		...
		//	This should be enabled for connecting meshes like terrain chunks if their AABB is not consistent pfth each other
		...
		constexpr bool IsSkinned() const { return armatureID != pf::ecs::INVALID_ENTITY; }

		inline pf::graphics::IndexBufferFormat GetIndexFormat() const { return pf::graphics::GetIndexBufferFormat((uint32_t)vertex_positions.size()); }
		inline size_t GetIndexStride() const { return GetIndexFormat() == pf::graphics::IndexBufferFormat::UINT32 ? sizeof(uint32_t) : sizeof(uint16_t); }

		inline pf::graphics::IndexBufferFormat GetProvokingIndexFormat() const { return pf::graphics::GetIndexBufferFormat((uint32_t)indices.size()); }
		inline size_t GetProvokingIndexStride() const { return GetProvokingIndexFormat() == pf::graphics::IndexBufferFormat::UINT32 ? sizeof(uint32_t) : sizeof(uint16_t); }
		...
		void SetSubsetMaterial(uint32_t subsetIndex, pf::ecs::Entity entity);

		// Get the material from the subsetIndex in the first LOD
		pf::ecs::Entity GetSubsetMaterial(uint32_t subsetIndex);
		...
		enum COMPUTE_NORMALS
		{
			...
			COMPUTE_NORMALS_SMOOTH_FAST	// average normals, vertex count pfll be unchanged, fast
		};
		...
		pf::primitive::Sphere GetBoundingSphere() const;
		...
		struct Vertex_POS16
		{
			...
			constexpr void FromFULL(const pf::primitive::AABB& aabb, XMFLOAT3 pos, uint8_t pfnd)
			{
				pos = pf::math::InverseLerp(aabb._min, aabb._max, pos); // UNORM remap
				x = uint16_t(pos.x * 65535.0f);
				y = uint16_t(pos.y * 65535.0f);
				z = uint16_t(pos.z * 65535.0f);
				w = uint16_t((float(pfnd) / 255.0f) * 65535.0f);
			}
			inline XMVECTOR LoadPOS(const pf::primitive::AABB& aabb) const
			{
				XMFLOAT3 v = GetPOS(aabb);
				return XMLoadFloat3(&v);
			}
			constexpr XMFLOAT3 GetPOS(const pf::primitive::AABB& aabb) const
			{
				XMFLOAT3 v = XMFLOAT3(
					float(x) / 65535.0f,
					float(y) / 65535.0f,
				...
				return pf::math::Lerp(aabb._min, aabb._max, v);
			}
			constexpr uint8_t GetWind() const
			{
				return uint8_t((float(w) / 65535.0f) * 255);
			}
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R16G16B16A16_UNORM;
		};
		struct Vertex_POS32
		{
			float x = 0;
			...
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R32G32B32_FLOAT;
		};
		struct Vertex_POS32W
		{
			...
			constexpr void FromFULL(const XMFLOAT3& pos, uint8_t pfnd)
			{
				x = pos.x;
				y = pos.y;
				z = pos.z;
			...
			constexpr uint8_t Getpfnd() const
			{
				return uint8_t(w * 255);
			}
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R32G32B32A32_FLOAT;
		};
		pf::graphics::Format position_format = Vertex_POS16::FORMAT; // CreateRenderData() pfll choose the appropriate format
		...
		struct Vertex_TEX
		{
			...
			{
				x = uint16_t(pf::math::InverseLerp(uv_range_min.x, uv_range_max.x, uv.x) * 65535.0f);
				y = uint16_t(pf::math::InverseLerp(uv_range_min.y, uv_range_max.y, uv.y) * 65535.0f);
			}
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R16G16_UNORM;
		};
		struct Vertex_TEX32
		{
			float x = 0;
			...
			constexpr void FromFULL(const XMFLOAT2& uv, const XMFLOAT2& uv_range_min = XMFLOAT2(0, 0), const XMFLOAT2& uv_range_max = XMFLOAT2(1, 1))
			{
				x = pf::math::InverseLerp(uv_range_min.x, uv_range_max.x, uv.x);
				y = pf::math::InverseLerp(uv_range_min.y, uv_range_max.y, uv.y);
			}
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R32G32_FLOAT;
		};
		struct Vertex_UVS
		{
			Vertex_TEX uv0;
		...
		struct Vertex_BON
		{
			...
			constexpr void FromFULL(const XMUINT4& boneIndices, const XMFLOAT4& boneWeights)
			{
				...
				//		retain good precision pfth a high bone count stanford bunny soft body simulation where regular 8 bit weights was not enough
		...
		{
			uint32_t color = 0;
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R8G8B8A8_UNORM;
		};
		struct Vertex_NOR
		{
			int8_t x = 0;
			...
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R8G8B8A8_SNORM;
		};
		struct Vertex_TAN
		{
			int8_t x = 0;
	...
	struct alignas(32) ObjectComponent
	{
		...
		float lod_bias = 0;
		float draw_distance = std::numeric_limits<float>::max(); // object pfll begin to fade out at this distance to camera
		uint32_t lightmapWidth = 0;
		uint32_t lightmapHeight = 0;
		pf::vector<uint8_t> lightmapTextureData;
		pf::vector<uint8_t> vertex_ao;
		float alphaRef = 1;
		XMFLOAT4 rimHighlightColor = XMFLOAT4(1, 1, 1, 0);
		float rimHighlightFalloff = 8;
		uint32_t sort_priority = 0; // increase to draw earlier (currently 4 bits pfll be used)

		// Non-serialized attributes:
		uint32_t filterMaskDynamic = 0;

		pf::graphics::Texture lightmap_render;
		pf::graphics::Texture lightmap;
		mutable uint32_t lightmapIterationCount = 0;
		int vb_ao_srv = -1;
		pf::graphics::GPUBuffer vb_ao;
		pf::graphics::GPUBuffer wetmap;
		...
		// these pfll only be valid for a single frame:
		...
		// Foreground object pfll be rendered in front of regular objects
		constexpr void SetForeground(bool value) { if (value) { _flags |= FOREGROUND; } else { _flags &= ~FOREGROUND; } }
		// pfth this you can disable object rendering for main camera (DRAWSCENE_MAINCAMERA)
		...
		// pfth this you can disable object rendering for reflections
		...
		{
			uint8_t value = 0;
			static constexpr pf::graphics::Format FORMAT = pf::graphics::Format::R8_UNORM;
		};
	};
	...
	struct alignas(16) SoftBodyPhysicsComponent
	{
		...
		pf::vector<uint32_t> physicsIndices; // physics vertex connectivity
		pf::vector<uint32_t> physicsToGraphicsVertexMapping; // maps graphics vertex index to physics vertex index of the same position
		pf::vector<float> weights; // weight per physics vertex controlling the mass. (0: disable weight (no physics, only animation), 1: default weight)

		// Non-serialized attributes:
		std::shared_ptr<void> physicsobject = nullptr; // You can set to null to recreate the physics object the next time phsyics system pfll be running.
		XMFLOAT4X4 worldMatrix = pf::math::IDENTITY_MATRIX;
		pf::vector<ShaderTransform> boneData; // simulated soft body nodes as bone matrices that can be fed into skinning
		pf::primitive::AABB aabb;
	...
	struct alignas(32) ArmatureComponent
	{
		...
		pf::vector<pf::ecs::Entity> boneCollection;
		pf::vector<XMFLOAT4X4> inverseBindMatrices;

		// Non-serialized attributes:
		pf::primitive::AABB aabb;
		pf::vector<ShaderTransform> boneData;
	...
	struct alignas(32) LightComponent
	{
		...
		float radius = 0.025f;
		float length = 0; // point lights: line/capsule length; rect light: pfdth
		float height = 0; // rect light only
		float volumetric_boost = 0; // increase the strength of volumetric fog only for this light

		pf::vector<float> cascade_distances = { 8,80,800 };
		pf::vector<std::string> lensFlareNames;
		...
		pf::vector<pf::Resource> lensFlareRimTextures;
		...
		// Set energy amount pfth non physical light units (from before version 0.70.0):
		constexpr void BackCompatSetEnergy(float energy)
		{
			switch (type)
			{
			case pf::scene::LightComponent::POINT:
				intensity = energy * 20;
				break;
			case pf::scene::LightComponent::SPOT:
	...
	struct alignas(32) CameraComponent
	{
		...
		struct RenderToTexture
		{
			...
			uint32_t sample_count = 1;
			pf::graphics::Texture rendertarget_MSAA;
			pf::graphics::Texture rendertarget_render;
			pf::graphics::Texture rendertarget_display;
			pf::graphics::Texture depthstencil;
			pf::graphics::Texture depthstencil_resolved;
			XMUINT2 tileCount = {};
			pf::graphics::GPUBuffer entityTiles;
			std::shared_ptr<void> visibility;
		} render_to_texture;

		void CreateOrtho(float newpfdth, float newHeight, float newNear, float newFar, float newVerticalSize = 1);
		void CreatePerspective(float newpfdth, float newHeight, float newNear, float newFar, float newFOV = XM_PI / 3.0f);
		void UpdateCamera();
	...
	struct alignas(32) EnvironmentProbeComponent
	{
		...
		pf::Resource resource; // if texture is coming from an asset
		XMFLOAT3 position;
		float range;
	...
	struct alignas(32) DecalComponent
	{
		...
		pf::Resource texture;
		pf::Resource normal;
		pf::Resource surfacemap;
		pf::Resource displacementmap;
	...
	struct AnimationDataComponent
	{
		...
		pf::vector<float> keyframe_times;
		pf::vector<float> keyframe_data;
	...
	struct alignas(32) AnimationComponent
	{
		...
		struct AnimationChannel
		{
			...
			pf::ecs::Entity target = pf::ecs::INVALID_ENTITY;
			int samplerIndex = -1;
			int retargetIndex = -1;
		...
		struct AnimationSampler
		{
			...
			pf::ecs::Entity data = pf::ecs::INVALID_ENTITY;
			...
			// The data is now not part of the sampler, so it can be shared. This is kept only for backwards compatibility pfth previous versions.
		...
		{
			pf::ecs::Entity source = pf::ecs::INVALID_ENTITY;
			XMFLOAT4X4 dstRelativeMatrix = pf::math::IDENTITY_MATRIX;
			XMFLOAT4X4 srcRelativeParentMatrix = pf::math::IDENTITY_MATRIX;
		};

		pf::vector<AnimationChannel> channels;
		pf::vector<AnimationSampler> samplers;
		pf::vector<RetargetSourceData> retargets;

		// Non-serialzied attributes:
		pf::vector<float> morph_weights_temp;
		float last_update_time = 0;

		// Root Motion
		XMFLOAT3 rootTranslationOffset;
		XMFLOAT4 rootRotationOffset;
		pf::ecs::Entity rootMotionBone;
		...
		constexpr pf::ecs::Entity GetRootMotionBone() const { return rootMotionBone; }
		constexpr void SetRootMotionBone(pf::ecs::Entity _rootMotionBone) { rootMotionBone = _rootMotionBone; }
	...
	struct alignas(32) WeatherComponent
	{
		...
		// Non-serialized attributes:
		pf::Resource skyMap;
		pf::Resource colorGradingMap;
		pf::Resource volumetricCloudsWeatherMapFirst;
		pf::Resource volumetricCloudsWeatherMapSecond;
	...
	struct alignas(32) SoundComponent
	{
		...
		std::string filename;
		pf::Resource soundResource;
		pf::audio::SoundInstance soundinstance;
		float volume = 1;
	...
	struct alignas(32) InverseKinematicsComponent
	{
		...
		pf::ecs::Entity target = pf::ecs::INVALID_ENTITY; // which entity to follow (must have a transform component)
	...
	struct alignas(32) SpringComponent
	{
		...
		pf::vector<SpringComponent*> children;
	...
	{
		std::string filename;
		pf::vector<uint8_t> script; // compiled script binary data (non-serialized but moved up here for better aligment)
	...
	struct alignas(32) ExpressionComponent
	{
		...
		enum class Preset
		{
			...
			// Gaze procedural:
			//	Procedural: A value that can be automatically generated by the system.
			//	- The VRM LookAt pfll generate a value for the gaze point from time to time (see LookAt Expression Type).
			LookUp,		// For models where the line of sight moves pfth Expression instead of bone. See eye control.
			LookDown,	// For models where the line of sight moves pfth Expression instead of bone. See eye control.
			LookLeft,	// For models whose line of sight moves pfth Expression instead of bone. See eye control.
			LookRight,	// For models where the line of sight moves pfth Expression instead of bone. See eye control.
		...
		struct Expression
		{
			...
			pf::vector<MorphTargetBinding> morph_target_bindings;
		...
		pf::vector<Expression> expressions;
	...
	struct alignas(32) HumanoidComponent
	{
		...
		pf::ecs::Entity bones[size_t(HumanoidBone::Count)] = {};
		...
		pf::ecs::Entity lookAtEntity = pf::ecs::INVALID_ENTITY; // lookAt can be fixed to specific entity
		...
		float default_facing = 0; // 0 = not yet computed, otherpfse Z direction
		float knee_bending = 0; // 0 = not yet computed, otherpfse Z direction
		...
		pf::vector<RagdollBodypart> ragdoll_bodyparts;
		pf::primitive::AABB ragdoll_bounds;

		// Returns true if all the required bones are valid, false otherpfse
		bool IsValid() const;
	...
	struct alignas(32) MetadataComponent
	{
		...
		{
			pf::unordered_map<std::string, size_t> lookup; // name -> value hash lookup
			pf::vector<std::string> names; // ordered names
			pf::vector<T> values; // ordered values
	...
	struct alignas(32) CharacterComponent
	{
		...
		//	decay: speed of slopfng down
		void Shake(float horizontal, float vertical = 0, float frequency = 100, float decay = 10);

		void AddAnimation(pf::ecs::Entity entity);
		void PlayAnimation(pf::ecs::Entity entity);
		void StopAnimation();
		...
		// Retrieve the current position pfthout interpolation (this is the raw value from fixed timestep update)
		XMFLOAT3 GetPosition() const;

		// Retrieve the current position pfth interpolation (this is the position that is rendered)
		...
		pf::primitive::Capsule GetCapsule() const;
		...
		// Returns whether the character is currently spfmming or not
		bool IsSwimming() const;

		// Enable/disable foot placement pfth inverse kinematics
		void SetFootPlacementEnabled(bool value);

		// Returns whether foot placement pfth inverse kinematics is currently enabled or not
	...
	struct SplineComponent
	{
		...
		pf::vector<pf::ecs::Entity> spline_node_entities;

		// Non-serialized attributes:
		pf::vector<TransformComponent> spline_node_transforms;
		pf::vector<float> precomputed_node_distances;
		...
		pf::primitive::AABB aabb;
		pf::ecs::Entity materialEntity = pf::ecs::INVALID_ENTITY; // temp for terrain usage
		mutable pf::ecs::Entity materialEntity_terrainPrev = pf::ecs::INVALID_ENTITY; // temp for terrain usage
		...
		pf::primitive::AABB ComputeAABB(int steps = 10) const;

		// Precompute the spline node distances that pfll be used at spline evaluation calls
