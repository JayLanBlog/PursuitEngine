...
namespace Luaer {
	...
	int Input_BindLua::Down(lua_State* L)
	{
		...
		if (argc > 0)
		{
			pf::input::BUTTON button = (pf::input::BUTTON)Luaer::SGetInt(L, 1);
			int playerindex = 0;
			if (argc > 1)
			{
				playerindex = Luaer::SGetInt(L, 2);
			}
			Luaer::SSetBool(L, pf::input::Down(button, playerindex));
			return 1;
		}
		else
			Luaer::SError(L, "Down(int code, opt int playerindex = 0) not enough arguments!");
		return 0;
	}
	int Input_BindLua::Press(lua_State* L)
	{
		...
		if (argc > 0)
		{
			pf::input::BUTTON code = (pf::input::BUTTON)Luaer::SGetInt(L, 1);
			int playerindex = 0;
			if (argc > 1)
			{
				playerindex = Luaer::SGetInt(L, 2);
			}
			Luaer::SSetBool(L, pf::input::Press(code, playerindex));
			return 1;
		}
		else
			Luaer::SError(L, "Press(int code, opt int playerindex = 0) not enough arguments!");
		return 0;
	}
	int Input_BindLua::Release(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SSetBool(L, pf::input::Release(code, playerindex));
			return 1;
		}
		else
			Luaer::SError(L, "Release(int code, opt int playerindex = 0) not enough arguments!");
		return 0;
	}
	int Input_BindLua::Hold(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (argc > 1)
			{
				duration = Luaer::SGetInt(L, 2);
			}
			bool continuous = false;
			if (argc > 2)
			{
				continuous = Luaer::SGetBool(L, 3);
			}
			int playerindex = 0;
			if (argc > 3)
			{
				playerindex = Luaer::SGetInt(L, 4);
			}
			Luaer::SSetBool(L, pf::input::Hold(button, duration, continuous, playerindex));
			return 1;
		}
		else
			Luaer::SError(L, "Hold(int code, opt int duration = 30, opt boolean continuous = false, opt int playerindex = 0) not enough arguments!");
		return 0;
	}
	int Input_BindLua::GetPointer(lua_State* L)
	{
		XMFLOAT4 P = pf::input::GetPointer();
	...
	int Input_BindLua::SetPointer(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (vec != nullptr)
			{
				pf::input::SetPointer(vec->data);
			}
			else
				Luaer::SError(L, "SetPointer(Vector props) argument is not a Vector!");
		}
		else
			Luaer::SError(L, "SetPointer(Vector props) not enough arguments!");
		return 0;
	}
	int Input_BindLua::GetPointerDelta(lua_State* L)
	{
		Luna<Vector_BindLua>::push(L, XMLoadFloat2(&pf::input::GetMouseState().delta_position));
		return 1;
	}
	int Input_BindLua::HidePointer(lua_State* L)
	{
		...
		if (argc > 0)
		{
			pf::input::HidePointer(Luaer::SGetBool(L, 1));
		}
		else
			Luaer::SError(L, "HidePointer(bool value) not enough arguments!");
		return 0;
	}
	int Input_BindLua::GetAnalog(lua_State* L)
	{
		...
		if (argc > 0)
		{
			pf::input::GAMEPAD_ANALOG type = (pf::input::GAMEPAD_ANALOG)Luaer::SGetInt(L, 1);
			int playerindex = 0;
			if (argc > 1)
			{
				playerindex = Luaer::SGetInt(L, 2);
			}
			result = pf::input::GetAnalog(type, playerindex);
		}
		else
			Luaer::SError(L, "GetAnalog(int type, opt int playerindex = 0) not enough arguments!");
	...
	{
		auto& touches = pf::input::GetTouches();
	...
	int Input_BindLua::SetControllerFeedback(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (fb != nullptr)
			{
				...
				pf::input::SetControllerFeedback(fb->feedback, playerindex);
			}
			else
			{
				Luaer::SError(L, "SetControllerFeedback(ControllerFeedback feedback, opt int playerindex = 0) first argument is not a ControllerFeedback!");
			}
		}
		else
			Luaer::SError(L, "SetControllerFeedback(ControllerFeedback feedback, opt int playerindex = 0) not enough arguments!");
		return 0;
	}
	int Input_BindLua::WhatIsPressed(lua_State* L)
	{
		...
		if (argc > 0)
		{
			playerindex = Luaer::SGetInt(L, 1);
		}
		Luaer::SSetInt(L, (int)pf::input::WhatIsPressed(playerindex));
		return 1;
	}
	int Input_BindLua::IsGamepadButton(lua_State* L)
	{
		...
		if (argc < 1)
		{
			Luaer::SError(L, "IsGamepadButton(int button) not enough arguments!");
			return 0;
		}
		pf::input::BUTTON button = (pf::input::BUTTON)Luaer::SGetInt(L, 1);
		Luaer::SSetBool(L, pf::input::IsGamepadButton(button));
		return 1;
	}
	int Input_BindLua::ButtonToString(lua_State* L)
	{
		...
		{
			pf::input::BUTTON button = (pf::input::BUTTON)Luaer::SGetInt(L, 1);
			pf::input::CONTROLLER_PREFERENCE preference = pf::input::CONTROLLER_PREFERENCE_GENERIC;
			if (argc > 1)
			{
				preference = (pf::input::CONTROLLER_PREFERENCE)Luaer::SGetInt(L, 2);
			}
			Luaer::SSetString(L, pf::input::ButtonToString(button, preference));
			return 1;
		}
		Luaer::SError(L, "ButtonToString(int button, opt int preference = CONTROLLER_PREFERENCE_GENERIC): not enough parameters!");
		return 0;
	}
	int Input_BindLua::StringToButton(lua_State* L)
	{
		...
		if (argc > 0)
		{
			const char* str = Luaer::SGetString(L, 1);
			Luaer::SSetInt(L, (int)pf::input::StringToButton(str));
			return 1;
		}
		Luaer::SError(L, "StringToButton(string str): not enough parameters!");
		return 0;
	}
	int Input_BindLua::SetCursor(lua_State* L)
	{
		...
		if (argc > 0)
		{
			pf::input::CURSOR cursor = (pf::input::CURSOR)Luaer::SGetInt(L, 1);
			pf::input::SetCursor(cursor);
			return 0;
		}
		Luaer::SError(L, "SetCursor(int cursor) not enough arguments!");
		return 0;
	}
	int Input_BindLua::SetCursorFromFile(lua_State* L)
	{
		...
		{
			pf::input::CURSOR cursor = (pf::input::CURSOR)Luaer::SGetInt(L, 1);
			const char* filename = Luaer::SGetString(L, 2);
			pf::input::SetCursorFromFile(cursor, filename);
			return 0;
		}
		Luaer::SError(L, "SetCursorFromFile(int cursor, string filename) not enough arguments!");
		return 0;
	}
	int Input_BindLua::ResetCursor(lua_State* L)
	{
		...
		{
			pf::input::CURSOR cursor = (pf::input::CURSOR)Luaer::SGetInt(L, 1);
			pf::input::ResetCursor(cursor);
			return 0;
		}
		Luaer::SError(L, "ResetCursor(int cursor) not enough arguments!");
		return 0;
	}
	...
	void Input_BindLua::Bind()
	{
		...
		{
			initialized = true;
			Luna<Input_BindLua>::Register(Luaer::GetLuaState());
...
CURSOR_CROSS = 9
	...
	{
		Luaer::SSetInt(L, (int)touch.state);
		return 1;
	}
	...
	void Touch_BindLua::Bind()
	{
		...
		{
			initialized = true;
			Luna<Touch_BindLua>::Register(Luaer::GetLuaState());
		}
	}
	...
	int ControllerFeedback_BindLua::SetVibrationLeft(lua_State* L)
	{
		...
		if (argc > 0)
		{
			feedback.vibration_left = Luaer::SGetFloat(L, 1);
		}
		else
		{
			Luaer::SError(L, "SetVibrationLeft(float value) not enough arguments!");
		}
		return 0;
	}
	int ControllerFeedback_BindLua::SetVibrationRight(lua_State* L)
	{
		...
		if (argc > 0)
		{
			feedback.vibration_right = Luaer::SGetFloat(L, 1);
		}
		else
		{
			Luaer::SError(L, "SetVibrationRight(float value) not enough arguments!");
		}
		return 0;
	}
	int ControllerFeedback_BindLua::SetLEDColor(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (vec != nullptr)
			{
				feedback.led_color = pf::Color::fromFloat4(vec->data);
			}
			else
			{
				feedback.led_color.rgba = Luaer::SGetInt(L, 1);
			}
		}
		else
		{
			Luaer::SError(L, "SetLEDColor(int hexcolor) not enough arguments!");
		}
		return 0;
	}
	...
	void ControllerFeedback_BindLua::Bind()
	{
		...
		{
			initialized = true;
			Luna<ControllerFeedback_BindLua>::Register(Luaer::GetLuaState());
		}
	}
