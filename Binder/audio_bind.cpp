...
namespace Luaer {
	...
	int Audio_BindLua::CreateSound(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (sound != nullptr)
			{
				sound->soundResource = pf::resourcemanager::Load(Luaer::SGetString(L, 1));
				if (!sound->soundResource.IsValid())
				{
					Luaer::SSetBool(L, false);
					return 1;
				}
				Luaer::SSetBool(L, sound->soundResource.GetSound().IsValid());
				return 1;
			}
			else
			{
				Luaer::SError(L, "CreateSound(string filename, Sound sound) second argument is not a Sound!");
			}
		}
		else
			Luaer::SError(L, "CreateSound(string filename, Sound sound) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::CreateSoundInstance(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (s == nullptr)
			{
				Luaer::SError(L, "CreateSoundInstance(Sound sound, SoundInstance soundinstance) first argument is not a Sound!");
				return 0;
			}
			if (!s->soundResource.IsValid())
			{
				Luaer::SError(L, "CreateSoundInstance(Sound sound, SoundInstance soundinstance) first argument Sound resource is not valid, probably it was not loaded successfully!");
				return 0;
			}
			SoundInstance_BindLua* soundinstance = Luna<SoundInstance_BindLua>::lightcheck(L, 2);
			if (soundinstance == nullptr)
			{
				Luaer::SError(L, "CreateSoundInstance(Sound sound, SoundInstance soundinstance) second argument is not a SoundInstance!");
				return 0;
			}

			const pf::audio::Sound& sound = s->soundResource.GetSound();
			bool result = pf::audio::CreateSoundInstance(&sound, &soundinstance->soundinstance);
			Luaer::SSetBool(L, result);
			return 1;
		}
		else
			Luaer::SError(L, "CreateSoundInstance(Sound sound, SoundInstance soundinstance) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::Play(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (soundinstance != nullptr)
			{
				pf::audio::Play(&soundinstance->soundinstance);
			}
			else
			{
				Luaer::SError(L, "Play(SoundInstance soundinstance) argument is not a SoundInstance!");
			}
		}
		else
			Luaer::SError(L, "Play(SoundInstance soundinstance) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::Pause(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (soundinstance != nullptr)
			{
				pf::audio::Pause(&soundinstance->soundinstance);
			}
			else
			{
				Luaer::SError(L, "Pause(SoundInstance soundinstance) argument is not a SoundInstance!");
			}
		}
		else
			Luaer::SError(L, "Pause(SoundInstance soundinstance) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::Stop(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (soundinstance != nullptr)
			{
				pf::audio::Stop(&soundinstance->soundinstance);
			}
			else
			{
				Luaer::SError(L, "Stop(SoundInstance soundinstance) argument is not a SoundInstance!");
			}
		}
		else
			Luaer::SError(L, "Stop(SoundInstance soundinstance) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::GetVolume(lua_State* L)
	{
		...
		{
			SoundInstance_BindLua* soundinstance = Luna<SoundInstance_BindLua>::lightcheck(L, 1);
			float volume = pf::audio::GetVolume(soundinstance == nullptr ? nullptr : &soundinstance->soundinstance);
			Luaer::SSetFloat(L, volume);
			return 1;
		}
		else
			Luaer::SError(L, "GetVolume(opt SoundInstance soundinstance) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::SetVolume(lua_State* L)
	{
		...
		if (argc > 0)
		{
			float volume =Luaer::SGetFloat(L, 1);
			SoundInstance_BindLua* soundinstance = Luna<SoundInstance_BindLua>::lightcheck(L, 2);
			pf::audio::SetVolume(volume, soundinstance == nullptr ? nullptr : &soundinstance->soundinstance);
			return 0;
		}
		else
			Luaer::SError(L, "SetVolume(float volume, opt SoundInstance soundinstance) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::ExitLoop(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (soundinstance != nullptr)
			{
				pf::audio::ExitLoop(&soundinstance->soundinstance);
			}
			else
			{
				Luaer::SError(L, "ExitLoop(SoundInstance soundinstance) argument is not a SoundInstance!");
			}
		}
		else
			Luaer::SError(L, "ExitLoop(SoundInstance soundinstance) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::GetSubmixVolume(lua_State* L)
	{
		...
		if (argc > 0)
		{
			pf::audio::SUBMIX_TYPE type = (pf::audio::SUBMIX_TYPE)Luaer::SGetInt(L, 1);
			float volume = pf::audio::GetSubmixVolume(type);
			Luaer::SSetFloat(L, volume);
			return 1;
		}
		else
			Luaer::SError(L, "GetSubmixVolume(int submixtype) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::SetSubmixVolume(lua_State* L)
	{
		...
		{
			pf::audio::SUBMIX_TYPE type = (pf::audio::SUBMIX_TYPE)Luaer::SGetInt(L, 1);
			float volume =Luaer::SGetFloat(L, 2);
			pf::audio::SetSubmixVolume(type, volume);
			return 0;
		}
		else
			Luaer::SError(L, "SetSubmixVolume(int submixtype, float volume) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::Update3D(lua_State* L)
	{
		...
		if (argc > 1)
		{
			...
			{
				pf::audio::Update3D(&soundinstance->soundinstance, soundinstance3D->soundinstance3D);
			}
			else
			{
				Luaer::SError(L, "Update3D(SoundInstance soundinstance, SoundInstance3D instance3D) argument types mismatch!");
			}
		}
		else
			Luaer::SError(L, "Update3D(SoundInstance soundinstance, SoundInstance3D instance3D) not enough arguments!");
		return 0;
	}
	int Audio_BindLua::SetReverb(lua_State* L)
	{
		...
		if (argc > 0)
		{
			pf::audio::SetReverb((pf::audio::REVERB_PRESET)Luaer::SGetInt(L, 1));
		}
		else
			Luaer::SError(L, "SetReverb(int reverbtype) not enough arguments!");
		return 0;
	}
	...
	void Audio_BindLua::Bind()
	{
		...
		{
			initialized = true;
			Luna<Audio_BindLua>::Register(Luaer::GetLuaState());
...
REVERB_PRESET_PLATE = 29
	...
	Sound_BindLua::Sound_BindLua(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundResource = pf::resourcemanager::Load(Luaer::SGetString(L, 1));
		}
	}

	int Sound_BindLua::IsValid(lua_State* L)
	{
		Luaer::SSetBool(L, soundResource.IsValid() && soundResource.GetSound().IsValid());
		return 1;
	}
	...
	void Sound_BindLua::Bind()
	{
		...
		{
			initialized = true;
			Luna<Sound_BindLua>::Register(Luaer::GetLuaState());
		}
	}
	...
	SoundInstance_BindLua::SoundInstance_BindLua(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			if (s == nullptr)
			{
				Luaer::SError(L, "SoundInstance(Sound sound, opt float begin,length) : first argument is not a Sound!");
				return;
			}
			if (argc > 1)
			{
				soundinstance.begin =Luaer::SGetFloat(L, 2);
				if (argc > 2)
				{
					soundinstance.length =Luaer::SGetFloat(L, 3);
				}
			}
			if (!s->soundResource.IsValid())
			{
				Luaer::SError(L, "SoundInstance(Sound sound, opt float begin,end) : Sound is not valid!");
				return;
			}
			const pf::audio::Sound& sound = s->soundResource.GetSound();
			if (!sound.IsValid())
	...
	int SoundInstance_BindLua::SetSubmixType(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetSubmixType(int submixtype) not enough arguments!");
		return 0;
	}
	int SoundInstance_BindLua::SetBegin(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundinstance.begin =Luaer::SGetFloat(L, 1);
		}
		else
			Luaer::SError(L, "SetBegin(float seconds) not enough arguments!");
		return 0;
	}
	int SoundInstance_BindLua::SetLength(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundinstance.length =Luaer::SGetFloat(L, 1);
		}
		else
			Luaer::SError(L, "SetLength(float seconds) not enough arguments!");
		return 0;
	}
	int SoundInstance_BindLua::SetLoopBegin(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundinstance.loop_begin =Luaer::SGetFloat(L, 1);
		}
		else
			Luaer::SError(L, "SetLoopBegin(float seconds) not enough arguments!");
		return 0;
	}
	int SoundInstance_BindLua::SetLoopLength(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundinstance.loop_length =Luaer::SGetFloat(L, 1);
		}
		else
			Luaer::SError(L, "SetLoopLength(float seconds) not enough arguments!");
		return 0;
	}
	int SoundInstance_BindLua::SetEnableReverb(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundinstance.SetEnableReverb(Luaer::SGetBool(L, 1));
		}
		else
			Luaer::SError(L, "SetEnableReverb(bool value) not enough arguments!");
		return 0;
	}
	int SoundInstance_BindLua::SetLooped(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundinstance.SetLooped(Luaer::SGetBool(L, 1));
		}
		else
	...
	void SoundInstance_BindLua::Bind()
	{
		...
		{
			initialized = true;
			Luna<SoundInstance_BindLua>::Register(Luaer::GetLuaState());
		}
	}
	...
	int SoundInstance3D_BindLua::SetListenerPos(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetListenerPos(Vector value) not enough arguments!");
		return 0;
	}
	int SoundInstance3D_BindLua::SetListenerUp(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetListenerUp(Vector value) not enough arguments!");
		return 0;
	}
	int SoundInstance3D_BindLua::SetListenerFront(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetListenerFront(Vector value) not enough arguments!");
		return 0;
	}
	int SoundInstance3D_BindLua::SetListenerVelocity(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetListenerVelocity(Vector value) not enough arguments!");
		return 0;
	}
	...
	int SoundInstance3D_BindLua::SetEmitterPos(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetEmitterPos(Vector value) not enough arguments!");
		return 0;
	}
	int SoundInstance3D_BindLua::SetEmitterUp(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetEmitterUp(Vector value) not enough arguments!");
		return 0;
	}
	int SoundInstance3D_BindLua::SetEmitterFront(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetEmitterFront(Vector value) not enough arguments!");
		return 0;
	}
	int SoundInstance3D_BindLua::SetEmitterVelocity(lua_State* L)
	{
		...
		if (argc > 0)
		{
			...
			Luaer::SError(L, "SetEmitterVelocity(Vector value) not enough arguments!");
		return 0;
	}
	int SoundInstance3D_BindLua::SetEmitterRadius(lua_State* L)
	{
		...
		if (argc > 0)
		{
			soundinstance3D.emitterRadius =Luaer::SGetFloat(L, 1);
		}
		else
			Luaer::SError(L, "SetEmitterVelocity(Vector value) not enough arguments!");
		return 0;
	...
	void SoundInstance3D_BindLua::Bind()
	{
		...
		{
			initialized = true;
			Luna<SoundInstance3D_BindLua>::Register(Luaer::GetLuaState());
		}
	}
