...
namespace pf::profiler {
	...
	pf::Color background_color = pf::Color(20, 20, 20, 230);
	pf::Color text_color = pf::Color::White();
	...
	pf::unordered_map<size_t, Range> ranges;
	...
	void BeginFrame()
	{
		...
		if (!initialized)
		{
			...
			{
				pf::backlog::post("[pf::profiler] Superluminal Performance API loaded");
			}
#endif // PERFORMANCEAPI_ENABLED
		}
	...
	range_id BeginRangeCPU(const char* name)
	{
		...
		{
			pf::helper::hash_combine(id, differentiator++);
		}
	...
	void LoadShaders()
	{
		...
		desc.rs = pf::renderer::GetRasterizerState(pf::enums::RSTYPE_WIRE_SMOOTH);
	...
	pf::unordered_map<std::string, Hits> time_cache_cpu;
	pf::unordered_map<std::string, Hits> time_cache_gpu;
	void DrawData(
	...
	)
	{
		...
		pf::font::Params params = pf::font::Params(x, y + (graph_size.y + graph_padding_y) * 2, pf::font::WIFONTSIZE_DEFAULT - 6, pf::font::WIFALIGN_LEFT, pf::font::WIFALIGN_TOP, text_color);

		// Background:
		pf::image::Params fx;
		fx.pos.x = x - 10;
		fx.pos.y = y - 10;
		fx.siz.x = std::max(graph_size.x, (float)pf::font::TextWidth(ss.str(), params)) + 200;
		fx.siz.y = (float)pf::font::TextHeight(ss.str(), params) + (graph_size.y + graph_padding_y) * 2 + 20;
		...
		{
			GraphicsDevice* device = pf::graphics::GetDevice();
			pf::graphics::GraphicsDevice::MemoryUsage gpu_memory_usage = device->GetMemoryUsage();
			pf::helper::MemoryUsage cpu_memory_usage = pf::helper::GetMemoryUsage();

			static bool shaders_loaded = false;
			if (!shaders_loaded)
			{
				shaders_loaded = true;
				static pf::eventhandler::Handle handle = pf::eventhandler::Subscribe(pf::eventhandler::EVENT_RELOAD_SHADERS, [](uint64_t userdata) { LoadShaders(); });
				LoadShaders();
			}
			...
			pf::font::Draw("16.6 ms", params, cmd);
			...
			params.color = pf::Color::fromFloat4(XMFLOAT4(1, 0.2f, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			params.color = pf::Color::fromFloat4(XMFLOAT4(0.2f, 1, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			params.color = pf::Color::fromFloat4(XMFLOAT4(1, 0.2f, 1, 1));
			ss.str("");
			ss.clear();
			...
			params.color = pf::Color::fromFloat4(XMFLOAT4(1, 1, 0.2f, 1));
			ss.str("");
			ss.clear();
			...
			pf::font::Draw("60 frames", params, cmd);
			params.position.x = x + graph_size.x;
			pf::font::Draw("current frame", params, cmd);
