...
namespace pf
{
	...
	bool Localization::Import(const std::string& filename)
	{
		...
		{
			pf::backlogger::postin("XML error in " + filename + " at offset = " + std::to_string(result.offset) + ": " + result.description(), LogLevel::Warning);
			return false;
		}
	...
	bool Localization::Export(const std::string& filename) const
	{
		...
		doc.append_child(pugi::node_comment).set_value("You can import this file with the pf::Localization::Import() function.");
