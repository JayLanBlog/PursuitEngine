...
namespace pf::network
{
	...
	bool CreateSocket(Socket* sock)
	{
		...
		{
			pf::backlogger::postin("pf::network_Linux error in CreateSocket: Could not create socket");
			return false;
		}
	...
	bool Send(const Socket* sock, const Connection* connection, const void* data, size_t dataSize)
	{
		if (sock->IsValid()) {
			...
			if (result < 0)
			{
				pf::backlogger::postin("pf::network_Linux error in Send: (Error Code: " + std::to_string(result) + ") " + std::string(strerror(result)));
				return false;
			}
