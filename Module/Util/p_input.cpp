...
#define KEY_UP(vk_code) (!KEY_DOWN(vk_code))
	...
	platform::window_type window = nullptr;
	pf::Canvas canvas;
	...
	pf::vector<Controller> controllers;
	...
	void Initialize()
	{
		...
		pf::input::rawinput::Initialize();
		pf::input::sdlinput::Initialize();

#ifdef PLATFORM_PS5
		pf::input::ps5::Initialize();
#endif // PLATFORM_PS5
		...
		p_log("pf::input Initialized (%d ms)", (int)std::round(timer.elapsed()));
		initialized.store(true);
	}

	void Update(platform::window_type _window, pf::Canvas _canvas)
	{
		window = _window;
		canvas = _canvas;
		if (!initialized.load())
		...
		auto range = pf::profiler::BeginRangeCPU("Input");

		pf::input::xinput::Update();
		pf::input::rawinput::Update();
		pf::input::sdlinput::Update();
...
#ifdef ENABLESDL3
		pf::input::sdlinput::GetMouseState(&mouse);
		pf::input::sdlinput::GetKeyboardState(&keyboard);
#elif
		
		pf::input::rawinput::GetMouseState(&mouse); // currently only the relative data can be used from this
		pf::input::rawinput::GetKeyboardState(&keyboard);
		...
		for (int i = 0; i < pf::input::xinput::GetMaxControllerCount(); ++i)
		{
			if (pf::input::xinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
		...
		for (int i = 0; i < pf::input::rawinput::GetMaxControllerCount(); ++i)
		{
			if (pf::input::rawinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (int i = 0; i < pf::input::sdlinput::GetMaxControllerCount(); ++i)
		{
			if (pf::input::sdlinput::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (int i = 0; i < pf::input::ps5::GetMaxControllerCount(); ++i)
		{
			if (pf::input::ps5::GetControllerState(nullptr, i))
			{
				int slot = -1;
				for (int j = 0; j < (int)controllers.size(); ++j)
				{
		...
		for (auto& controller : controllers)
		{
			...
			case Controller::XINPUT:
				connected = pf::input::xinput::GetControllerState(&controller.state, controller.deviceIndex);
				break;
			case Controller::RAWINPUT:
				connected = pf::input::rawinput::GetControllerState(&controller.state, controller.deviceIndex);
				break;
			case Controller::SDLINPUT:
				connected = pf::input::sdlinput::GetControllerState(&controller.state, controller.deviceIndex);
				break;
			case Controller::PS5:
#ifdef PLATFORM_PS5
				connected = pf::input::ps5::GetControllerState(&controller.state, controller.deviceIndex);
#endif // PLATFORM_PS5
				break;
		...
		if (Press(MOUSE_BUTTON_LEFT))
		{
			...
			if (elapsed < 0.5 && pf::math::Distance(doubleclick_prevpos, pos) < 5)
			{
				double_click = true;
			}
	...
	bool Down(BUTTON button, int playerindex)
	{
		...
		else if (playerindex == 0) // keyboard or mouse
		{
			...
			switch (button)
			{
			case pf::input::MOUSE_BUTTON_LEFT:
				if (mouse.left_button_press)
					return true;
				return false;
			case pf::input::MOUSE_BUTTON_RIGHT:
				if (mouse.right_button_press)
					return true;
				return false;
			case pf::input::MOUSE_BUTTON_MIDDLE:
				if (mouse.middle_button_press)
					return true;
				return false;
#ifdef _WIN32
			case pf::input::KEYBOARD_BUTTON_UP:
				keycode = VK_UP;
				break;
			case pf::input::KEYBOARD_BUTTON_DOWN:
				keycode = VK_DOWN;
				break;
			case pf::input::KEYBOARD_BUTTON_LEFT:
				keycode = VK_LEFT;
				break;
			case pf::input::KEYBOARD_BUTTON_RIGHT:
				keycode = VK_RIGHT;
				break;
			case pf::input::KEYBOARD_BUTTON_SPACE:
				keycode = VK_SPACE;
				break;
			case pf::input::KEYBOARD_BUTTON_RSHIFT:
				keycode = VK_RSHIFT;
				break;
			case pf::input::KEYBOARD_BUTTON_LSHIFT:
				keycode = VK_LSHIFT;
				break;
			case pf::input::KEYBOARD_BUTTON_F1:
				keycode = VK_F1;
				break;
			case pf::input::KEYBOARD_BUTTON_F2:
				keycode = VK_F2;
				break;
			case pf::input::KEYBOARD_BUTTON_F3:
				keycode = VK_F3;
				break;
			case pf::input::KEYBOARD_BUTTON_F4:
				keycode = VK_F4;
				break;
			case pf::input::KEYBOARD_BUTTON_F5:
				keycode = VK_F5;
				break;
			case pf::input::KEYBOARD_BUTTON_F6:
				keycode = VK_F6;
				break;
			case pf::input::KEYBOARD_BUTTON_F7:
				keycode = VK_F7;
				break;
			case pf::input::KEYBOARD_BUTTON_F8:
				keycode = VK_F8;
				break;
			case pf::input::KEYBOARD_BUTTON_F9:
				keycode = VK_F9;
				break;
			case pf::input::KEYBOARD_BUTTON_F10:
				keycode = VK_F10;
				break;
			case pf::input::KEYBOARD_BUTTON_F11:
				keycode = VK_F11;
				break;
			case pf::input::KEYBOARD_BUTTON_F12:
				keycode = VK_F12;
				break;
			case pf::input::KEYBOARD_BUTTON_ENTER:
				keycode = VK_RETURN;
				break;
			case pf::input::KEYBOARD_BUTTON_ESCAPE:
				keycode = VK_ESCAPE;
				break;
			case pf::input::KEYBOARD_BUTTON_HOME:
				keycode = VK_HOME;
				break;
			case pf::input::KEYBOARD_BUTTON_LCONTROL:
				keycode = VK_LCONTROL;
				break;
			case pf::input::KEYBOARD_BUTTON_RCONTROL:
				keycode = VK_RCONTROL;
				break;
			case pf::input::KEYBOARD_BUTTON_INSERT:
				keycode = VK_INSERT;
				break;
			case pf::input::KEYBOARD_BUTTON_DELETE:
				keycode = VK_DELETE;
				break;
			case pf::input::KEYBOARD_BUTTON_BACKSPACE:
				keycode = VK_BACK;
				break;
			case pf::input::KEYBOARD_BUTTON_PAGEDOWN:
				keycode = VK_NEXT;
				break;
			case pf::input::KEYBOARD_BUTTON_PAGEUP:
				keycode = VK_PRIOR;
				break;
	...
	void SetControllerFeedback(const ControllerFeedback& data, int playerindex)
	{
		if (playerindex < (int)controllers.size())
		{
			...
			{
				pf::input::xinput::SetControllerFeedback(data, controller.deviceIndex);
			}
			else if (controller.deviceType == Controller::RAWINPUT)
			{
				pf::input::rawinput::SetControllerFeedback(data, controller.deviceIndex);
			}
			else if (controller.deviceType == Controller::SDLINPUT)
			{
				pf::input::sdlinput::SetControllerFeedback(data, controller.deviceIndex);
			}
#ifdef PLATFORM_PS5
			else if (controller.deviceType == Controller::PS5)
			{
				pf::input::ps5::SetControllerFeedback(data, controller.deviceIndex);
			}
#endif // PLATFORM_PS5
		}
	}
	...
	const pf::vector<Touch>& GetTouches()
	{
		return touches;
	}
	...
	void SetCursorFromFile(CURSOR cursor, const char* filename)
	{
		...
		pf::helper::StringConvert(filename, wfilename, arraysize(wfilename));
		...
		if (pf::helper::FileRead(filename, data))
		{
			...
			for (uint32_t y = height; y > 0; --y)
			{
				...
				{
					colors.push_back(pf::Color(src[3], src[2], src[1], src[0]));
					src += 4;
				}
			}
	...
	ShortReturnString ButtonToString(BUTTON button, CONTROLLER_PREFERENCE preference)
	{
		...
		if (preference == CONTROLLER_PREFERENCE_XBOX)
		{
			switch (button)
			{
			case pf::input::GAMEPAD_BUTTON_XBOX_X: return "(X)";
			case pf::input::GAMEPAD_BUTTON_XBOX_A: return "(A)";
			case pf::input::GAMEPAD_BUTTON_XBOX_B: return "(B)";
			case pf::input::GAMEPAD_BUTTON_XBOX_Y: return "(Y)";
			case pf::input::GAMEPAD_BUTTON_XBOX_L1: return "L1";
			case pf::input::GAMEPAD_BUTTON_XBOX_LT: return "LT";
			case pf::input::GAMEPAD_BUTTON_XBOX_R1: return "R1";
			case pf::input::GAMEPAD_BUTTON_XBOX_RT: return "RT";
			case pf::input::GAMEPAD_BUTTON_XBOX_L3: return "L3";
			case pf::input::GAMEPAD_BUTTON_XBOX_R3: return "R3";
			case pf::input::GAMEPAD_BUTTON_XBOX_BACK: return "Back";
			case pf::input::GAMEPAD_BUTTON_XBOX_START: return "Start";
			default:
				break;
			}
		}
		...
		switch (button)
		{
			//case pf::input::GAMEPAD_BUTTON_UP: return "Dpad ¡ü";
			//case pf::input::GAMEPAD_BUTTON_LEFT: return "Dpad ¡û";
			//case pf::input::GAMEPAD_BUTTON_DOWN: return "Dpad ¡ý";
			//case pf::input::GAMEPAD_BUTTON_RIGHT: return "Dpad ¡ú";
		case pf::input::GAMEPAD_BUTTON_1: return "Gamepad 1";
		case pf::input::GAMEPAD_BUTTON_2: return "Gamepad 2";
		case pf::input::GAMEPAD_BUTTON_3: return "Gamepad 3";
		case pf::input::GAMEPAD_BUTTON_4: return "Gamepad 4";
		case pf::input::GAMEPAD_BUTTON_5: return "Gamepad 5";
		case pf::input::GAMEPAD_BUTTON_6: return "Gamepad 6";
		case pf::input::GAMEPAD_BUTTON_7: return "Gamepad 7";
		case pf::input::GAMEPAD_BUTTON_8: return "Gamepad 8";
		case pf::input::GAMEPAD_BUTTON_9: return "Gamepad 9";
		case pf::input::GAMEPAD_BUTTON_10: return "Gamepad 10";
		case pf::input::GAMEPAD_BUTTON_11: return "Gamepad 11";
		case pf::input::GAMEPAD_BUTTON_12: return "Gamepad 12";
		case pf::input::GAMEPAD_BUTTON_13: return "Gamepad 13";
		case pf::input::GAMEPAD_BUTTON_14: return "Gamepad 14";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_UP: return "Left Stick ¡ü";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_LEFT: return "Left Stick ¡û";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_DOWN: return "Left Stick ¡ý";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_L_AS_BUTTON_RIGHT: return "Left Stick ¡ú";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_UP: return "Right Stick ¡ü";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_LEFT: return "Right Stick ¡û";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_DOWN: return "Right Stick ¡ý";
			//case pf::input::GAMEPAD_ANALOG_THUMBSTICK_R_AS_BUTTON_RIGHT: return "Right Stick ¡ú";
		case pf::input::GAMEPAD_ANALOG_TRIGGER_L_AS_BUTTON: return "Left Trigger";
		case pf::input::GAMEPAD_ANALOG_TRIGGER_R_AS_BUTTON: return "Right Trigger";
		case pf::input::MOUSE_BUTTON_LEFT: return "Left Mouse Button";
		case pf::input::MOUSE_BUTTON_RIGHT: return "Right Mouse Button";
		case pf::input::MOUSE_BUTTON_MIDDLE: return "Middle Mouse Button";
			//case pf::input::MOUSE_SCROLL_AS_BUTTON_UP: return "Mouse Wheel ¡ü";
			//case pf::input::MOUSE_SCROLL_AS_BUTTON_DOWN: return "Mouse Wheel ¡ý";
			//case pf::input::KEYBOARD_BUTTON_UP: return "¡ü";
			//case pf::input::KEYBOARD_BUTTON_DOWN: return "¡ý";
			//case pf::input::KEYBOARD_BUTTON_LEFT: return "¡û";
			//case pf::input::KEYBOARD_BUTTON_RIGHT: return "¡ú";
		case pf::input::KEYBOARD_BUTTON_SPACE: return "Space";
		case pf::input::KEYBOARD_BUTTON_RSHIFT: return "Right Shift";
		case pf::input::KEYBOARD_BUTTON_LSHIFT: return "Left Shift";
		case pf::input::KEYBOARD_BUTTON_F1: return "F1";
		case pf::input::KEYBOARD_BUTTON_F2: return "F2";
		case pf::input::KEYBOARD_BUTTON_F3: return "F3";
		case pf::input::KEYBOARD_BUTTON_F4: return "F4";
		case pf::input::KEYBOARD_BUTTON_F5: return "F5";
		case pf::input::KEYBOARD_BUTTON_F6: return "F6";
		case pf::input::KEYBOARD_BUTTON_F7: return "F7";
		case pf::input::KEYBOARD_BUTTON_F8: return "F8";
		case pf::input::KEYBOARD_BUTTON_F9: return "F9";
		case pf::input::KEYBOARD_BUTTON_F10: return "F10";
		case pf::input::KEYBOARD_BUTTON_F11: return "F11";
		case pf::input::KEYBOARD_BUTTON_F12: return "F12";
		case pf::input::KEYBOARD_BUTTON_ENTER: return "Enter";
		case pf::input::KEYBOARD_BUTTON_ESCAPE: return "Escape";
		case pf::input::KEYBOARD_BUTTON_HOME: return "Home";
		case pf::input::KEYBOARD_BUTTON_RCONTROL: return "Right Control";
		case pf::input::KEYBOARD_BUTTON_LCONTROL: return "Left Control";
		case pf::input::KEYBOARD_BUTTON_DELETE: return "Delete";
		case pf::input::KEYBOARD_BUTTON_BACKSPACE: return "Backspace";
		case pf::input::KEYBOARD_BUTTON_PAGEDOWN: return "Page Down";
		case pf::input::KEYBOARD_BUTTON_PAGEUP: return "Page Up";
		case pf::input::KEYBOARD_BUTTON_NUMPAD0: return "Numpad 0";
		case pf::input::KEYBOARD_BUTTON_NUMPAD1: return "Numpad 1";
		case pf::input::KEYBOARD_BUTTON_NUMPAD2: return "Numpad 2";
		case pf::input::KEYBOARD_BUTTON_NUMPAD3: return "Numpad 3";
		case pf::input::KEYBOARD_BUTTON_NUMPAD4: return "Numpad 4";
		case pf::input::KEYBOARD_BUTTON_NUMPAD5: return "Numpad 5";
		case pf::input::KEYBOARD_BUTTON_NUMPAD6: return "Numpad 6";
		case pf::input::KEYBOARD_BUTTON_NUMPAD7: return "Numpad 7";
		case pf::input::KEYBOARD_BUTTON_NUMPAD8: return "Numpad 8";
		case pf::input::KEYBOARD_BUTTON_NUMPAD9: return "Numpad 9";
		case pf::input::KEYBOARD_BUTTON_MULTIPLY: return "*";
		case pf::input::KEYBOARD_BUTTON_ADD: return "+";
		case pf::input::KEYBOARD_BUTTON_SEPARATOR: return "Separator";
		case pf::input::KEYBOARD_BUTTON_SUBTRACT: return "-";
		case pf::input::KEYBOARD_BUTTON_DECIMAL: return "Decimal";
		case pf::input::KEYBOARD_BUTTON_DIVIDE: return "/";
		case pf::input::KEYBOARD_BUTTON_TAB: return "Tab";
		case pf::input::KEYBOARD_BUTTON_TILDE: return "Tilde";
		case pf::input::KEYBOARD_BUTTON_INSERT: return "Insert";
		case pf::input::KEYBOARD_BUTTON_ALT: return "Alt";
		case pf::input::KEYBOARD_BUTTON_ALTGR: return "Alt Gr";
		default:
			break;
		}
